...
FrameGraphShadowOutputs AddShadowPasses(FrameGraph& frameGraph)
{
	...
	for (std::size_t lightIndex = 0; lightIndex < FrameGraphShadowOutputs::MaxShadowMaps; ++lightIndex)
	{
		...
		outputs.ShadowMaps[lightIndex] = frameGraph.CreateTexture(FrameGraphTextureDesc::CreateColor(
		    mapName.c_str(),
		    RenderConfig::Shadows::ShadowMapResolution,
		    RenderConfig::Shadows::ShadowMapResolution,
		    RenderConfig::Shadows::ShadowMapFormat));
		const TextureHandle shadowDepthHandle = frameGraph.CreateTexture(FrameGraphTextureDesc::CreateDepthStencil(
		    depthName.c_str(),
		    RenderConfig::Shadows::ShadowMapResolution,
		    RenderConfig::Shadows::ShadowMapResolution));
		...
		frameGraph.AddRasterPass<ShadowOpaquePass>(
		    passName,
		    shadowParameters,
		    lightIndex);
	}
