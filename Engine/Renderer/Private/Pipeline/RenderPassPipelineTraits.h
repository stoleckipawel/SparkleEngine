...
template <> struct RenderPassPipelineTraits<GBufferPass>
{
	    ...
	    std::string& outErrorMessage)
	{
		return RenderPassShaderRuntime::TryCreateGraphicsRuntime(
		    ...
		    [](GraphicsPipelineStateDesc& pipelineDesc)
		    {
			    ...
			    pipelineDesc.RenderTargetFormats[6] = RenderConfig::GBuffer::MotionVectorFormat;
			    pipelineDesc.RenderTargetCount = 7;
	...
	{
		return RuntimeType{*storage.BindingLayout, *storage.PipelineState, storage.WireframePipelineState.get(), storage.TwoSidedPipelineState.get()};
	}
};
