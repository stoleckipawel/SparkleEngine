...
struct PreparedRenderObject final
{
	...
	RenderMeshInstanceGroupIndex InstanceGroupIndex =
	    kInvalidRenderMeshInstanceGroupIndex;
	std::uint32_t MaterialAlphaMode = 0u;
	RenderMaterialClassification MaterialClassification =
...
class RenderObjectPreparation final
{
  ...
  private:
	static RenderMeshWorldBounds TransformBounds(
	    const MeshDrawGeometry& geometry,
	    const DirectX::XMFLOAT4X4& worldMatrix) noexcept;
	static std::array<DirectX::XMFLOAT3, 8> BuildBoundsCorners(
	...
	static RenderMaterialClassification ClassifyMaterial(
