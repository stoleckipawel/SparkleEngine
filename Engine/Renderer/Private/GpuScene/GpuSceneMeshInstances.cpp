#include "../../PCH.h"
#include "GpuScene/GpuSceneMeshInstances.h"
...
static const auto g_gpuSceneMeshInstancesLogger = Logging::GetOrCreateLogger("Renderer.GpuScene.MeshInstances");

GpuSceneMeshInstances GpuSceneMeshInstances::Build(RenderHardwareInterface& renderHardwareInterface, const RenderSceneData& sceneData)
{
	...
	{
		SPDLOG_LOGGER_WARN(
		    g_gpuSceneMeshInstancesLogger,
		    "GpuSceneMeshInstances::Build: failed to upload {} mesh instance records ({} bytes).",
		    instances.size(),
		    instances.size() * sizeof(MeshInstanceData));
		return {};
	}

	GpuSceneMeshInstances frameData;
