    ...
    const RenderFrameGraphImportedSceneResources& externalResources)
{
	...
	parameters->BaseColor = builder.CreateRenderTarget(
	    targets.BaseColor,
	    FrameGraphAttachmentLoadAction::Clear,
	    FrameGraphAttachmentStoreAction::Store);
	parameters->Normal = builder.CreateRenderTarget(
	    targets.Normal,
	    FrameGraphAttachmentLoadAction::Clear,
	    FrameGraphAttachmentStoreAction::Store);
	parameters->Material = builder.CreateRenderTarget(
	    targets.Material,
	    FrameGraphAttachmentLoadAction::Clear,
	    FrameGraphAttachmentStoreAction::Store);
	parameters->Emissive = builder.CreateRenderTarget(
	    targets.Emissive,
	    FrameGraphAttachmentLoadAction::Clear,
	    FrameGraphAttachmentStoreAction::Store);
	parameters->Subsurface = builder.CreateRenderTarget(
	    targets.Subsurface,
	    FrameGraphAttachmentLoadAction::Clear,
	    FrameGraphAttachmentStoreAction::Store);
	parameters->MotionVector = builder.CreateRenderTarget(
