...
void RestirIndirectTemporalPassParameters::Describe(ShaderParameterStructBuilder<RestirIndirectTemporalPassParameters>& builder)
{
	...
	builder.Uniform("SceneLighting", &RestirIndirectTemporalPassParameters::SceneLighting, ShaderStageVisibility::Compute);
...
{
	return ComputePassOperations::BuildParameterMetadata<RestirIndirectTemporalPass>();
}
...
void RestirIndirectTemporalPass::Execute(
    PassCommandContext& context,
    ParameterInstance& parameters,
    std::uint32_t outputWidth,
    std::uint32_t outputHeight) const
{
	ComputePassOperations::DispatchSized<RestirIndirectTemporalPass>(context, m_runtime, parameters, outputWidth, outputHeight);
}
