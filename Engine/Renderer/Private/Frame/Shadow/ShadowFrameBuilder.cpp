	...
	ShadowBuilder& shadowBuilder) const
{
	...
	const std::size_t lightCount = std::min(
	    ...
	    PerViewLightingConstantBufferData::MaxDirectionalLights);
	...
	for (std::size_t lightIndex = 0; lightIndex < lightCount; ++lightIndex)
	{
		...
		for (std::size_t cascadeIndex = 0; cascadeIndex < ShadowFrameBuildResult::MaxShadowCascades; ++cascadeIndex)
		{
			...
			const ShadowBuildResult shadowData = shadowBuilder.Build(
			    mainCamera,
			    sceneData.directionalLights[lightIndex].direction,
			    cascadeRange.nearZ,
			    cascadeRange.farZ);
