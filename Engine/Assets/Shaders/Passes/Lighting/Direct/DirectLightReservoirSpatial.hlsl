...
[numthreads(8, 8, 1)] void main(uint3 dispatchThreadId : SV_DispatchThreadID)
{
	...
	if (!surface.Valid)
	{
		...
		CurrentReservoirSampleTexture[pixelCoord] = DirectLightReservoir::PackReservoirSample(emptyReservoir);
		CurrentReservoirWeightTexture[pixelCoord] = DirectLightReservoir::PackReservoirWeight(emptyReservoir);
		CurrentReservoirSurfaceTexture[pixelCoord] = 0.0f.xxxx;
		return;
	}
	...
	[unroll] for (uint sampleIndex = 0u; sampleIndex < RestirReservoirCommon::SpatialNeighborCount; ++sampleIndex)
	{
		...
		if (neighborCoord.x < 0 ||
		    neighborCoord.y < 0 ||
		    neighborCoord.x >= (int)width ||
		    neighborCoord.y >= (int)height)
		{
			continue;
		}

		const DirectLightReservoir::Surface neighborSurface =
		    DirectLightReservoir::LoadSurface((uint2)neighborCoord);
	...
	CurrentReservoirSampleTexture[pixelCoord] = DirectLightReservoir::PackReservoirSample(reservoir);
	CurrentReservoirWeightTexture[pixelCoord] = DirectLightReservoir::PackReservoirWeight(reservoir);
	CurrentReservoirSurfaceTexture[pixelCoord] = DirectLightReservoir::PackSurface(surface);
}
