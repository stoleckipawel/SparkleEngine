...
#include "/Engine/RayTracing/ReferencePathTracerUniform.hlsli"

Texture2D SkyTexture;
SamplerState SamplerLinearClamp;

void ClearReferencePathTracerIndirectLightingPixel(uint2 pixelCoord)
{
...
void main(uint3 dispatchThreadId : SV_DispatchThreadID)
{
	...
	{
		ClearReferencePathTracerIndirectLightingPixel(pixelCoord);
		return;
	}
	const RayTracingPathTrace::TraceSettings traceSettings =
	    RayTracingPathTrace::BuildSurfaceTraceSettings(ReferencePathTracerNormalBias, ReferencePathTracerMaxDistance);

	float3 indirectDiffuse = 0.0f.xxx;
	float3 indirectSpecular = 0.0f.xxx;
	const uint sampleCount = max(ReferencePathTracerSamplesPerPixel, 1u);
	[loop]
	for (uint sampleIndex = 0u; sampleIndex < sampleCount; ++sampleIndex)
	{
		const RayTracingPathLighting::Result path =
		    RayTracingPathLighting::TraceSurfacePath(SkyTexture,
		                                             ...
		                                             ReferencePathTracerBounceCount,
		                                             traceSettings);
