...
namespace Exposure
{
	    ...
	    float sceneAverageLuminance)
	{
		...
		const float unclampedExposure =
		    exposureMode == ExposureModeManual
		        ? max(manualExposure, 0.0f) * compensation
		        : (max(targetLuminance, MinimumMeteredLuminance) / max(sceneAverageLuminance, MinimumMeteredLuminance)) *
		              compensation;
	    ...
	    float adaptationSpeedDown)
	{
		...
		const float adaptationSpeed =
		    max(safeTargetExposure > safePreviousExposure ? adaptationSpeedUp : adaptationSpeedDown, 0.0f);
