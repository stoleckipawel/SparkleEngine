...
namespace RayTracingGBufferPathSurface
{
	...
	Surface Load(uint2 pixelCoord)
	{
		...
		const float3 positionWorld = ReconstructGBufferWorldPosition(
		    pixelCoord,
		    surface.GBuffer.SceneDepth,
		    Camera.InvViewMTX,
		    Camera.InvProjectionMTX);
		const float3 cameraToSurface = positionWorld - Camera.Position;
		surface.ViewDistance = length(cameraToSurface);
		const float3 viewDirWorld = surface.ViewDistance > 1.0e-5f
		                                ? -cameraToSurface / surface.ViewDistance
		                                : surface.GBuffer.NormalWorld;
		surface.PathSurface = BuildPrimaryRayTracingPathSurface(
		    positionWorld,
		    surface.GBuffer.NormalWorld,
		    viewDirWorld,
