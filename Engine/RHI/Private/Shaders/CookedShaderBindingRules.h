...
{
	inline bool HasAllStages(ShaderStageMask value, ShaderStageMask flags) noexcept
	{
		return (static_cast<std::uint8_t>(value) & static_cast<std::uint8_t>(flags)) == static_cast<std::uint8_t>(flags);
	}

	inline ShaderStageMask ToPackageStageMask(ShaderStageVisibility visibility) noexcept
	{
		switch (visibility)
		{
	...
	inline bool ResourceKindMatchesSemantic(
	    CookedShaderResourceKind resourceKind, ShaderParameterSemanticKind semanticKind) noexcept
	{
		switch (semanticKind)
		{
			...
			case ShaderParameterSemanticKind::ReadTexture: return resourceKind == CookedShaderResourceKind::Texture;
			...
			case ShaderParameterSemanticKind::DepthTarget: return resourceKind == CookedShaderResourceKind::RWTexture;
			...
			case ShaderParameterSemanticKind::SamplerSet: return resourceKind == CookedShaderResourceKind::Sampler;
