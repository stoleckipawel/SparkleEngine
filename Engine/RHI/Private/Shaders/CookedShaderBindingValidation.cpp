...
public:
	...
	private:
		[[noreturn]] static void Reject(std::string message) { throw Diagnostics::Error(std::move(message)); }
		...
		void ValidateReflectionAvailability()
		{
			...
			{
				runtimeBindingStages |=
				    CookedShaderBindingRules::ToPackageStageMask(expectedParameter.Visibility) & m_definition.ExpectedStages;
			}
		...
		bool IsRelevantBinary(const CookedShaderBinaryRecord& binaryRecord) const noexcept
		{
			return m_package.IsRuntimeBinary(binaryRecord, m_runtimeBinaryFormat)
			    && CookedShaderBindingRules::HasAllStages(m_definition.ExpectedStages, ToShaderStageMask(binaryRecord.Stage));
		}
		    ...
		    ShaderStage stage) const noexcept
		{
			...
			for (const PassParameterDesc& expectedParameter : m_expectedParameters)
			{
				if (CookedShaderBindingRules::HasAllStages(
				        CookedShaderBindingRules::ToPackageStageMask(expectedParameter.Visibility),
				        stageMask)
				    && expectedParameter.Name == resourceName
		...
		void ValidateReflectionResources(const CookedShaderBinaryRecord& binaryRecord, const CookedShaderReflectionRecord& reflection)
		{
			for (std::uint32_t resourceIndex = 0; resourceIndex < reflection.ResourceBindingCount; ++resourceIndex)
			{
				...
				const std::string_view resourceName =
				    m_package.ResolveString(CookedShaderStringRef{resourceBinding.NameOffsetInBytes, resourceBinding.NameSizeInBytes});
				...
				{
					Reject(
					    CookedShaderBindingDiagnostics::Append(
					        std::format(
			...
			for (const PassParameterDesc& expectedParameter : m_expectedParameters)
			{
				if (!ContainsExpectedParameter(expectedParameter))
				{
					Reject(
					    CookedShaderBindingDiagnostics::Append(
					        std::format(
					            "Cooked shader package '{}' failed package contract check: runtime parameter '{}' is missing reflected "
		...
		bool ContainsExpectedParameter(const PassParameterDesc& expectedParameter)
		{
			...
			for (std::size_t reflectionIndex = 0; reflectionIndex < reflectionRecords.size(); ++reflectionIndex)
			{
				...
				for (std::uint32_t resourceIndex = 0; resourceIndex < reflection.ResourceBindingCount; ++resourceIndex)
				{
					...
					const std::string_view resourceName =
					    m_package.ResolveString(CookedShaderStringRef{resourceBinding.NameOffsetInBytes, resourceBinding.NameSizeInBytes});
					if (resourceName == expectedParameter.Name
...
{
	CookedShaderBindingValidationImplementation::ReflectedBindingValidator(package, definition, expectedParameters, runtimeBinaryFormat)
	    .Validate();
}
