...
class RhiMemoryCategoryAggregationPolicy final
{
  public:
	...
	static RhiMemoryCategoryAggregation<BlockIdentity>& FindOrCreate(
	    ...
	    std::uint64_t budgetBytes = 0)
	{
		...
		aggregations.push_back(RhiMemoryCategoryAggregation<BlockIdentity>{
		    .Stats = RhiMemoryCategoryStats{
		        .Category = category,
	...
	template <typename BlockIdentity>
	static void AddUniqueBlock(
	    RhiMemoryCategoryAggregation<BlockIdentity>& aggregation,
	    BlockIdentity block,
	    std::uint64_t budgetBytes = 0)
	{
