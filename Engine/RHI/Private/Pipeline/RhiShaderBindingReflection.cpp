...
class RhiShaderBindingReflectionImpl final
{
  public:
	    ...
	    CookedShaderBinaryFormat binaryFormat)
	{
		...
		const PassParameterDesc* const parameterDesc = parameterLayout.FindParameter(bindingName);
		if (parameterDesc == nullptr)
		{
			Fail(std::format(
			    "Shader binding '{}' has no matching parameter in layout '{}'.",
			    bindingName,
			    parameterLayout.GetDebugName()));
		}
		     ...
		     ++reflectionIndex)
		{
			...
			for (std::uint32_t resourceIndex = 0; resourceIndex < reflection.ResourceBindingCount; ++resourceIndex)
			{
				...
				const auto existing = std::ranges::find_if(
				    ...
				    {
					    return location.BindingPoint.Set == bindingPoint.Set &&
					           location.BindingPoint.Binding == bindingPoint.Binding;
				    });
				...
				{
					locations.push_back(RhiReflectedBindingLocation{
					    .BindingPoint = bindingPoint,
					    .VisibilityMask = ToShaderStageMask(binaryRecord.Stage)});
				}
			}
		}
		...
		{
			Fail(std::format(
			    "Shader parameter '{}' did not resolve to a reflected resource named '{}'.",
			    bindingName,
			    shaderName));
		}
