...
void FbxGeometryImporter::ImportGeometry(const aiScene& scene, SceneImportResult& result)
{
	...
	SceneImportResult& result)
{
	...
	for (unsigned int meshReferenceIndex = 0; meshReferenceIndex < node.mNumMeshes; ++meshReferenceIndex)
	{
		...
		{
			LOG_WARNING(std::format("FbxImporter: Node '{}' references invalid mesh index {}", GetNodeName(node), sceneMeshIndex));
			continue;
		}
...
MeshData FbxGeometryImporter::ExtractMeshGeometry(const aiMesh& mesh, const aiNode& node, SceneImportResult& result)
{
	...
	if (mesh.HasBones())
	{
		LOG_WARNING(std::format("FbxImporter: Mesh '{}' contains bones and will be imported as static geometry only", GetMeshName(mesh)));
	}

	if (mesh.mNumAnimMeshes > 0)
	{
		LOG_WARNING(std::format("FbxImporter: Mesh '{}' contains morph targets which will be ignored", GetMeshName(mesh)));
	}
	...
	if (!meshData.IsValid())
	{
		LOG_WARNING(std::format("FbxImporter: Mesh '{}' did not produce valid triangle geometry", GetMeshName(mesh)));
	}
...
void FbxGeometryImporter::AppendTriangleIndices(const aiMesh& mesh, MeshData& meshData, SceneImportResult& result)
{
	for (unsigned int faceIndex = 0; faceIndex < mesh.mNumFaces; ++faceIndex)
	{
		...
		{
			LOG_WARNING(std::format("FbxImporter: Skipping non-triangle face {} in mesh '{}'", faceIndex, GetMeshName(mesh)));
			continue;
		}
...
MaterialHandle FbxGeometryImporter::ResolveMaterialHandle(const aiMesh& mesh, SceneImportResult& result) noexcept
{
