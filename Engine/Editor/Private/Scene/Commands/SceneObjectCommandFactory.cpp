	...
	const EditorSceneModel& model, const SceneObjectSelection& selection, bool visible)
{
	...
	if (selection.type == SceneObjectType::Light)
	{
		...
		if (light == nullptr) return std::nullopt;
...
std::optional<EditorCommandPair> SceneObjectCommandFactory::SetActiveCamera(
	const EditorSceneModel& model, EntityId camera)
{
	if (model.FindCamera(camera) == nullptr) return std::nullopt;
	EntityId previous = EntityId::Invalid();
	for (const WorldCameraReadData& entry : model.GetCameras())
		if (entry.Active) previous = entry.Entity;
	if (previous == camera || !previous.IsValid()) return std::nullopt;
