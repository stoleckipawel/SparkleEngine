...
namespace Json
{
	...
	SPARKLE_CORE_API bool TryReadStringProperty(
	    std::string_view objectText,
	    std::string_view key,
	    std::string& outValue);

	template <typename TNumber>
	bool TryReadUnsignedIntegerProperty(
	    std::string_view objectText,
	    std::string_view key,
	    TNumber& outValue)
	{
		...
		const std::from_chars_result result =
		    std::from_chars(objectText.data() + numberStart, objectText.data() + cursor, outValue);
		return result.ec == std::errc {} && result.ptr == objectText.data() + cursor;
	}

	SPARKLE_CORE_API bool TryReadUInt64Property(
	    std::string_view objectText,
	    std::string_view key,
