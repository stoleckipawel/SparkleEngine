...
namespace Paths
{
	SPARKLE_CORE_API std::array<std::filesystem::path, 3> ExecutableLookupCandidates(
	    std::string_view executableFileName);
	SPARKLE_CORE_API std::filesystem::path LogFile(
	    std::string_view configuredFile = {},
	    bool ensureParentExists = true);
	...
	SPARKLE_CORE_API std::filesystem::path CookedSceneManifestRelative(
	    const std::filesystem::path& relativeManifestPath);
	...
	SPARKLE_CORE_API std::filesystem::path ShaderRecookSignal(
