...
class SPARKLE_ENGINE_API RenderObjectId final
{
  public:
	...
	static constexpr RenderObjectId FromParts(Value value, std::uint32_t generation) noexcept
	{
		return RenderObjectId(value, generation);
	}
  ...
  private:
	constexpr RenderObjectId(Value value, std::uint32_t generation) noexcept : m_value(value), m_generation(generation) {}
