...
namespace SparkleLauncher
{
	...
	inline QString FormatDirectoryInventory(const std::filesystem::path& path)
	{
		std::error_code errorCode;
	...
	inline QString FormatStatusPath(const std::filesystem::path& path)
	{
	...
	inline bool ReadinessContains(const std::vector<std::string>& messages, const QString& needle)
	{
	...
	inline QString CombineStatusDetail(const QString& first, const QString& second)
	{
		if (first.isEmpty())
		{
			return second;
	...
	inline QString BuildFilesRecoveryHint(const BuildFilesFreshnessStatus& freshness)
	{
