...
namespace
{
	...
	Assets::CookedAnimationTargetPath ToCookedTargetPath(ImportedAnimationTargetPath targetPath) noexcept
	{
		switch (targetPath)
		{
			...
			default:
				return Assets::CookedAnimationTargetPath::Unknown;
		}
	}

	Assets::CookedAssetId ResolveTargetSkeletonAssetId(const CookedSceneBuild& build, const ImportedAnimationClip& clip) noexcept
	{
		if (clip.targetSkeletonIndex >= build.manifest.skeletonRefs.size())
		{
			return Assets::InvalidCookedAssetId;
		}
...
void CookedSceneAnimationBuilder::BuildAnimations(const SourceImportResult& importResult, std::string_view sceneAssetId, CookedSceneBuild& outBuild)
{
	...
	outBuild.manifest.animationRefs.reserve(importResult.scene.animations.size());

	for (const ImportedAnimationClip& importedClip : importResult.scene.animations)
	{
		if (!importedClip.IsValid())
		{
			continue;
		}
		...
		for (const ImportedAnimationChannel& importedChannel : importedClip.channels)
		{
			...
			{
				animationAsset.keyframes.push_back(
				    Assets::CookedAnimationKeyframeRecord{
			...
			animationAsset.channels.push_back(
			    Assets::CookedAnimationChannelRecord{
			        .targetPath = ToCookedTargetPath(importedChannel.targetPath),
		...
		if (!animationAsset.channels.empty() && !animationAsset.keyframes.empty())
		{
			outBuild.manifest.animationRefs.push_back(
			    Assets::CookedSceneAnimationRef{
			        .animationAssetId = animationAsset.assetId,
