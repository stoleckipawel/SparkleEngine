	...
	SceneImportResult& result)
{
	switch (textureType)
	{
		...
		{
			const std::optional<std::filesystem::path> metallicRoughnessTexture =
			    ResolveTexturePath(material, materialHandle, sourceDirectory, aiTextureType_SPECULAR, "specular", result);
			materialDesc.SetTexturePath(textureType, metallicRoughnessTexture);
		...
		{
			std::optional<std::filesystem::path> occlusionTexture = ResolveTexturePath(
			    material,
			    materialHandle,
			    sourceDirectory,
			...
			if (!occlusionTexture)
			{
				occlusionTexture = ResolveTexturePath(material, materialHandle, sourceDirectory, aiTextureType_SPECULAR, "specular", result);
			}

			materialDesc.SetTexturePath(textureType, occlusionTexture);
			break;
		}
