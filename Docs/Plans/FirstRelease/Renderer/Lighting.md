...
**Architecture:** [Lighting](../../../Architecture/Modules/Engine/Renderer/Features/Lighting/README.md), [Direct Lighting](../../../Architecture/Modules/Engine/Renderer/Features/Lighting/DirectLighting.md), [Indirect Lighting](../../../Architecture/Modules/Engine/Renderer/Features/Lighting/IndirectLighting.md), and [Offline Path Tracer](../../../Architecture/Modules/Engine/Renderer/Features/Lighting/OfflinePathTracer/README.md)
...
flowchart LR
    ...
    D -->|PASS| P[PTD-01<br/>new implementation plan]
...
| `PTD-01` | `FCR-REN-08`, implementation | intentionally cannot be authored until accepted discovery outputs freeze its goals |
| `LGT-4` | all three | candidate comparison/adoption reports absent |

> [!CAUTION]
> `LGT-3` is a hard planning stop. The [discovery contract](../../../Architecture/Modules/Engine/Renderer/Features/Lighting/OfflinePathTracer/Discovery.md) deliberately requires `PTD-00` to pass before `PTD-01` can create a detailed implementation plan. Pre-writing transport goals, numerical tolerances, or an implementation prompt would manufacture the very assumptions discovery must test.

NVIDIA's [RTXDI integration guide](https://github.com/NVIDIA-RTX/RTXDI/blob/main/Doc/Integration.md) is a primary reference for the boundary where the application owns scene, materials, GBuffer, rays, and API integration while the SDK supplies reservoir algorithms. [RTX Path Tracing](https://github.com/NVIDIA-RTX/RTXPT) and the repository's [completion study](../../../Research/GraphicsArchitecture/OfflinePathTracerCompletion.md) are precedents and research inputs, never local proof.
...
**Required work:** execute every discovery item, risk, failure mode, check, and evidence package named by the [binding `PTD-00` contract](../../../Architecture/Modules/Engine/Renderer/Features/Lighting/OfflinePathTracer/Discovery.md); use the [research study](../../../Research/GraphicsArchitecture/OfflinePathTracerCompletion.md) and pinned primary sources; distinguish verified local source, observed behavior, mathematical requirement, precedent, unknown, and release decision.
...
**Phase exit criteria:** the Architecture gate records `PASS` with all required outputs, or the release records `BLOCKED`/re-scope. Only `PASS` authorizes `PTD-01`.

**Ready-to-use prompt:**

```text
Execute PTD-00 exactly as Docs/Architecture/Modules/Engine/Renderer/Features/Lighting/OfflinePathTracer/Discovery.md specifies. Create its iteration record at the current revision and preserve source/observed/reference distinctions. Inspect the complete local transport, material, light, ray, accumulation, export, reset, backend, package, and evidence routes; run only the discovery checks the contract requires. Compare against the pinned NVIDIA/Falcor/RTXPT and mathematical sources for explicit questions, recording revisions and transfer boundaries. Produce the required evidence package and binary gate decision. Do not change implementation or create PTD-01 on BLOCKED/incomplete results.
```

## Conditional `PTD-01` Handoff — Author The Offline Plan From Accepted Facts
...
**Phase exit criteria:** `Docs/Plans/Renderer/OfflinePathTracer.md` exists only after gate authorization, is linked from plan indexes, maps every accepted discovery output and acceptance row, and contains per-phase goals/non-goals/failures/exit criteria/prompts.

**Ready-to-use prompt:**

```text
Run this prompt only if PTD-00 has an explicit PASS and complete accepted evidence package. Create PTD-01 as Docs/Plans/Renderer/OfflinePathTracer.md. Derive its current-state baseline, transport scope, numerical oracles/tolerances, material/light/backend/package matrices, dependencies, and stop conditions only from the accepted PTD-00 outputs and Architecture OfflinePathTracer contract. Map every included OPT-FS, AC-OPT, FM-OPT, and CHK-OPT exactly once. Split work into owner-sized vertical phases; for each provide goal, non-goals, required implementation, controlled failures, phase exit criteria, validation escalation, reference-transfer boundary, and a ready-to-run implementation prompt. Do not edit code, copy acceptance authority, or add vendor architecture by analogy. Link the plan from the indexes and validate coverage before handoff.
```
