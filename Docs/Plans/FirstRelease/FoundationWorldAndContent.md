...
**Primary Architecture route:** [Engine modules](../../Architecture/Modules/Engine/README.md), [Tools](../../Architecture/Modules/Tools/README.md), [Shader System](../../Architecture/CrossModule/ShaderSystem/README.md), and [Multithreaded Engine](../../Architecture/CrossModule/MultithreadedEngine.md)
...
**Non-goals:** executing the entire [Shader System target plan](../CrossModule/ShaderSystem.md) merely because it exists, preserving legacy shader layouts, or adding a second diagnostic schema.
...
```text
Implement FWC-5 for FCR-SHDR-01 and FCR-TOOL-01. Audit shader registration, CMake/generated membership, cook planner, compiler backends, reflection/ABI, global map/code library, Renderer materialization consumers, replacement/retirement, and all ToolConsoleSupport callers. Compare the release criteria with Docs/Plans/CrossModule/ShaderSystem.md; execute only a selected required phase, not a wholesale redesign. Close missing jobs and ABI checks through the existing owner, make publication transactional, and remove replaced paths. Exercise missing/corrupt/stale shaders, include changes, backend compiler failure, reflection mismatch, reload in flight, cancellation, Unicode paths, stdout/stderr, exit codes, and false-success summaries.
```
