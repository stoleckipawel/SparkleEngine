...
An offline path-tracer implementation plan is intentionally absent. The [completion study](../Research/OfflinePathTracerCompletion.md) and [`PTD-00` discovery contract](../Acceptance/Renderer/OfflinePathTracerDiscovery.md) must pass before `PTD-01` may add that plan; its later presence will not itself authorize implementation.
