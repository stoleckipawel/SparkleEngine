...
| `FCR-REN-08` | Reference Path Tracer | **50** | `45/5/0/0` | Stage 6 **IMPLEMENTED / VALIDATION DEFERRED**: the host-agnostic alternate recipe now contains one feature-owned GPU session with component digest invalidation, exact ordered prefix commit, persistent working/committed mean/M2 resources, bounded row scheduling, Lit suspension/retention, exact progress, and a committed display derivative over the Stage-5 transport; the current candidate is not compiled/cooked or GPU-exercised and the user-selectable view mode remains hidden until Stage 7 | carry forward current-source compile/cook, higher-precision accumulation and complete lifecycle/response-budget evidence, raw GPU analytic/statistical/fault and robustness evidence, independent precision review, live UX/transitions, parity, packaging, and full oracle evidence; grant no executable acceptance credit until run |
