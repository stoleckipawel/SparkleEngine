...
**Snapshot:** 2026-09-13 against committed revision `9689e6ba870a01ef703da723648d3837e6b20863` plus the current uncommitted Reference Path Tracer Stage-6 working tree; no release-approved candidate report was found
...
| `FCR-REN-08` | Reference Path Tracer | **50** | `45/5/0/0` | Stage 7 **IMPLEMENTED / VALIDATION DEFERRED**: the host-agnostic alternate recipe and feature-owned GPU session now have an Editor menu row immediately after Lit, automatic start, exact progress/reset/discard projection, pause/resume/restart, Lit suspension/retention, capacity transfer, route/backend disclosure, and a feature-local overlay; Game/runtime uses the same ordinary request API; the current candidate is not compiled/cooked, GPU-exercised, or interactively reviewed | carry forward current-source compile/cook, higher-precision accumulation and complete lifecycle/response-budget evidence, raw GPU analytic/statistical/fault and robustness evidence, independent precision review, Editor/Game interaction and accessibility, parity, Shipping exclusion, packaging, and full oracle evidence; grant no executable acceptance credit until run |
