...
**Snapshot:** 2026-09-13 against committed revision `5e6badf03df44133eb75db54875417821a9909c8` plus the current uncommitted Reference Path Tracer Stage-5 working tree; no release-approved candidate report was found
...
| `FCR-REN-08` | Reference Path Tracer | **40** | `35/5/0/0` | Stage 5 **IMPLEMENTED / VALIDATION DEFERRED**: the host-agnostic alternate recipe now consumes canonical evaluated skin/morph geometry, material/texture/alpha/normal/emission semantics, and shared binary32 robust endpoints without Reference-local decode or scene-tuned ray bias; the current candidate is not compiled/cooked or GPU-exercised and user-facing session state remains unavailable | carry forward current-source compile/cook, raw GPU analytic/statistical/fault and robustness-matrix evidence, independent precision review, session identity, deterministic accumulation, live UX/transitions, parity, packaging, and full oracle evidence; grant no executable acceptance credit until run |
