...
**Snapshot:** 2026-09-13 against committed revision `709e04385c3d98aa3492fb3f09dc9caabe34cb3a` plus the current uncommitted Reference Path Tracer Stage-3 working tree; no release-approved candidate report was found
...
| `FCR-REN-08` | Reference Path Tracer | **35** | `30/5/0/0` | Stage 3 **IMPLEMENTED / VALIDATION DEFERRED**: the host-agnostic alternate recipe has an ordinary-binary32 canonical-camera, Philox, Lambertian/emissive/environment-miss finite GPU source path over shared path-tracing owners; the latest shared-family refactor is not compiled/cooked, user-facing state remains unavailable, and no real raw GPU oracle or independent binary32 accumulation review ran, but deferred owner-operated checks do not stop continued implementation | carry forward current-source compile/cook, raw GPU analytic/fault evidence, independent precision review, full estimator/session identity, deterministic accumulation, live UX/transitions, parity, packaging, and full oracle evidence; grant no acceptance credit until executed |
