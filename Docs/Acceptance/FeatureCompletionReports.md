...
| Acceptance | Binary `AC-<feature>-NN` criteria covering promised behavior and every applicable polish dimension. |
| Failure | `FM-<feature>-NN` feature faults plus applicable common [`FM-REL-*`](FirstRelease.md#failure-mode-acceptance-matrix) rows. |
| Checks | `CHK-<feature>-NN` mappings that cover every criterion and failure through the [check/test design contract](../Engineering/Verification/ValidationAndEvidence.md#check-and-test-design-contract). |
...
1. **Result and traceability** — feature ID/name, iteration, `NS-*`, `PGE-*`, roadmap/gate/workload, mapped capability/capability-evidence plan IDs, owner, audience, candidate/hash, state, and exact pass/block reason.
...
| --- | --- |
| Target coverage | `NS-*`/`PGE-*`/`REL-*`/`MAP-*`/`FCR-*`/capability-plan ID; applicability; advance/preserve/block; evidence; reviewer. |
...
| `FCR-REN-06` | Direct lighting/shadows, sky, deferred composite, and material channels combine GBuffer and scene-light buffers into separated direct/indirect products. | BRDF/light units, all light kinds, shadow bias/leaks, energy/conservation expectations, alpha/transmission/subsurface scope, reference comparison, debug separability and cost. | `PGE-02`, `PGE-08`, `PGE-09`, `PGE-13` |
| `FCR-REN-07` | ReSTIR direct/indirect passes generate, temporally/spatially reuse, resolve, and optionally reconstruct reservoirs/signals using ray-tracing scene data and history invalidation. | Estimator/reservoir math, sample/PDF/weight validity, visibility, history identity/reset, motion/disocclusion, fireflies/bias/noise, temporal failure gallery, quality-time-memory frontier. | `PGE-02`, `PGE-05`, `PGE-08`, `PGE-09`, `PGE-10`, `PGE-13` |
| `FCR-REN-08` | Reference path-traced lighting accumulates a convergent comparison route using shared scene/material/ray data and a reference-specific history lifecycle. It is the first technical closure case, but its [research study](../Research/OfflinePathTracerCompletion.md) and [`PTD-00` discovery gate](Renderer/OfflinePathTracerDiscovery.md) currently prohibit calling it an unbiased oracle. | Freeze estimator/domain and dependency independence first; then prove deterministic sample identity, raw accumulation/export, convergence/unbiasedness limits, reset, NaN/Inf/firefly behavior, robust ray spawn, material/light scope, analytic and independent references, backend parity, controlled failure, and non-real-time operation. | `PGE-02`, `PGE-05`, `PGE-06`, `PGE-07`, `PGE-08`, `PGE-09`, `PGE-10`, `PGE-13`, `PGE-15` |
| `FCR-REN-09` | Exposure metering/adaptation -> optional reconstruction/upscaling -> debug selection -> tone mapping -> one output-encoding/presentation boundary. | Scene-linear/display-linear semantics, metering equivalence, adaptation/reset, HDR/SDR scope, no double encoding/clipping/banding, debug-view presentation, resize/DPI behavior and output reference. | `PGE-02`, `PGE-05`, `PGE-08`, `PGE-09`, `PGE-13` |
| `FCR-REN-10` | Linear upscaling or NVIDIA DLSS provider maps render extent to output extent; optional NVIDIA ray reconstruction consumes ReSTIR signal/guides before presentation. | Provider requested/active/fallback truth, version/signature/redistribution, input semantics/jitter/exposure/motion, unsupported hardware, missing DLL, temporal quality, latency/memory and linear baseline. | `PGE-01`, `PGE-05`, `PGE-06`, `PGE-09`, `PGE-13` |
...
| Lighting | Direct visibility, ReSTIR direct, ReSTIR indirect, reference direct/indirect, composite, sky, each supported light kind, history reset, and the absence or exclusion of a non-ray fallback. |
| Display and reconstruction | Manual exposure, each automatic-metering method, async scheduling, linear upscale, each supported DLSS SR mode, DLSS ray reconstruction, every tone mapper, every output encoding, resize, and supported SDR/HDR claim. |
