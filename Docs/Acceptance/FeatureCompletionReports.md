...
**Implementation routing:** [First Release Implementation Plan](../Architecture/CrossModule/FirstRelease/README.md) assigns every registry family to one staged owner plan; it does not own the criteria or verdicts defined here
...
All entries below are `Blocked` at this snapshot. Established families are source-present or partial; `FCR-REN-23` through `FCR-REN-26` are admitted first-release targets whose implementation was not found. The “how it works” column is a source-navigation or target-boundary summary, not proof that a path builds or behaves correctly.

Current completion snapshot: 49 family rows identified; 49 blocked; zero candidate-bound feature reports found under the required release-artifact path; zero release-map reports because the `ReleaseMapSet` is not frozen. The family count is a navigation measure, not a feature-completeness score, and will increase when the mandatory result axes below are assigned independent verdicts. Every current row has an explicit Architecture owner in the [stable identifier coverage ledger](../Architecture/CrossModule/FeatureDocumentation/StableIdentifierCoverage.md#feature-completion-report-families).

Architecture also contains explicit target/negative dossiers for capabilities such as owned neural graphics, native Linux support, formal packaging/CI, and broader Python tooling. They are tracked by the [source-document coverage audit](../Architecture/CrossModule/FeatureDocumentation/SourceDocumentCoverage.md) and [Capability Evidence Plan](../Architecture/Modules/CapabilityEvidencePlan.md), but they do not enter this release registry until the Roadmap admits a reachable product/tool promise. Admission requires a new `FCR-*` family and complete feature-local `AC-*`/`FM-*`/`CHK-*` mapping in the same change; an architecture target alone cannot inflate readiness or imply implementation.
...
| `FCR-REN-06` | [Direct Lighting](../Architecture/Modules/Engine/Renderer/Features/Lighting/DirectLighting/README.md) samples four analytic light kinds through a source-present direct reservoir, resolves inline/native visibility, and writes direct diffuse/specular/subsurface lobes. | Light/BRDF units and energy, exhaustive baseline, initial PDFs, ReSTIR DI normalization/bias/light translation, temporal/disocclusion, visibility parity, reconstruction, many-light degradation, failure, and quality-time-memory. | `PGE-02`, `PGE-08`, `PGE-09`, `PGE-13` |
| `FCR-REN-07` | [Indirect Lighting](../Architecture/Modules/Engine/Renderer/Features/Lighting/IndirectLighting/README.md) currently replays seed identities through an unproved reservoir prototype, traces secondary paths, writes indirect diffuse/specular lobes, and consumes the sky/environment boundary. | Path/technique accounting, explicit path record, shift/inverse/Jacobian/support, GRIS contribution weights, history/motion/disocclusion, correlation/bias/noise, reconstruction, sky/emission semantics, and quality-time-memory. | `PGE-02`, `PGE-05`, `PGE-08`, `PGE-09`, `PGE-10`, `PGE-13` |
| `FCR-REN-08` | Reference Path Tracer accumulates a convergent comparison route using shared scene/material/ray data and a reference-specific history lifecycle. It is the first technical closure case, but its [research study](../Architecture/Modules/Engine/Renderer/Features/Lighting/ReferencePathTracer/Research.md), [`PTD-00` discovery gate](../Architecture/Modules/Engine/Renderer/Features/Lighting/ReferencePathTracer/Discovery.md), and [feature acceptance contract](../Architecture/Modules/Engine/Renderer/Features/Lighting/ReferencePathTracer/README.md) currently prohibit calling it an unbiased oracle. | Pass every included `RPT-FS-*` row and `AC-RPT-*` criterion; exercise every applicable `FM-RPT-*`; then prove deterministic sample identity, raw accumulation/export, convergence/unbiasedness limits, reset, NaN/Inf/firefly behavior, robust ray spawn, material/light scope, analytic and independent references, backend parity, controlled failure, packaged operation, and release-map adoption. | `PGE-02`, `PGE-05`, `PGE-06`, `PGE-07`, `PGE-08`, `PGE-09`, `PGE-10`, `PGE-13`, `PGE-15` |
...
| `FCR-REN-15` | [Presentation and Output](../Architecture/Modules/Engine/Renderer/Features/PostProcessing/DisplayPipeline/PresentationAndOutput.md) encodes current SDR display-linear color and publishes it to the back buffer or a generation-qualified viewport product after optional debug replacement and tone mapping. | Every SDR encoding/format pair, no double encoding/clipping/banding, exact-debug limitation, output/product identity, resize/DPI, capture interpretation, backend present, UI boundary, and clean separation from `FCR-REN-26` HDR ownership. | `PGE-01`, `PGE-02`, `PGE-05`, `PGE-06`, `PGE-08`, `PGE-09`, `PGE-13` |
...
| `FCR-REN-23` | [Deferred Decals](../Architecture/Modules/Engine/Renderer/Features/DeferredDecals/README.md) defines the admitted authored/cooked/runtime decal representation and raster plus secondary-ray GBuffer composition; no implementation was found. | Identity/lifetime, bounds/inside-outside rasterization, deterministic overlap/layers, channel/normal blending, consumer ordering, raster/ray agreement, overflow/failure, backend, package, quality, and cost. | `PGE-02`, `PGE-05`, `PGE-06`, `PGE-08`, `PGE-09`, `PGE-10`, `PGE-13` |
| `FCR-REN-24` | [Color Grading](../Architecture/Modules/Engine/Renderer/Features/PostProcessing/DisplayPipeline/ColorGrading/README.md) defines an admitted scene-referred control and single-LUT stage before tone mapping; no implementation was found. | Neutral and analytic controls, LUT asset/domain/interpolation, view/reset, stage/color-domain truth, debug/capture, failure, backend, package, quality, and cost. | `PGE-01`, `PGE-02`, `PGE-05`, `PGE-08`, `PGE-09`, `PGE-13` |
| `FCR-REN-25` | [Chromatic Aberration](../Architecture/Modules/Engine/Renderer/Features/PostProcessing/DisplayPipeline/ChromaticAberration/README.md) defines an admitted bounded output-space radial channel-separation pass; no implementation was found. | Zero identity, normalized controls, resolution scaling, edge/alpha behavior, stage/debug/UI order, view isolation, failure, backend, package, quality, and cost. | `PGE-01`, `PGE-02`, `PGE-05`, `PGE-08`, `PGE-09`, `PGE-13` |
| `FCR-REN-26` | [HDR Display Output](../Architecture/Modules/Engine/Renderer/Features/PostProcessing/DisplayPipeline/HDRDisplayOutput/README.md) defines an admitted Windows HDR output transform and RHI presentation route with mandatory SDR fallback; no implementation was found and the exact surface/metadata policy remains discovery-blocked. | PQ/Rec.2020/peak/UI policy, complete format/color-space surface contract, metadata disposition, requested/active/fallback truth, transitions/recovery, paired backends, HDR hardware, capture, package, quality, and cost. | `PGE-01`, `PGE-02`, `PGE-05`, `PGE-06`, `PGE-08`, `PGE-09`, `PGE-13`, `PGE-15` |
...
| Indirect lighting | Current seed-replay temporal/spatial/resolve, target ReSTIR GI/GRIS path conformance, unavailable reference-transport dependency, bounce/path domain, sky/environment/emission accounting, history reset, bias/correlation/convergence, reconstruction, and absence of probe/lightmap fallback. |
| Volumetric lighting | [Target package](../Architecture/Modules/Engine/Renderer/Features/Lighting/VolumetricLighting/README.md) plus explicit Not Implemented/Excluded state for media, fog, Volumetric ReSTIR, atmosphere/sky, aerial perspective, products, selectors, and debug views; no candidate report exists until `REL-11` and roadmap admission assign one. |
...
| Deferred decals | Authored/cooked/runtime data; identity/lifetime; raster inside/outside and overlap; each channel/normal blend; downstream ordering; secondary-ray behavior; overflow/failure; D3D12/Vulkan and package cells. |
| Color grading and chromatic aberration | Grading controls versus LUT, neutral/analytic/look fixtures, scene/display placement, view/reset/asset failures; chromatic zero/strength/center/start/resolution/edge/alpha/debug/UI behavior; each backend and package cell. |
...
| Presentation and output | Every SDR encoding/format and HDR10 format/color-space/metadata state; back-buffer and viewport-product routes; resize/DPI/monitor/OS transitions; UI paper white; capture interpretation; exact-debug limitation; requested/active/fallback truth. |
