...
| Animated Knight | Opt-in add-on download | Layered on the base-plus-curtains composition. The published FBX is usable for static meshes only; animation remains blocked until the Alembic source passes the staged [Geometry Cache Animation Pipeline](../Architecture/GeometryCacheAnimationImplementationPlan.md). |
...
- conformance to the canonical [source-to-engine coordinate, unit, handedness, and transform policy](../Architecture/WorldCoordinateAndUnits.md);
