...
This workload is the shared proof surface for [the canonical requirements](../Strategy/Requirements.md), [the gap assessment](../Strategy/Assessments/GapAssessment.md), [the execution roadmap](../Strategy/Roadmap.md), [the executive direction](../Strategy/ExecutiveSummary.md), and [the engineering persona](../Strategy/EngineerPersona.md).
...
| Animated Knight | Opt-in add-on download | Layered on the base-plus-curtains composition. The published FBX is usable for static meshes only; animation remains blocked until the Alembic source passes the staged [Geometry Cache Animation Pipeline](../Architecture/CrossModule/GeometryCacheAnimation.md). |
...
| Animated Knight | Intel archive byte count matched `1202508298`; SHA-256 `9112d9789ab2da50c77529907833bd008e5fa602f89438a5c1e82d7d4bcde2a5`. Helper curves/points were excluded after Assimp primitive separation; renderable triangles remained strict. A 2026-08-17 re-inspection found 29 meshes and one 80-channel controller animation, but zero bones or skin weights; the cook therefore emits no skeleton or animation products. The archive also contains the baked `knight_ANIM_001.rnd.abc` source. | The metre-normalized static FBX remained responsive; this proves scale and loadability, not animation. | Not accepted as an animated workload. Closure is the native [Geometry Cache Animation Pipeline](../Architecture/CrossModule/GeometryCacheAnimation.md): tools-only Alembic import, bounded native streaming, visible raster deformation and motion, then shared dynamic-BLAS reflection/GI parity. A static pose, controller-name remap, USD runtime, or knight-only transform is not an accepted substitute. |
...
| `MAP-11` | Modern Sponza Animated Knight | Curtains baseline plus native geometry-cache cook/streaming, visible playback and motion stability, then shared raster/ray deformation and reflection/GI parity. | Waiting |
...
- conformance to the canonical [source-to-engine coordinate, unit, handedness, and transform policy](../Architecture/Decisions/WorldCoordinateAndUnits.md);
...
The minimum run/sample count does not make a result automatically definitive. Analysis follows the [comparison and regression contract](CrossModule/PerformanceDiagnostics.md#comparison-and-regression-contract): per-run distributions are primary, a combined view is secondary, exclusions and tail `FrameId` values remain visible, worst-to-worst comparison requires equal `N` or an explicit model, and the predeclared decision combines absolute/relative practical-effect bands with a correlation-aware uncertainty method. An interval overlapping the practical-effect band is `Inconclusive`; a p-value alone never closes the gate.
