...
- The serial editor composes its UI inside one scoped renderer submission call; Prompt 14 replaces that same-thread callback with owned UI packets.
...
Main/editor thread
  ...
  renderer.RenderSerialUiFrame
    prepare + record renderer work
...
Prompt 07 made that reasoning concrete in the live scene. Prompt 08 then established `GameWorld` as the sole public world owner and private `GameWorldState` as the ECS/storage owner. `MeshInstance` contains resource/material/source handles and classification, `LocalTransform` and `Visibility` occupy independently queried columns, and `Name`, `AuthoredIdentity`, and `EditorMetadata` stay cold. A generation-validated mesh-resource handle resolves only inside the world boundary; destroying an entity invalidates and releases the resource slot. Skeletal morph weights are authoritative in a generation-validated deformation-state store referenced by `MorphState`; mutable mesh geometry is a derived compatibility cache for today's renderer, not a second authored state. Animation clips remain resource definitions while each clip entity owns its `AnimationState`. Prompts 09-12 still must make construction transactional, schedule systems, replace editor mutation facades, and publish renderer-owned packets.
...
Do not confuse a migration boundary with a destination abstraction. After Prompt 07, `SceneAnimations` failed the abstraction test—it merely forwarded calls, had no outside consumer, and owned no policy—so it was removed immediately. `SceneCameras`, `SceneLighting`, `SceneMeshes`, and non-owning `SceneSky` still isolate named synchronous consumers from private ECS storage, but their existence is time-bounded. Prompt 08 supplied immutable world reads; Prompts 09-12 replace construction, controller/editor writes, and renderer snapshots. Once those consumers migrate, the forwarding facades and corresponding `GameWorld` friend declarations are deleted. Moving the same forwarding methods onto `GameWorld`, exposing `GameWorldState`, or publishing the registry would reduce the class count while making ownership worse; the target is fewer *authorities and access paths*, not superficially fewer C++ types.
...
- `GameWorld`: the sole authoritative runtime world lifetime, owner-thread commit boundary, future system graph host, and publisher. Prompt 08 established this name without a compatibility alias.
- private `GameWorldState`: ECS registry, component pools, world resources, derived-state evaluation, bounded change journal, and immutable read-generation construction. It replaced the competing `SceneWorld` name and is never an SDK exposed to Editor or Renderer.
...
Prompt 08 also makes the difference between **authoritative**, **derived**, and **published** data executable. `LocalTransform` is owner-written input. `WorldTransform` and `CameraDerivedState` are derived outputs written only by explicit serial systems at commit. `WorldReadView` is neither of those: it is an immutable consumer projection for one committed generation. Treating these three roles as separate prevents two common mistakes: a const getter secretly updating a cache while readers overlap, and a renderer/editor treating live ECS memory as a stable packet.

The change journal is not event sourcing. It is a bounded invalidation/publication aid for current engine consumers. A journal record says which stable entity/domain changed at a committed sequence; the current `GameWorldState` remains authoritative. Consumers apply retained records to their own derived model, but if they fall behind they discard that model, acquire the latest immutable full baseline, acknowledge its sequence, and continue. This recovery rule is essential because bounded memory and unlimited replay history are mutually exclusive. Holding a `WorldChangeBatch` or `WorldReadView` pins only that immutable storage generation; it never pins mutable ECS arrays.

The memory-model edge is equally specific. The owner constructs an entire private read generation, then release-stores its shared immutable storage pointer. A reader acquire-loads the pointer before reading ordinary vectors and values. The atomic pointer publishes ownership and visibility; it does not make `GameWorldState`, its vectors, or arbitrary objects thread-safe. Journal metadata has its own narrow mutex because publication and cursor reads may overlap, while the world itself deliberately has no frame-scale shared mutex.

Prompt 08 uses copy-and-patch publication as the simple serial reference: copy the previous immutable generation, erase/upsert only changed stable entities, and full-rebuild on initial publication, reset, or overflow. This is not claimed as the final large-world cost model. Prompts 11, 12, and 15 measure and replace consumer-specific projections/renderer streams where persistent tables and dirty ranges are better. The important invariant already exists: consumers cannot observe partial mutation or reclaim a generation they still hold.

Learning exercise:

- explain why a pure `Transform::GetWorldMatrix()` computation is thread-safe while a mutable cached const getter is not;
- trace one local-transform edit through dirty compaction, derived evaluation, typed journal records, release publication, editor acquisition, and acknowledgement;
- show why a lagging cursor must resynchronize instead of reading an overwritten ring slot;
- distinguish the journal sequence, read-view generation, generational `EntityId`, and future renderer object/asset generations;
- identify which data remains temporary compatibility (`GameWorldSnapshot`, raw mesh resolution, direct editor commands) and name the prompt that removes each path.
...
- GameFramework ECS is the authoritative mutable world-instance source. `RenderWorld` is a deliberately derived, sequenced, render-owned projection—not a second gameplay authority. No renderer change flows backward into ECS storage.
...
`Mesh` asset data, cooked manifests, material definitions, skeleton definitions, textures, and shader packages are not ECS components. They are immutable resources referenced by typed handles; regeneration republishes the newest immutable content rather than preserving content-version compatibility. This prevents entity movement or component compaction from moving heavyweight resources.
...
| [InputSystem.h](../../../Engine/Platform/Public/Input/InputSystem.h) and [InputSystem.cpp](../../../Engine/Platform/Private/Input/InputSystem.cpp) | Prompt 03 declared owner-thread dispatch, snapshots eligible callbacks under a narrow registry lock, and invokes after unlock; subscribe/unsubscribe during dispatch affects the next snapshot |
