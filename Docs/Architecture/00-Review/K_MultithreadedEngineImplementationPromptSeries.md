...
The renderer now consumes one owned `RenderInputFrame` composed of a sequenced structural `RenderWorldDelta` and per-frame `RenderFrameDynamicData`. `GameWorld` delegates extraction to `World/Extraction/RenderInputExtractor`; `FramePipeline` delegates admission to `SceneData/Input/RenderInputConsumer`; serial structural application is isolated in `RenderWorld`; `RenderSceneDataBuilder` is orchestration only, with mesh upload/batching and temporal mesh history isolated in `RenderMeshDrawBuilder`. `RendererSystemRoot` owns no `GameWorld`, `LevelManager`, ECS query, or level callback.

Rule 13 access inventory:

| data / transform | authority and ownership | layout, stable identity, deterministic transform | source precedent and measured falsifier |
|---|---|---|---|
| extracted ECS meshes -> structural operations | ECS/world resource stores are authoritative; `RenderInputExtractor` owns the one `EntityId -> RenderObjectId` table; packets are derived | ordered maps give deterministic entity/object order; typed create/update/destroy vectors carry scene generation and sequence; `RenderObjectId` is separate from `EntityId` and GUID domains | J `EPIC-OWN`, Donut scene/proxy observations, and AMD access-oriented guidance; headless replay rejects duplicate/stale/gap/new-generation-without-reset input |
| mesh/material/texture resources -> proxy tables | generation stores own resources; packet/proxy holds `shared_ptr<const Mesh>` only through `ImmutableRenderMeshHandle`; renderer owns copied resource tables | handle carries asset ID + version; independent content revisions publish material/texture tables only on scene reset, resource change, or sky-texture dependency change; proxy table is ordered by `RenderObjectId` | J Donut/Cauldron resource-lifetime precedent; validation clears producer handles and packet storage then reads retained mesh geometry |
| transforms/visibility/camera/lights -> dynamic frame | committed extraction/read generation is frame authority; packet owns all values | object rows carry `RenderObjectId`, matrix, inverse transpose, visibility; camera/light arrays are value records; traversal/batching use stable object order | J two-data-streams and MT-29 deterministic merge; DebugEditor/DebugGame executable links are current parity build gates; interactive visual parity is not inferred |
| animation output -> skinning/morph rows | generation-prepared animation output is authoritative until extraction; packet owns copied matrices/weights | skinning and morph rows use resolved target `RenderObjectId`, not shared skeleton ID, output index, or invalid placeholder; mesh builder owns previous matrices per render object | J Prompt 10 exclusive output/authored-instance isolation; actors sharing a skeleton no longer alias one joint offset |
| packet -> serial `RenderWorld` | packet sequence is input authority; renderer proxy/resource tables are persistent derived projection | validate operation set before mutation, then reset/destroy/create/update/resource publication in fixed order; stale/duplicate/gap are typed statuses | J serial-first boundary; headless validation replays one recording into two worlds and compares final scene/sequence/proxy cardinality |
| `RenderWorld` + dynamic -> draw data | renderer projection is authoritative; `RenderMeshDrawBuilder` alone owns mesh conversion/history | visible traversal, per-object joint offsets, material resolution, stable group conversion, deterministic batch builder | AMD/NVIDIA renderer data/batching guidance scoped in J; DebugEditor and DebugGame links falsify dependency breakage; GPU visual/performance evidence remains interactive |
...
Rule 12 placement record: public packet contracts are responsibility-split in `GameFramework/Public/Rendering`; extraction/edit application are private GameFramework capabilities. Renderer admission is under `SceneData/Input`, projection under `SceneData`, orchestration/mesh construction under `SceneData/Builders`, and the headless gate under `Renderer/Validation`. Game-system descriptors live under `World/Systems/Descriptors`; simulation, animation, transform, extraction, and deterministic change-commit execution live under `World/Systems/Execution`. The cooked animation implementation was renamed from `CookedSceneAnimationBuilder` to operation-accurate `CookedAnimationAssetBuilder`; `CookedSceneAnimationRef` became `CookedAnimationReference`. No `SceneAnimation*` implementation spelling remains.

Deletion ledger: `GameWorldSnapshot`, `MeshSnapshot`, camera/lighting/material/texture/sky snapshot families, `RenderSceneSnapshot`, `SceneRenderStateCoordinator`, `SceneLightingSnapshotBuilder`, and `RenderMeshSnapshotAdapter` were deleted. Direct renderer level subscriptions/cache clearing and all `RendererSystemRoot` world/level accessors were deleted. Unconditional per-frame resource-table copies, skeleton-keyed skinning offsets, invalid-object morph rows, and the empty editor preview adapter were removed during this audit.

Validation evidence: `cmake --build build --config DebugEditor --target RenderWorldContractValidationRun -j 4` builds and runs a headless create/update/destroy recording without `GameWorld` or RHI lifetime; it proves deterministic replay, immutable asset retention after producer/packet release, and duplicate/stale/gap/reset rejection. `ShowcaseEditor` DebugEditor, `ShowcaseRuntime` DebugGame, and `SceneCooker` DebugEditor build. Searches find no old snapshot/facade/controller family, renderer `GameWorld`/ECS access, second scheduler/pool, or `SceneAnimation*` implementation. Exact packet-byte/extraction-time and interactive raster/RT/provider/capture parity remain measured Prompt 12D/product gates and are explicitly not claimed by compile/headless evidence.
