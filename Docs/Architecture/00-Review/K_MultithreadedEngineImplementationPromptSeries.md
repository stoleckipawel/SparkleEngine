...
| `PGE-11` | preserve | AI-assisted edits were independently traced through allocation, publication, recording, submission, retirement, resize, capture, and teardown; the pending-resource republication defect found during review was corrected |
...
- `LC-14`: workers receive copied `VulkanRecordingResource` values and opaque use tokens, never allocation or pending-release records. Pending resources are excluded from the next publication, closing the retention cycle found during source review.
