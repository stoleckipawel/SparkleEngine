...
- the intermediate feature history headers and all three `Persistent*History` layers were deleted rather than retained as wrappers;
...
- `FramePipeline` performs no graph-layout assignment, activation/configuration, physical binding, frame-index selection, or end-of-frame history commit;
...
These paths should not be turned into texture histories. Future Renderer-owned previous/current GPU textures should be declared with `FrameGraphBuilder::CreateTextureHistory`; pass/resource frontend files consume the named record without adding allocation, frame-slot, configuration, binding, or commit code.
...
Priority 2A is complete for uploads, frame-graph transfer, and selected compute work. `FramePipeline` selects copy when the backend truthfully exposes it, `TextureManager` remains unaware of submission policy, and `RhiUploadService` records the native copy. The copy submission returns `RhiSubmissionToken { Queue, Value }`; graphics installs a GPU wait and performs the final `Common -> ShaderResource` transition because a D3D12 copy queue cannot legally perform that transition. No ordinary upload or cross-queue dependency performs a CPU wait.

RHI owns three D3D12 command queues with independent monotonic fences. Vulkan selects queue locations by both family and queue index: it prefers specialized families, then unused compatible indices, and finally a legal logical-role alias. Exact native queue aliases share one submission mutex, logical roles retain their own timeline tokens, and asynchronous capability is advertised only when the role is physically distinct from graphics. Binary semaphores remain at the acquire/present edge. Vulkan resources and swapchain images use concurrent sharing across selected families. Both backends use dynamically growing per-frame, per-queue command-list pools and wait the last real queue submission before recycling a frame slot.
...
Frame-graph pass kind and queue preference are separate value types. Passes explicitly request `AsyncCompute` or `Copy`; the compiler resolves that preference against truthful queue capabilities, compiles queue-local batches and cross-queue token dependencies, and emits conservative producer-release/consumer-acquire transitions through `Common`. Scene-depth linearization, sky motion vectors, the exposure moment chain, and exposure history update opt into compute; copy helpers prefer copy. All other compute remains graphics work until its workload is reviewed, so exposing a physical queue is never treated as permission for automatic migration.
