...
| --- | --- | --- | --- | --- |
| `EASSET-001` | Shader entry sources | 29 `.hlsl` files implement registered compute, raster, ray-tracing, lighting, post, presentation, and debug entry points. Exact registrations are in the [Shader Program Catalog](../Renderer/ShaderProgramCatalog.md). | An `.hlsl` file without a typed registration is not a runtime shader capability. | `S` |
