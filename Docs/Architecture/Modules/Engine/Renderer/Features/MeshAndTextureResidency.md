...
Verified: 2026-09-06 against committed `master` revision `baa0adc9`; current mesh, texture, residency, diagnostic, and submission owners inspected
...
- `AC-RES-07` — public mesh, texture, and memory diagnostics report attributable generation/state/count/byte information consistent with cache and RHI observations.
...
| `CHK-RES-04` | paired-backend stream/unload/reset/shutdown run with delayed graphics/compute/copy completion and native validation; reconcile cache/RHI memory and retained high-water | `AC-RES-06`–`AC-RES-08`; `FM-RES-04` |
