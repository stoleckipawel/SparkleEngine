...
Verified: 2026-09-06 against committed `master` revision `baa0adc9`; public state/section, persistence, CVar runtime, editor callback, application startup, and Renderer control routes inspected

Scope: `REN-SET-01` through `REN-SET-05`; owns the lifecycle of the aggregate rendering-settings state, editor commit, workspace persistence, startup restore, render-thread handoff, live versus restart-required application, and requested-state limitations. [Feature Selector Catalog](FeatureSelectors.md) remains the exact per-selector ledger.
...
- [Feature Selector Catalog](FeatureSelectors.md) owns exact names, defaults, domains, persistence membership, consumers, and absent/ineffective controls.
