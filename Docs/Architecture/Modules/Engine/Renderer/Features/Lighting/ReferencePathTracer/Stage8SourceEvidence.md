...
| Reference policy | `ReferencePathTracer` retains the canonical ray-tracing scene reference it already requires for pass construction and consumes only its resolved frontend for session identity and composition. Sample identity, estimator consequence, accumulation, progress, and reset policy remain inside the feature capsule. Traversal selection is not a view-mode or Reference-specific setting. |
