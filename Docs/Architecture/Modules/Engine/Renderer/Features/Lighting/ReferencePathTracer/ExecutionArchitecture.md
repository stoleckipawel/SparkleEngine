...
**Current readiness:** **50/100 (`45/5/0/0`)**. The original-frame alternate recipe now composes the included estimator and one feature-owned GPU session over shared camera, scene-identity, evaluated triangle, material/texture, path, light, visibility, MIS, roulette, robust endpoint, and accumulation-arithmetic owners. Stage 7 source exposes the existing mode and its operational projection. Stage 8 source adds strict requested/active Inline and Pipeline execution over one Reference kernel, shared material trace/payload/hit mechanics, and the existing D3D12/Vulkan RHI mechanisms. The current tree is not compiled, cooked, or GPU exercised; numeric, lifecycle, navigation, response-budget, accessibility, four-route backend/frontend, and raw-oracle evidence remains deferred with zero executable-readiness credit. Oracle authority remains undelivered. See [Current Feature Readiness](../../../../../../../Acceptance/CurrentReadiness.md#renderer).
...
| --- | --- | --- |
| Invocation | The current working source carries one `RenderViewMode` per view. Editor exposes `ReferencePathTracer = 1` immediately after Lit; Game/runtime can submit the same value through `RuntimeApplication::SubmitViewportRenderRequest`; `RenderViewBuilder` freezes it into View; and `BuildRenderFrameGraph` owns the Lit-versus-Reference execution branch. | Retain this one request route and prove both hosts, navigation, progress, comparison, capability, and package behavior through executable evidence. No `LightingMode`, Editor mirror enum, target/show-flag split, selector CVar, or dedicated command may return. |
| Traversal frontend | The Reference middle resolves `r.PathTracing.Execution` to strict requested/active Inline or native Pipeline execution. Two thin entry adapters invoke one shared estimator kernel; Pipeline reuses generic material payload/hit shaders, scene SBT, and RHI pipeline/table/dispatch mechanisms. | Prove D3D12 Inline, D3D12 Pipeline, Vulkan Inline, and Vulkan Pipeline parity and strict unsupported-route refusal. Source presence is not parity evidence. |
...
| generic viewport product/progress channel | the operational state, exact work prefix, invalidation/blocking reason, retention/capacity, selected route/backend, throughput, and ETA consumed by the current viewport overlay | transport digest, estimator configuration, resource identity, user-facing strings, logs, debug counters, or control ownership | consumer audit proves every field drives current product behavior or it is deleted with the feature |
...
| `ViewportRenderProgress` | The small renderer-agnostic product boundary used by the real Stage-7 overlay: operational state, exact committed/target/discarded work, reset or blocking reason, retention/capacity, requested/active route, backend, throughput, and ETA. It carries no view-mode selector, estimator settings, digest, resource handles, strings, logs, event stream, or debug counters. |
