...
The target is one bounded, restartable **per-view reference session** exposed by Editor as `EditorViewportViewMode::ReferencePathTracer` immediately after Lit and executed by Renderer through `r.ReferencePathTracer`. The UI view mode is not Renderer, View, graph, shader, or RHI state. The feature is a specialized setup inside the existing `FramePipeline`, frame graph, Scene, View, and RHI architecture—not a separate renderer. It traces camera paths over immutable Scene- and View-owned generations, accumulates automatically while the effective view is unchanged, invalidates before mixing changed inputs, and can publish raw scene-linear evidence atomically. A secondary host uses the same Renderer switch and session semantics. It is not a quality preset layered onto the retired GBuffer-seeded branch, and it does not duplicate frame ownership.
...
| --- | --- | --- |
| Invocation | Editor owns the ordered `EditorViewportViewMode`; `ReferencePathTracer = 1` remains immediately after Lit. Selecting it sets Renderer-owned `r.ReferencePathTracer=1` and `r.Visualization=Lit`, while Game/runtime may set the same controls without Editor. `FramePipeline::BuildRenderFrameGraph` reads the feature CVar at the sole Lit-versus-Reference branch. The former global `LightingMode` branch and Renderer-owned view-mode selector are deleted. | Selection automatically validates and starts one bounded per-view session once Stage 7 exposes the completed transport route; export and offscreen automation consume the same Renderer switch and session contract. |
...
Sparkle's live frame route is the template: `FramePipeline::OnRender` admits and prepares the frame, and `FramePipeline::BuildRenderFrameGraph` creates shared resources before reading `CVarReferencePathTracer` at one explicit middle-frame branch. The Lit branch schedules GBuffer, lighting, exposure, reconstruction, and upscaling; the Reference branch calls the feature owner, which schedules transport, accumulation, exposure, and its display derivative. Both return the same resolved scene-color product to the shared post-processing/presentation tail. No recipe hierarchy, graph factory, dependency aggregate, UI enum, or repeated selector mediates this branch. The feature is not dispatched by a second renderer, a side job, or an unrelated post-process/debug path.

`RenderViewKind` identifies existing producer semantics—Editor-authored `Scene` or runtime `Game`—and remains independent of the renderer-wide Reference switch. Any supported kind rendered while `r.ReferencePathTracer` is enabled reaches the same branch, feature owner, estimator, session, and downstream product contract. Editor's ordered view-mode state is translated into that CVar plus `r.Visualization` at the frontend boundary; it is never copied into `RenderView`. No feature code branches on “Editor versus Game.”
...
flowchart LR
    ...
    Scene --> Select{r.ReferencePathTracer in BuildRenderFrameGraph}
...
The built value of `r.ReferencePathTracer` participates in graph-topology identity. A change rebuilds the matching graph at the existing safe topology boundary, while the Private Reference Path Tracer owner persists long enough to implement the accepted Lit suspend/revalidate policy. The CVar is read by topology/composition ownership and the built route is passed to the feature lifecycle; it is not copied through View or graph settings. This is one renderer with two mutually exclusive middle setups, not two renderers and not one monolithic graph that runs both estimators.

Stage 1 intentionally leaves this alternate setup unavailable because no correct estimator exists yet. Its deletion of the former shaders/resources removed a GBuffer-seeded approximation that violated the accepted reference integral; it did not reject shaders, resources, or frame integration as concepts. Stage 2 establishes the single CVar-driven branch and corrects the Editor View kind. Stage 3 consumes the canonical View camera and adds sample identity together with the first real transport pass; Stages 4 through 6 then add transport breadth, resources, digest/invalidation, ranges, and accumulation through that feature entry before Stage 7 exposes the Editor menu row. No Reference-specific camera, standalone camera utility layer, unused sampler, host-side session machinery, recipe abstraction, or per-feature graph dependency is prepaid.
...
- an Editor **view mode** is frontend state translated to `r.ReferencePathTracer` and `r.Visualization`;
...
- each approved **host** may present its own view-mode affordance, but translates that frontend state into the same Renderer CVar and renders a generic progress observation; Editor and Game adapters do not define separate transport behavior.
...
| --- | --- | --- | --- |
| Editor `EditorViewportViewMode` declaration | contiguous `ReferencePathTracer = 1` presentation semantic only | Renderer session, samples, backend, product, reset, estimator, or resource policy | removing the enum/UI row and its CVar assignment leaves no Editor selector reference |
| canonical viewport request and `RenderView` | view-intrinsic camera, identity, extent, selection, display, and requested-product data only | Editor view mode, Renderer feature/visualization selectors, digest, progress taxonomy, session, frontend/backend, or accumulator fields | no `ReferencePathTracer`, `ViewMode`, or visualization-selector member/type exists; only the shader uniform carries the consumed visualization scalar |
| frame topology and composition | `FramePipeline::BuildRenderFrameGraph` reads `CVarReferencePathTracer` once and directly invokes either Lit stages or the feature owner; FramePipeline invokes the feature's narrow lifecycle operations and publishes generic progress/output | recipe/base interfaces, graph factories, dependency bags, View/UI/RHI selector fields, feature mechanism in `FramePipeline`, or repeated selector switches | deleting the CVar branch, feature lifetime, and lifecycle calls leaves the ordinary Lit frame coherent |
...
`FramePipeline` directly owning `ReferencePathTracer` is permitted because it is the persistent render lifecycle and composition owner and the header uses one Private incomplete-type member. Its feature contact is limited to construction, the single CVar-driven graph branch, per-frame update/resource binding, generic progress publication, and post-execution submission notification. The feature state machine and every transition remain inside the capsule. Graph settings/resources, Scene, View, RHI, capture, visualization, and provider owners contain no Reference selector or state.
...
```text
Editor view mode or Game/runtime control sets r.ReferencePathTracer
...
Names are illustrative until implementation review, but responsibilities are fixed. Except for the Renderer-owned feature/visualization CVars, the minimal visualization scalar consumed by shaders, and the feature-neutral viewport progress payload, these are feature-local concepts and must not become generic Renderer, View, Scene, RHI, settings, or Editor vocabulary. The ordered Editor view-mode enum remains frontend-owned.

| Contract | Required content |
| --- | --- |
| `CVarReferencePathTracer` at Renderer topology/composition sites | The only transport selector. Editor selection remains in `EditorViewportSession`, which translates its UI enum to this CVar and `CVarVisualization`; a Game/runtime host may set the same Renderer controls. No UI enum, Reference selector, or visualization selector is copied through `ViewportRenderRequest`, `RenderView`, graph settings, capture, or RHI. |
...
1. **Editor viewport adapter:** `Reference Path Tracer` is the second top-level Editor view-mode item, immediately after Lit. Selection sets `r.ReferencePathTracer=1` and `r.Visualization=Lit`, preserves Lit settings, performs automatic preflight, starts accumulation on success, remains responsive while the camera moves, and displays the newest accepted composition with a compact progress/reset/completion overlay. Details expose exact settings and pause/restart without becoming a prerequisite; checkpoint/save appear later under a secondary Evidence/Output group.
2. **Game/runtime adapter:** a Game-kind RenderView uses the same `r.ReferencePathTracer` Renderer control through its approved development owner. Its UI may differ, but frame composition, estimator/session behavior, canonical camera identity, invalidation, progress, target completion, raw/presentation separation, and failure states cannot.
...
1. add Editor-owned `EditorViewportViewMode::ReferencePathTracer` immediately after Lit, add the Renderer-owned `r.ReferencePathTracer` composition switch, and remove `LightingMode::ReferencePathTracer` plus any Renderer/RHI view-mode selector as competing authority;
