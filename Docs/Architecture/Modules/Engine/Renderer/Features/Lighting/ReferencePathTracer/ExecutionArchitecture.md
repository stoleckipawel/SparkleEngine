...
| --- | --- | --- |
| Invocation | The current working source carries concrete `Visualization` per view: Editor resolves its ordered mode locally into `ViewportRenderRequest::ActiveVisualization`, Application/Renderer submit only that request, and `RenderViewBuilder` freezes it into View. The Reference row maps to Lit and remains unavailable; private `r.ReferencePathTracer` is now the only transitional global selector. | Editor owns the ordered `EditorViewportViewMode`; `ReferencePathTracer = 1` remains immediately after Lit. Its preset submits `Visualization::Lit` plus `RenderShowFlag::ReferencePathTracer` through `ViewportRenderRequest`. Game/runtime may submit the same concrete values without Editor. `BuildRenderFrameGraph` reads that per-view flag at the sole Lit-versus-Reference branch. No `LightingMode`, Renderer view-mode enum, selector CVar, or dedicated command remains. |
...
| View | Viewport identity, kind, camera model, projection, transform, resolution/crop/filter, concrete Renderer visualization/show flags, and other intrinsically per-view render facts. | Frontend view-mode identity, Scene data, estimator internals, process-global Reference Path Tracer policy, or RHI mechanism. |
...
- an Editor **view mode** is frontend preset state resolved to concrete `Visualization` and `RenderShowFlagSet` values on the ordinary viewport request;
...
| canonical viewport request and `RenderView` | view-intrinsic camera, identity, extent, selection, display, requested products, concrete `Visualization`, and the resolved `RenderShowFlagSet` | Editor view mode, override deltas, digest, progress taxonomy, session, frontend/backend, accumulator fields, or RHI copies | removing the Reference flag from presets and composition leaves generic per-view controls coherent |
...
1. add Editor-owned `EditorViewportViewMode::ReferencePathTracer` immediately after Lit, map it to Renderer-owned `RenderShowFlag::ReferencePathTracer` plus `Visualization::Lit`, and remove `LightingMode::ReferencePathTracer`, `r.ReferencePathTracer`, and any Renderer/RHI view-mode selector as competing authority;
