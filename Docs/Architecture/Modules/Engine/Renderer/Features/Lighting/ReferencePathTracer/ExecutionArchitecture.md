...
**Current readiness:** **50/100 (`45/5/0/0`)**. The original-frame alternate recipe now composes the included estimator and one feature-owned GPU session over shared camera, scene-identity, evaluated triangle, material/texture, path, light, visibility, MIS, roulette, robust endpoint, and accumulation-arithmetic owners. Stage 7 source exposes the existing mode immediately after Lit and projects exact prefix, reset/discard, pause/resume/restart, retention/capacity, route/backend, throughput, ETA, and completion through the ordinary viewport request/product boundary and one feature-local Editor overlay. The current tree is not compiled, cooked, or GPU exercised; numeric, lifecycle, navigation, response-budget, accessibility, backend, and raw-oracle evidence remains deferred with zero executable-readiness credit. Oracle authority remains undelivered. See [Current Feature Readiness](../../../../../../../Acceptance/CurrentReadiness.md#renderer).
