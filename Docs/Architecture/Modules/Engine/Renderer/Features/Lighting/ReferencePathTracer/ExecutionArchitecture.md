...
**Current readiness:** **50/100 (`45/5/0/0`)**. The original-frame alternate recipe now composes the included estimator and one feature-owned GPU session over shared camera, scene-identity, evaluated triangle, material/texture, path, light, visibility, MIS, roulette, robust endpoint, and accumulation-arithmetic owners. Stage 6 source includes component-classified invalidation, exact ordered prefix commit, row-quantized work, persistent working/committed mean/M2 resources, Lit suspension/retention, and a one-way committed display derivative. The current tree is not compiled, cooked, or GPU exercised; numeric, lifecycle, response-budget, backend, and raw-oracle evidence remains deferred with zero executable-readiness credit. Clickable UI and oracle authority remain undelivered. See [Current Feature Readiness](../../../../../../../Acceptance/CurrentReadiness.md#renderer).
...
The target is one bounded, restartable **per-view reference session** exposed by Editor as `EditorViewportViewMode::ReferencePathTracer` immediately after Lit and executed when the concrete per-view `RenderShowFlag::ReferencePathTracer` is enabled. The UI view mode is not Renderer, graph, shader, or RHI state; only its resolved optional visualization target and rendering show flags enter the ordinary view request and immutable `RenderView`. Reference keeps the target at `None`, so no debug-product selection duplicates its composition flag. The feature is a specialized setup inside the existing `FramePipeline`, frame graph, Scene, View, and RHI architecture—not a separate renderer. It traces camera paths over immutable Scene- and View-owned generations, accumulates automatically while the effective view is unchanged, invalidates before mixing changed inputs, and can publish raw scene-linear evidence atomically. A secondary host submits the same Renderer semantics and receives the same session behavior. It is not a quality preset layered onto the retired GBuffer-seeded branch, and it does not duplicate frame ownership.

> [!IMPORTANT]
> **Current state:** `PTD-01-R2 PASS` established selector order, one stateless Private owner, a proved generic progress reader, and the obsolete-route/generated-product clean break under `CHK-RPT-17`. Stages 3-6 later established source-present GPU transport and accumulation. Stages 6A-6C now place one optional visualization target and the non-overlapping Wireframe/Reference rendering flags on the ordinary request/View route and delete both global selector seams; none of this is accepted-reference output.
>
...
| --- | --- | --- |
| Invocation | The current working source carries optional `VisualizationTarget` and `RenderShowFlagSet` per view. Editor resolves its ordered mode locally; the Reference preset produces target `None` plus `RenderShowFlag::ReferencePathTracer`; Application/Renderer submit only the ordinary request; `RenderViewBuilder` freezes both values into View; and `BuildRenderFrameGraph` owns the sole Lit-versus-Reference execution branch. The Reference menu item remains intentionally hidden until Stage 7. | Editor exposes `ReferencePathTracer = 1` immediately after Lit, and an approved Game/runtime host may submit the same concrete values without importing Editor. Both use the existing request, feature session, frame shell, and generic progress product; no `LightingMode`, Renderer view-mode enum, selector CVar, or dedicated command exists. |
...
| View | Viewport identity, kind, camera model, projection, transform, resolution/crop/filter, optional Renderer visualization target, rendering show flags, and other intrinsically per-view render facts. | Frontend view-mode identity, Scene data, estimator internals, process-global Reference Path Tracer policy, or RHI mechanism. |
...
Sparkle's live frame route is the template: `FramePipeline::OnRender` admits and prepares the frame, and `FramePipeline::BuildRenderFrameGraph` creates shared resources before reading the accepted request's `RenderShowFlag::ReferencePathTracer` value at one explicit middle-frame branch. The Lit branch schedules GBuffer, lighting, exposure, reconstruction, and upscaling; the Reference branch calls the feature owner, which schedules transport, accumulation, exposure, and its display derivative. Both return the same resolved scene-color product to the shared post-processing/presentation tail. No recipe hierarchy, graph factory, dependency aggregate, UI enum, selector CVar, or repeated switch mediates this branch. The feature is not dispatched by a second renderer, a side job, or an unrelated post-process/debug path.

`RenderViewKind` identifies existing producer semantics—Editor-authored `Scene` or runtime `Game`—and remains independent of the Reference flag. Any supported kind whose request enables `RenderShowFlag::ReferencePathTracer` reaches the same branch, feature owner, estimator, session, and downstream product contract. Editor resolves its ordered view-mode preset to concrete Renderer values; the UI value itself is never copied into `RenderView`. No feature code branches on “Editor versus Game.”
...
`FramePipeline` owns the feature lifetime because it already sequences per-frame update, graph construction, resource binding, execution, progress publication, and submission. Its `BuildRenderFrameGraph` member is the only composition point allowed to name Lit and Reference and reads the CVar directly there. It must not own Reference Path Tracer validation, accumulation counters, reset classification, transport digest, sample state, shader binding, resource allocation, or error policy. No graph dependency record, recipe/base interface, factory, Lighting subpass, post-process stage, presentation path, View, or RHI frontend independently chooses the renderer again.
...
flowchart LR
    ...
    Scene --> Select{request ShowFlags.ReferencePathTracer in BuildRenderFrameGraph}
...
The submitted `ReferencePathTracer` topology bit selects the required provider/render extent before View construction; the resulting graph settings participate in the existing topology comparison and rebuild the matching graph at its safe boundary. `BuildRenderFrameGraph` owns the sole Lit-versus-Reference execution branch. The accepted request value is also frozen into `RenderView`, where the Private Reference Path Tracer owner observes it for the Lit suspend/revalidate policy. No bit is copied through graph settings, no built selector cache remains, and no independent value is resolved. This is one renderer with two mutually exclusive middle setups, not two renderers and not one monolithic graph that runs both estimators.

Stage 1 intentionally left this alternate setup unavailable because no correct estimator existed yet. Its deletion of the former shaders/resources removed a GBuffer-seeded approximation that violated the accepted reference integral; it did not reject shaders, resources, or frame integration as concepts. Stage 2 established the first branch and corrected the Editor View kind; Stages 3 through 6 added transport and accumulation. DVP-0 freezes the route, DVP-1 separates debug targets from Wireframe behavior, and DVP-2 adds the Reference composition flag before Stage 7 expands the viewport UX. No Reference-specific camera, standalone camera utility layer, unused sampler, host-side session machinery, recipe abstraction, or per-feature graph dependency is prepaid.
...
- an Editor **view mode** is frontend preset state resolved to one optional `VisualizationTarget` plus non-overlapping `RenderShowFlagSet` values on the ordinary viewport request;
...
| --- | --- | --- | --- |
| Editor `EditorViewportViewMode` declaration | contiguous `ReferencePathTracer = 1` presentation semantic only | Renderer session, samples, backend, product, reset, estimator, resource policy, or direct CVar access | removing the enum/UI row and Application control adapter leaves no Editor selector reference |
| canonical viewport request and `RenderView` | view-intrinsic camera, identity, extent, selection, display, requested products, optional `VisualizationTarget`, and the resolved `RenderShowFlagSet` | Editor view mode, override deltas, digest, progress taxonomy, session, frontend/backend, accumulator fields, or RHI copies | removing the Reference flag from presets and composition leaves generic per-view controls coherent |
| frame topology and composition | `FramePipeline::BuildRenderFrameGraph` reads the accepted request's `ReferencePathTracer` bit once and directly invokes either Lit stages or the feature owner; FramePipeline invokes the feature's narrow lifecycle operations and publishes generic progress/output | recipe/base interfaces, graph factories, dependency bags, UI/RHI selector fields, feature mechanism in `FramePipeline`, selector CVars, or repeated flag switches | deleting the flag branch, feature lifetime, and lifecycle calls leaves the ordinary Lit frame coherent |
| existing shared Renderer shader capability owners | canonical View camera rays, trace result/query, material-hit loading, established path surface/direction-sample contracts, reusable `PathTracer` state/radiance/throughput/BSDF semantics, narrow `CommonRandom` counter-RNG mechanics, and accurately named hit-count constants required by current production consumers | Reference target, key/counter/dimension identity, session/resource/product/host policy; optimized ReSTIR/cache/denoiser policy; or a custom precision facade | delete a new shared mechanism if the feature was its only consumer; otherwise GBuffer/path/ReSTIR/shadow consumers retain one coherent non-feature owner |
...
`FramePipeline` directly owning `ReferencePathTracer` is permitted because it is the persistent render lifecycle and composition owner and the header uses one Private incomplete-type member. Its feature contact is limited to construction, the single show-flag-driven graph branch, per-frame update/resource binding, generic progress publication, and post-execution submission notification. The feature state machine and every transition remain inside the capsule. Graph settings/resources, Scene, RHI, capture, visualization, and provider owners contain no Reference selector or state; the generic request/View contain only the typed flag shared by all view producers.
...
```text
Editor view-mode preset or Game/runtime control enables RenderShowFlag::ReferencePathTracer
...
Names are illustrative until implementation review, but responsibilities are fixed. The optional per-view `VisualizationTarget` and `RenderShowFlagSet`, the focused target index consumed by shaders, and the feature-neutral viewport progress payload are generic Renderer contracts with multiple viewport consumers. Targets select debug products/channels; flags select rendering behavior or contributions, and no semantic may exist in both domains. All transport/session concepts remain feature-local and must not become generic Renderer, View, Scene, RHI, settings, or Editor vocabulary. The ordered Editor view-mode enum remains frontend-owned, and show flags never enter RHI.

| Contract | Required content |
| --- | --- |
| `RenderShowFlag::ReferencePathTracer` | The only transport selector. Editor selection remains in `EditorViewportSession`, whose preset leaves `VisualizationTarget` at `None` and submits this bit through the ordinary `ViewportRenderRequest`; a Game/runtime host may submit the same concrete values. `RenderView` freezes them for pass consumption. No UI enum, selector CVar, override delta, or show flag enters graph settings, capture, or RHI. |
...
1. **Editor viewport adapter:** `Reference Path Tracer` is the second top-level Editor view-mode item, immediately after Lit. Its preset keeps `VisualizationTarget::None`, enables `RenderShowFlag::ReferencePathTracer`, preserves Lit settings, performs automatic preflight, starts accumulation on success, remains responsive while the camera moves, and displays the newest accepted composition with a compact progress/reset/completion overlay. Details expose exact settings and pause/restart without becoming a prerequisite; checkpoint/save appear later under a secondary Evidence/Output group.
2. **Game/runtime adapter:** a Game-kind RenderView uses the same concrete `ReferencePathTracer` show flag through its approved viewport owner. Its UI may differ, but frame composition, estimator/session behavior, canonical camera identity, invalidation, progress, target completion, raw/presentation separation, and failure states cannot.
...
The tool is excluded from `ShippingEditor`, `ShippingGame`, and consumer first run by default. Shipping profiles compile out every Reference producer/session factory, the `ReferencePathTracer` show flag and its presets/controls, CLI route, UI row, writer, codec/package, and documentation route; no Reference value remains in the Renderer visualization-target domain or RHI contracts. If release scope later exposes it, dependencies, writable roots, support contract, selector reachability, and performance expectations must be admitted explicitly. Development UI and non-UI hosts submit the same concrete per-view flag; no composition-selector CVar remains.
...
1. add Editor-owned `EditorViewportViewMode::ReferencePathTracer` immediately after Lit, map it to Renderer-owned `RenderShowFlag::ReferencePathTracer` with `VisualizationTarget::None`, and remove `LightingMode::ReferencePathTracer`, `r.ReferencePathTracer`, and any Renderer/RHI view-mode selector as competing authority;
