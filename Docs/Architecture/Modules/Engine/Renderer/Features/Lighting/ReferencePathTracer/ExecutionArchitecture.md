...
The target is one bounded, restartable **per-view reference session** selected by `RenderViewMode::ReferencePathTracer`, ordered immediately after Lit. The same mode travels through the ordinary view request and immutable `RenderView`; Editor owns only its label, icon, menu placement, and interaction, while RHI remains unaware. The feature is a specialized setup inside the existing `FramePipeline`, frame graph, Scene, View, and RHI architecture—not a separate renderer. It traces camera paths over immutable Scene- and View-owned generations, accumulates automatically while the effective view is unchanged, invalidates before mixing changed inputs, and can publish raw scene-linear evidence atomically. A secondary host submits the same Renderer semantic and receives the same session behavior. It is not a quality preset layered onto the retired GBuffer-seeded branch, and it does not duplicate frame ownership.
...
| --- | --- | --- |
| Invocation | The current working source carries one `RenderViewMode` per view. Editor and Game/runtime submit the same value through the ordinary request; `RenderViewBuilder` freezes it into View; and `BuildRenderFrameGraph` owns the Lit-versus-Reference execution branch. The Reference menu item remains intentionally hidden until Stage 7. | Editor exposes `ReferencePathTracer = 1` immediately after Lit, and an approved Game/runtime host may submit the same mode without importing Editor code. Both use the existing request, feature session, frame shell, and generic progress product; no `LightingMode`, Editor mirror enum, target/show-flag split, selector CVar, or dedicated command exists. |
...
Sparkle's live frame route is the template: `FramePipeline::OnRender` admits and prepares the frame, and `FramePipeline::BuildRenderFrameGraph` creates shared resources before selecting `RenderViewMode::ReferencePathTracer` at the middle-frame branch. The Lit branch schedules GBuffer, lighting, exposure, reconstruction, and upscaling; the Reference branch calls the feature owner, which schedules transport, accumulation, exposure, and its display derivative. Both return the same resolved scene-color product to the shared post-processing/presentation tail. No recipe hierarchy, graph factory, dependency aggregate, Editor mirror enum, target/flag translation, or selector CVar mediates this branch. The feature is not dispatched by a second renderer, a side job, or an unrelated post-process/debug path.
...
flowchart LR
    ...
    Scene --> Select{request ViewMode == ReferencePathTracer in BuildRenderFrameGraph}
...
| canonical viewport request and `RenderView` | view-intrinsic camera, identity, extent, selection, display, requested products, and one `RenderViewMode` | UI presentation, digest, progress taxonomy, session, frontend/backend, accumulator fields, or RHI copies | removing the Reference enum row and composition consumer leaves generic per-view mode selection coherent |
...
`FramePipeline` directly owning `ReferencePathTracer` is permitted because it is the persistent render lifecycle and composition owner and the header uses one Private incomplete-type member. Its feature contact is limited to construction, the single mode-driven graph branch, per-frame update/resource binding, generic progress publication, and post-execution submission notification. The feature state machine and every transition remain inside the capsule. Graph settings/resources, Scene, RHI, capture, and provider owners contain no Reference selector or state; the generic request/View contain only the one mode shared by all view producers.
