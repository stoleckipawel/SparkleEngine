...
| Noninteractive Editor offscreen session | Required reproducible secondary route | Source-present through `ShowcaseEditor --reference-path-tracer-request`; executable equivalence and failure evidence are deferred. |
...
| Raw EXR/manifest/checkpoint | Required for final evidence; secondary product surface after the live viewport milestone | Source-present through generic raw readback and the private ApplicationEditor transaction; decoder round trip, corruption/failure injection, and publication evidence are deferred. |
