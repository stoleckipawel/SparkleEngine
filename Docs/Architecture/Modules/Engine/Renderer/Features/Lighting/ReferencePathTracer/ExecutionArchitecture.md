...
| Integrator semantic core | Canonical View camera/sample use, path state, material/light evaluation contracts, NEE/MIS, roulette, robust spawn use, and fail-closed invalid handling. | A second camera representation, API-specific SBT/root-signature/descriptor vocabulary, diagnostic event streams, or UI policy. |
...
The steady-state implementation home is `Engine/Renderer/Private/Passes/Lighting/ReferencePathTracer/`, with shaders in one matching feature directory under `Engine/Assets/Shaders/Passes/Lighting/ReferencePathTracer/`. The feature home owns its entry point, session/accumulation lifetime, input digest, estimator, resources, shader parameter records, traversal-facing semantic adapters, progress production, required error accounting, and cleanup. Private files may be split by real responsibility, but remain under this capsule. Feature-specific diagnostic passes, GPU readbacks, dashboards, debug panels, and parallel inspection APIs are excluded; implementation evidence uses real product outputs, existing generic capture/validation routes, or temporary probes removed before handoff.
...
| `Engine/Assets/Shaders/Passes/Lighting/ReferencePathTracer/` | Reference-specific sample identity, path-state, BSDF/light estimator, robust-ray, accumulation, and frontend shader code, introduced only with passes that consume it. Camera matrices and raster-to-view mapping remain canonical shared Renderer shader utilities. Do not fork semantic code by API or add a parallel diagnostic renderer. |
