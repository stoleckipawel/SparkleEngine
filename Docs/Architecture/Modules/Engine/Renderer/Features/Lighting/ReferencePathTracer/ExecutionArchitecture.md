...
| --- | --- | --- |
| Invocation | The current working source translates the Editor selection into a render command that applies `r.Visualization` and private `r.ReferencePathTracer`; this transitional global route is unbuilt and not the accepted architecture. | Editor owns the ordered `EditorViewportViewMode`; `ReferencePathTracer = 1` remains immediately after Lit. Its preset submits `Visualization::Lit` plus `RenderShowFlag::ReferencePathTracer` through `ViewportRenderRequest`. Game/runtime may submit the same concrete values without Editor. `BuildRenderFrameGraph` reads that per-view flag at the sole Lit-versus-Reference branch. No `LightingMode`, Renderer view-mode enum, selector CVar, or dedicated command remains. |
