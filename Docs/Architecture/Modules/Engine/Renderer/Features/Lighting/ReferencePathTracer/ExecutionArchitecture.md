...
**Current readiness:** **50/100 (`45/5/0/0`)**. The original-frame alternate recipe now composes the included estimator and one feature-owned GPU session over shared camera, scene-identity, evaluated triangle, material/texture, path, light, visibility, MIS, roulette, robust endpoint, and accumulation-arithmetic owners. Stage 7 source exposes the existing mode and its operational projection. Stage 8 source adds one automatic engine-wide Inline/Pipeline resolution policy over one Reference kernel, shared scene-trace/payload/hit mechanics, and the existing D3D12/Vulkan RHI mechanisms. The current tree is not compiled, cooked, or GPU exercised; numeric, lifecycle, navigation, response-budget, accessibility, backend/frontend, and raw-oracle evidence remains deferred with zero executable-readiness credit. Oracle authority remains undelivered. See [Current Feature Readiness](../../../../../../../Acceptance/CurrentReadiness.md#renderer).
...
| `ViewportRenderProgress` | The small renderer-agnostic product boundary used by the real Stage-7 overlay: operational state, exact committed/target/discarded work, reset or blocking reason, retention/capacity, active route, backend, throughput, and ETA. It carries no view-mode selector, estimator settings, digest, resource handles, strings, logs, event stream, or debug counters. |
