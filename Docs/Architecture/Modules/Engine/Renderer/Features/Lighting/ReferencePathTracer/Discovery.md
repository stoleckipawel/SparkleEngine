...
**Current readiness:** `PTD-00-R1` remains accepted and was independently confirmed compatible with the stricter feature-enclosure design. `PTD-01-R2 PASS` now closes Stage 1 against its exact production/evidence identity and authorizes Stage 2. Discovery and Stage-1 acceptance do not provide estimator, image, oracle, package, or release evidence. See [Current Feature Readiness](../../../../../../../Acceptance/CurrentReadiness.md#renderer).
...
| Hardware | Capability-driven route: Windows x64, selected D3D12 or Vulkan backend, hardware ray tracing for the requested Inline/Pipeline frontend, required shader binary64 controls, typed readback where used, and enough reported memory for the requested session. A failed predicate is `Unavailable`, not fallback. | Minimum/reference adapter, driver, performance, memory, TDR, and support promises require named-machine evidence before `FCR-REN-08` release closure. |
...
| `RPTC-06 Accumulation` | Known binary64 sequences, high-mean/small-variance values, prefix/reorder/batch/pause/resume/restart/overflow cases. | `MATH-10`, exact committed ordinal, deterministic merge, variance, checkpoint identity. |
