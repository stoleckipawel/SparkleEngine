...
| Application operations | At the report input, the now-retired `EditorOperationService.cpp` contained cancellable shader-recook and BMP-write work only; no reference operation/progress/offscreen manifest existed. The current private [`EditorOperationRuntime`](../../../../../../../../Engine/Application/Private/EditorOperations/EditorOperationRuntime.h) owns document task lifetime only, while each feature coordinator owns its typed slot and operation body. | Keep one bounded Reference-owned development operation consuming semantic Renderer snapshots; Application owns paths/process results, not estimator state, and no central service enumerates unrelated workflows. |
...
| Build reachability | `DebugEditor` and `DevelopmentEditor` expose the viewport/operation; `DebugGame` and `DevelopmentGame` may consume the semantic only through an approved development selector. | `ShippingEditor` and `ShippingGame` retain only the public enum value and compile out all producers/session factories/UI/CLI/writer/package routes; Stage 10 proves this from the package. |
