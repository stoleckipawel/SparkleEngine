...
| `AC-PTD-13` | The development viewport product and backend/capability/failure behavior are finite, discoverable, and actionable; every hardware-, package-, and Shipping-dependent assertion is explicitly a later executable gate rather than an implementation input. | strict D3D12/Vulkan/Inline/Pipeline matrix; accepted [User Experience](UserExperience.md); exact lifecycle, budgets, refusal behavior, output-root contract, six-profile target matrix, and later proof owners | Implementation must invent a user action, limit, fallback, route, writable location, or failure outcome, or documentation calls an unproved hardware/package/Shipping result supported. |
...
| `RPTC-08 Route/output` | All four strict backend/frontend pairs where capable; raw/AOV/manifest/checkpoint round trip; read-only/full disk, corrupt hash, staging cancellation, and Shipping reachability inspection. | adapter parity, no fallback, raw/preview lineage, atomicity, exact failure and profile policy. |
