...
**Authority boundary:** the [research report](Research.md) owns precedent and initial findings, [Transport And Estimator](TransportAndEstimator.md) owns the proposed mathematical contract and decision slots, [Execution Architecture](ExecutionArchitecture.md) owns the proposed system boundary, [User Experience](UserExperience.md) owns the proposed interactive/manual-output workflow, the [conditional plan](Plan.md) owns Stage 0 and the provisional delivery route, the [roadmap](../../../../../../../Strategy/Roadmap.md#reference-path-tracer-truth-first) owns priority, the [feature dossier](README.md) owns the eventual feature set and definition of done, and [`FCR-REN-08`](../../../../../../../Acceptance/FeatureCompletionReports.md#initial-completion-report-registry) owns its result
...
`PASS` freezes `PTD-01` to the exact discovery report and permits development-only Stages 1 through 9 under their own exit gates. It does not accept the feature, authorize release-map oracle use, approve a package or Shipping claim, or make the current route trustworthy. Stage 10, `FCR-REN-08` release closure, and `PTD-03` adoption remain gated by the accepted `ReleaseMapSet`, support machines, `REL-03`, and executable evidence. Any unresolved question that can change the target integral, development domain, architecture, ownership, or evidence validity keeps this gate blocked.
...
Every row must be `PASS`. “Documented,” “reviewed,” and “accepted” mean the exact retained artifact is linked and a named reviewer can reproduce the decision from it. Development unknowns may remain only when they are converted into an owned implementation-plan check with a bounded consequence that cannot change target architecture, development feature scope, estimator correctness, or evidence design. A later release decision must have an explicit Stage-10/`PTD-03` owner and may narrow, exclude, or reopen scope; it may not silently change the frozen development contract or infer support.
...
| `AC-PTD-03` | Every camera, geometry/deformation, material/texture, alpha/sidedness, light, environment, emission, and dynamic-scene behavior is included or excluded for the development `v0.1` contract. | feature-domain matrix reconciled with the frozen `RPTConformanceSet` and every development selector; release-map adoption explicitly deferred to Stage 10/`PTD-03` | A development-reachable or conformance-required semantic has no verdict, or an unreconciled release map is treated as supported. |
...
| `AC-PTD-13` | The development viewport product and backend/capability/failure behavior are finite, discoverable, and actionable; every hardware-, package-, and Shipping-dependent assertion is explicitly a later executable gate rather than an implementation input. | D3D12/Vulkan automatic frontend matrix; accepted [User Experience](UserExperience.md); exact lifecycle, budgets, refusal behavior, output-root contract, six-profile target matrix, and later proof owners | Implementation must invent a user action, limit, fallback, route, writable location, or failure outcome, or documentation calls an unproved hardware/package/Shipping result supported. |
...
| `AC-PTD-16` | The development implementation handoff is complete without beginning implementation; release adoption is isolated in Stage 10. | ordered backlog with owners, prerequisites, exact clean-break paths, estimates, key checks, and a development-versus-release gate table | Stages 1-9 need a release-map/package decision, or planning must reopen scope, architecture, estimator, evidence, or ownership. |
| `AC-PTD-17` | Reviewers outside the author path can reconstruct the development claim and identify at least one way each evidence class could fail. | exact-revision math/numeric, architecture/current-route, and UX/evidence review record with corrections and final decisions; repository-owner ratification | The report requires private explanation, review finds a plan-shaping ambiguity, or a reviewer/owner approval is inferred. |
...
| `RISK-PTD-03` | Feature-domain mismatch: a development selector, conformance scene, or later release map exercises an unmodeled or differently approximated material, light, camera, geometry, or alpha behavior. | Reconcile the development contract against `RPTConformanceSet` and development selectors now; require a separate Stage-10/`PTD-03` reconciliation for every future `ReleaseMapSet` row. | `CHK-PTD-01`, `CHK-PTD-05`, per-feature fixture ledger, later `CHK-RPT-14`. | Reject unsupported development content before sample zero; exclude or explicitly reopen scope before any release-map adoption. | Zero unmatched development/conformance semantic; later zero unmatched advertised/map-required semantic. |
...
| `RISK-PTD-08` | Backend/compiler/capability divergence changes output or failure behavior. | One semantic contract, truthful active-route reporting, pinned compiler/settings, and no product fallback. Design readiness requires the exact automatic matrix and refusal contract; executable support is earned per route in Stage 8. | `CHK-PTD-01`, `CHK-PTD-10`, native validation and paired numeric comparison. | Keep the failing route unavailable or repair before oracle use; Renderer/RHI owners. | Every advertised route passes capability, validation, controlled failure, and numeric tolerance before `FCR-REN-08`. |
...
| `RISK-PTD-10` | Reference accumulation hangs, TDRs, exhausts VRAM/disk, cannot cancel, or leaves a plausible partial result. | Freeze time/resource budgets, progress, watchdog, cancellation, atomic publish, cleanup, and recovery; treat hardware-specific achievement as Stage-6/9 evidence, not a Stage-0 fact. | `CHK-PTD-07`, `CHK-PTD-10` controlled timeout/OOM/disk/cancel cases. | Refuse or terminate safely, preserve diagnostic/partial state as non-candidate, and keep an unproved hardware row unavailable; runtime owner. | Every claimed support row's controlled failures end within budget with no complete marker, leaked state, or corrupted prior evidence. |
...
| `RISK-PTD-12` | Production implementation begins, or a conditional plan choice is treated as frozen, while discovery can still change architecture, scope, estimator, or evidence. | Hard `PTD-00` Stage-1 gate bound to one immutable accepted report; release-package gates remain hard only for Stage 10 and release claims. | `CHK-PTD-11`, `CHK-PTD-12`; inspect code/worktree changes, plan status, prerequisites, and unresolved questions. | Stop premature implementation and return to the unresolved discovery slice; do not use missing release packaging to alter the technical target. | All `AC-PTD-*` pass, zero plan-shaping unknowns remain, independent review and repository owner approve the exact revision, and Stage 1 records it. |
...
| `FM-PTD-11` | D3D12 and Vulkan differ beyond declared numeric tolerance, automatic resolution finds no complete frontend, or native validation reports an unexplained issue. | Fail `AC-PTD-13`; hold the affected backend/claim and preserve raw diagnostics. |
...
| `FM-PTD-16` | Discovery “passes” with an unresolved development item capable of changing architecture, development scope, estimator math, or evidence design, or with an unowned/inferred release claim that can silently broaden that contract. | Keep `PTD-00` blocked; unresolved development-shaping unknowns may not be transferred as ordinary implementation tasks, while explicit later release decisions remain at Stage 10/`PTD-03`. |
...
| `CHK-PTD-10` backend/failure protocol | `AC-PTD-13`; `FM-PTD-11`, `FM-PTD-13`; `RISK-PTD-08`, `RISK-PTD-10` | Specify D3D12/Vulkan automatic capability checks, native validation, compiler/settings identity, device removal/TDR/OOM/disk-full/timeout/cancel behavior, cleanup, and numeric parity. | matrix and expected diagnostic/artifact states | Incomplete automatic selection or unexplained validation output blocks the backend. |
...
10. accepted [User Experience](UserExperience.md) revision covering its P0-P3 delivery order, Reference Path Tracer immediately after Lit, automatic preflight/start, mode-safe defaults, responsive live navigation, newest-camera presentation, automatic post-motion refinement, exact progress/reset/completion, Editor/Game camera invalidation, Lit comparison retention/revalidation, pause/restart, secondary checkpoint/manual raw save, errors, accessibility, first use, and Shipping exclusion;
...
## Historical `PTD-00-R0` Stage-0 Execution Report
...
| Term | Frozen meaning |
| --- | --- |
| Reference Path Tracer | The development-only viewport product and manual evidence-output route defined by this dossier; a name alone grants no oracle authority. |
...
The target shape is accepted as one Renderer-owned per-view session, independent camera rays, one binary32 semantic megakernel-first estimator, thin Inline/RGS traversal adapters, binary32 transactional accumulation with exact integer count, live viewport presentation, and secondary evidence publication. A wavefront scheduler may be considered only after profiling and semantic equivalence; a GBuffer seed, separate batch renderer, duplicate scene/material system, multiple estimators, shader-double emulation layer, or compatibility selector is rejected.
...
| Backend/frontend | Included target: D3D12 and Vulkan through the engine-wide automatic Pipeline-then-Inline policy, with truthful active-route reporting and identical estimator/sample semantics. Both adapters require focused evidence wherever an accepted capability cell can exercise them. | An unavailable backend is an excluded product row, never a fabricated fallback. |
| Raw output | Included target: exact-extent binary32 prefix state, FLOAT OpenEXR beauty/AOVs, exact integer counters/statistics, canonical manifest, SHA-256, optional checkpoint. | BMP/screenshot/UI-scaled/display output is preview only. |
| Workflow | Included target: DevelopmentEditor live `Scene` view and DevelopmentGame `Game` view semantics with manual Editor evidence output. | Debug console/CVar choreography and command-line rendering are excluded. |
| Package | DevelopmentEditor includes selector/UI/manual writer after acceptance. DevelopmentGame may expose the semantic only through an approved development UI. ShippingEditor and ShippingGame exclude selector, CVar, writer, operation, docs promise, and optional dependencies. | Current recursive source/link membership does not establish this exclusion; package proof is required. |

`RPT-FS-01` through `RPT-FS-16` and shared-family architecture row `RPT-FS-20` are **Included development scope**, `RPT-FS-17` and `RPT-FS-18` are **Excluded**, and `RPT-FS-19` is **Prohibited in raw**. This disposition is sufficient for the development-only selector and `RPTConformanceSet`; it is not release-map ratification. Any later admitted map or public release selector reaching an excluded row blocks Stage 10/`PTD-03` or forces an explicit scope reopening before that map is adopted, without retroactively blocking implementation of the already bounded development contract.
...
| --- | --- | --- |
| Selectors | At the immutable report input, `RenderViewMode.h` and `ViewportTopPanel.cpp` contained Lit/debug only, while `LightingMode::ReferencePathTracer` was global, appeared in `RenderingSettingsPanel.cpp`, persisted as `r.Lighting.Mode`, and was reachable through generic command-line/runtime CVar surfaces. These are historical source-input facts; Stage 1 has since completed the clean break. | Add `RenderViewMode::ReferencePathTracer` immediately after Lit; remove the Lighting enum value, panel row, CVar/persistence/CLI meaning, and all producers/consumers in the same change. |
...
| Render route | At the report input, `Lighting.cpp` selected separate direct/indirect compute producers, common composite/sky, then frame-history accumulation. The now-retired `ReferencePathTracerIndirectLighting.hlsl` started from `RayTracingGBufferPathSurface`; the now-retired `ReferencePathTracerDirectLighting.hlsl` looped every analytic light and averaged per-frame samples. | Replace with an independent camera ray and one semantic estimator inside the existing frame architecture; delete the old producer topology after consumers migrate. Preserve real-time GBuffer/lighting only outside reference transport. |
...
| Invalidation | At the report input, the now-retired `ReferencePathTracerInvalidation.cpp` hashed lighting state, SPP/bounce/bias/distance, global view mode, and matrices; the now-retired accumulation shader additionally used a motion-vector epsilon and float mean/count. | Replace with semantic digest, exact committed range, generation rejection, reason taxonomy, target-SPP continuation rules, and presentation/scheduling non-reset. |
| RHI frontends | At the report input, the three now-retired Reference Path Tracer registrations were compute shaders requiring Inline RayQuery. Generic D3D12/Vulkan Inline and RGS machinery exists; no RPT RGS adapter exists. | One estimator with thin four-pair traversal adapters; RHI owns mechanism/capability, Renderer owns path semantics. |
...
| Application operations | At the report input, the now-retired `EditorOperationService.cpp` contained cancellable shader-recook and BMP-write work only; no Reference artifact operation existed. The current private [`EditorOperationRuntime`](../../../../../../../../Engine/Application/Private/EditorOperations/EditorOperationRuntime.h) owns document task lifetime only, while each feature coordinator owns its typed slot and operation body. | Keep one bounded, manually initiated Reference artifact operation consuming semantic Renderer snapshots; Application owns paths/results, not estimator state, and no central service enumerates unrelated workflows. |
...
| `PTD-Q-05` | Resolved for implementation: the development domain is frozen against `RPTConformanceSet` and development selectors. Exact `ReleaseMapSet` reconciliation is deliberately owned by Stage 10/`PTD-03`; no unreconciled map may inherit support. |
...
| `PTD-Q-10` | Re-resolved by the owner-directed precision amendment: binary32 mean/M2, exact uint32 count, ordered range merge, max `1,048,576` SPP, and mandatory comparison to a higher-precision host oracle. |
...
| Binary32 mean/M2 and exact uint32 count through `1,048,576` SPP, with ordered merge and a higher-precision error oracle. | Shader float64 and a custom double-math layer add capability, compiler, maintenance, and backend complexity without demonstrated product benefit. A packed float count, EMA, or `sum(X^2)-sum(X)^2/n` remains rejected. |
...
| --- | --- |
| Delete | `Engine/Assets/Shaders/Passes/RayTracing/ReferencePathTracerAccumulation.hlsl`; `ReferencePathTracerDirectLighting.hlsl`; `ReferencePathTracerIndirectLighting.hlsl`; `Engine/Assets/Shaders/RayTracing/ReferencePathTracerUniform.hlsli`; the old accumulation/invalidation/sample helpers below `Engine/Renderer/Private/Passes/Lighting/ReferencePathTracer/` while replacing its narrow entry point in place; `Engine/Renderer/Private/Passes/Lighting/Direct/ReferencePathTracerDirectLighting.{h,cpp}`; `Engine/Renderer/Private/Passes/Lighting/ReferencePathTracerIndirectLighting.{h,cpp}`; `Engine/Renderer/Private/Passes/RayTracing/ReferencePathTracer{Accumulation,DirectLighting,IndirectLighting}Shader.h`; `Engine/Renderer/Private/RayTracing/Effects/ReferencePathTracer/ReferencePathTracerAccumulationUniformData.h`, `ReferencePathTracerCVars.{h,cpp}`, `ReferencePathTracerSettings.{h,cpp}`, and `ReferencePathTracerUniformData.h`; `Engine/Renderer/ShaderRegistrations/ReferencePathTracer{Accumulation,DirectLighting,IndirectLighting}Shaders.cpp`. |
| Modify | `Engine/Renderer/Public/Settings/EngineRenderingRayTracingTypes.h`; `Engine/Renderer/Public/Settings/EngineRenderingSettings.h`; `Engine/Renderer/Public/Debug/RendererCVars.h`; `Engine/Renderer/Private/Debug/RendererCVars.cpp`; the Editor-private rendering-settings section; `Engine/Renderer/Private/Settings/EngineRenderingSettingsRuntime.cpp`; `Engine/Editor/Private/Panels/RenderingSettingsPanel.cpp`; `Engine/Renderer/Private/Passes/Scene/SceneRenderingPasses.cpp`; `Engine/Renderer/Private/Resources/History/FrameHistory.{h,cpp}`; `Engine/Renderer/Private/View/RenderViewState.{h,cpp}`; `Engine/Renderer/Private/Frame/FramePipeline.h`; `Engine/Renderer/Private/Frame/FramePipelineGraph.cpp`; `Engine/Renderer/CMakeLists.txt`; `Engine/Editor/Private/UI.cpp`; `Engine/Editor/Private/Panels/ViewportPanel.cpp`; Application-owned rendering-settings persistence if its serialized numeric contract changes. |
...
Revised ranges after the live audit are: Stage 0 `70-110 h` including external owner reconciliation/re-review; Stage 1 `50-85 h`; Stage 2 `80-130 h`; Stage 3 `75-125 h`; Stage 4 `110-180 h`; Stage 5 `90-150 h`; Stage 6 `80-135 h`; Stage 7 `70-120 h`; Stage 8 `90-150 h`; Stage 9 `145-250 h`; Stage 10 `70-125 h`; total `930-1,560 h`. The precision amendment removes Reference-specific shader-float64 work but retains the full binary32 error study, four traversal pairs, exact Scene/Game migration, shader/package clean break, and independent external/statistical evidence, so no unsupported schedule reduction is claimed. Release-map breadth may revise Stage 5/9/10 only through a new accepted report.
...
Historical R0 decision: **`PTD-00-R0 BLOCKED` at `669637cf23b9748f8b94635409e74159d31d0bc2`**. R0 required the affected release-map/support/package reconciliation and all independent reviews after `REL-01` supplied the exact inputs, and did not authorize Stage 1. The R1 section below supersedes only that gate placement; it does not rewrite or erase the evidence and blocker record that produced R0.

## `PTD-00-R1` Gate-Separation Reconciliation

### Why R0 was over-constrained

R0 correctly refused to invent release evidence, but incorrectly made three later delivery facts prerequisites for writing the development implementation: the final `ReleaseMapSet`, named support machines, and an already-proved package/Shipping route. That ordering is circular. The tracer must exist before package and map evidence can exercise it, while the exact estimator, ownership, failure, and development reachability decisions are already frozen independently of those facts.

R1 therefore changes gate placement, not the evidence bar. Missing release inputs can no longer alter Stages 1-9 by implication. They remain hard blockers for the exact claims they own: Stage 10 release adoption, `FCR-REN-08` release closure, `PTD-03`, and any packaged/Shipping/support statement.

| Concern | Development implementation gate | Later release gate |
| --- | --- | --- |
| Scene coverage | Exact `RPTConformanceSet` below plus whole-scene preflight against the frozen included/excluded domain. Unsupported content rejects before sample zero. | Every `ReleaseMapSet` map/camera must be reconciled row by row in Stage 10 and `PTD-03`; no automatic inheritance. |
| Hardware | Capability-driven route: Windows x64, selected D3D12 or Vulkan backend, hardware ray tracing for the requested Inline/Pipeline frontend, ordinary binary32 shader support, typed readback where used, and enough reported memory for the requested session. Shader float64 is not a Reference Path Tracer prerequisite. A failed predicate is `Unavailable`, not fallback. | Minimum/reference adapter, driver, performance, memory, TDR, and support promises require named-machine evidence before `FCR-REN-08` release closure. |
| Output root | Stages 1-8 need no filesystem output. Stage 9 requires an explicit ApplicationEditor-owned writable destination, unique staging sibling, and transactional publication; it must never write through Renderer or assume an install root. | `REL-03` owns immutable installed bytes, the per-user writable default, staged package membership, notices, and clean-machine proof. |
| Build reachability | `DebugEditor` and `DevelopmentEditor` expose the viewport/manual-output operation; `DebugGame` and `DevelopmentGame` may consume the semantic only through an approved development selector. | `ShippingEditor` and `ShippingGame` retain only the public enum value and compile out all producers/session factories/UI/writer/package routes; Stage 10 proves this from the package. |
| Authority label | All Stages 1-9 output remains `candidate comparison`; target SPP and a clean image confer no oracle authority. | Only accepted `FCR-REN-08` evidence permits the bounded reference claim; only `PTD-03` admits a release-map oracle use. |

### Frozen `RPTConformanceSet`

This is an implementation/evidence input, not distributable Showcase content. Fixtures are project-authored numeric manifests generated into ignored temporary evidence roots and removed unless an accepted evidence record retains their hashes.

| ID | Required construction | Primary decisions falsified |
| --- | --- | --- |
| `RPTC-01 Camera` | Empty/constant environment; odd/even extents; center, edge, corner, and fixed subpixel samples; translated/rotated camera; projection/crop rejection. | `MATH-01`, coordinate handedness, half-pixel, Y orientation, clip interval, reset identity. |
| `RPTC-02 Geometry` | One indexed triangle plus adjacent, coplanar, thin-gap, mirrored, nonuniform-scale, shear, and `10^-6..10^6` scale variants. | hit reconstruction, winding/sidedness, transforms, robust endpoints, stale generation. |
| `RPTC-03 Material` | Base-color/vertex RGBA, metallic/roughness channel/factor extremes, flat and mapped normals, opaque/mask cutoff equality, one-/two-sided emission, unsupported blend/transmission. | glTF decode/order, BSDF/sample/PDF, delta limit, normal correction, alpha continuation, preflight rejection. |
| `RPTC-04 Lights` | Individually isolated point, hard/smooth spot, directional, rectangle, emissive triangle, black/constant/one-hot environment, plus duplicate-event and collinear-distinct-emitter cases. | units, eligibility/order/PMF, area Jacobian, event-specific MIS, delta handling, environment mapping. |
| `RPTC-05 Estimator` | Zero, isolated Lambert term, realizable mirror, equal/zero competitor PDFs, emission-hit split, roulette threshold, finite depths `D=1,2,3`, safety-depth and invalid-value injection. | `MATH-02..09`, double counting, exact probability compensation, full-versus-finite labels. |
| `RPTC-06 Accumulation` | Known binary32-representable sequences, high-mean/small-variance values within the declared representable domain, higher-precision expected values, prefix/reorder/batch/pause/resume/restart/overflow cases. | `MATH-10`, exact committed ordinal, deterministic merge, variance/error bound, checkpoint identity. |
| `RPTC-07 Lifecycle` | Editor `Scene` and runtime `Game` cameras; motion/reset/refine; scene/material/light mutations; presentation/scheduling changes; Lit retained and no-retention paths; cancel/timeout/capacity/transfer/device failure. | session state machine, invalidation classification, newest-view behavior, bounded settlement and resource truth. |
| `RPTC-08 Route/output` | Automatic D3D12/Vulkan frontend cells plus focused evidence for both adapters where capable; raw/AOV/manifest/checkpoint round trip; read-only/full disk, corrupt hash, staging cancellation, and Shipping reachability inspection. | adapter parity, no product fallback, raw/preview lineage, atomicity, exact failure and profile policy. |

Pinned Falcor, Capsaicin, and Mitsuba interchange recipes remain external challenges after semantic equivalence is established. They are not members of `RPTConformanceSet`, copied assets, release maps, or sole expected-value authorities.

### R1 independent review record

Three fresh read-only review tracks inspected the live R1 documentation reconciliation against committed source input `30597d7d0bb70af9f2836ab01d81d47c3e20bcde`. They made no file changes and ran no build, shader, GPU, runtime, statistical-execution, package, or clean-machine check.

| Review | Result | Findings and retained correction |
| --- | --- | --- |
| Mathematical and numerical | **PASS (design)** | Moving release-map/support/package gates does not change any accepted `MATH-*` equation, measure, unit, sampler dimension, accumulation rule, maximum SPP, numerical policy, estimator domain, or statistical threshold. Review caught and corrected premature Stage-9 “oracle authority” wording; Stage 9 now produces oracle-candidate evidence, while Stage 10 plus accepted `FCR-REN-08` grants bounded authority. |
| Architecture and plan readiness | **PASS (design)** | The development/release gate split is coherent across the dossier, roadmap, first-release Renderer route, and acceptance projection. Review caught and corrected the reusable Stage-0 stop rule so absent assigned release inputs cannot recreate the R0 circle, and verified that the public source enum remains while Shipping producers and executable routes are excluded. |
| UX, first use, and evidence | **PASS (design)** | View-mode-first interaction, automatic accumulation/reset/refinement, progress truth, Lit comparison, refusal/error behavior, numerical/resource/lifecycle budgets, raw-versus-preview separation, conformance/statistical/oracle/fault protocol, and later Stage-10 release gates remain fully specified with no plan-shaping ambiguity. |

These are independent AI-assisted design reviews, not implementation evidence and not repository-owner approval. Their `PASS` removes the R1 cross-discipline review blocker only; immutable repository-owner ratification remains the sole `AC-PTD-17` action.

### R1 criterion disposition

| Criterion group | R1 candidate result | Reason |
| --- | --- | --- |
