...
**Status:** **`PTD-00-R1 PASS`** remains the immutable discovery prerequisite; **`PTD-01-R2 PASS`** accepts the Stage-1 ordinary-selector/Private-capsule clean break; **`PTD-02-R0 PASS`** accepts the host-agnostic alternate-frame recipe at source revision `8b650c7450f8a59fb3bcc18edbb4d217a7b11ed5`; Stage 3 is authorized, while Stage 10/release closure remains gated by `REL-03`, release maps, support identities, and executable evidence

**Scope:** deliver `FCR-REN-08` end to end through one feature-local Renderer per-view session, one semantic path estimator, viewport-first Lit comparison, optional raw evidence publication, D3D12/Vulkan traversal parity, secondary runtime/offscreen workflows, controlled failure, and release-map adoption
...
**Current readiness:** **25/100 (`20/5/0/0`)**. `PTD-02-R0 PASS` adds one topology-keyed, host-agnostic Reference middle recipe to the original frame and corrects Editor View kinds without adding transport machinery. Stage 3 is authorized. No camera-ray implementation, sampler, estimator, session identity, accumulation, clickable product route, runtime transition evidence, oracle, package, or release credit is claimed. See [Current Feature Readiness](../../../../../../../Acceptance/CurrentReadiness.md#renderer).

**Non-claims:** the prior Stage 1 build and focused checks prove neither feature enclosure nor ownership acceptance. They also produce no shader transport, image, convergence, backend parity, performance, package, Shipping, or accepted-reference evidence.
...
3. Define one Renderer shader semantic core for path state, surface event, BSDF result, light event, trace result, contribution, and required invalid disposition; keep API-specific and Editor/Game host types outside it.
