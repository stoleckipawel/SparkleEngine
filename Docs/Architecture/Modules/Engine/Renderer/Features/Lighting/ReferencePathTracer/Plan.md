...
**Current readiness:** **40/100 (`35/5/0/0`)**. Stage 5 now routes the host-agnostic Reference estimator through the canonical evaluated-triangle, material-hit, alpha, texture, path-surface, and robust-endpoint owners shared with existing ray/path consumers. UV0 transforms, glTF address modes, base-level bilinear reconstruction, color decode through cooked texture formats, vertex color, normal scale, sidedness, emission, and frozen skin/morph snapshots are source-present; primary, continuation, and two-ended connection rays use one binary32 endpoint owner without a scene-tuned bias. The candidate remains uncompiled, uncooked, and GPU-unexercised, so no analytic/robustness matrix, runtime, parity, or reference-authority credit is claimed. See [Current Feature Readiness](../../../../../../../Acceptance/CurrentReadiness.md#renderer).

**Non-claims:** Stage-5 source inspection and predecessor builds/cooks do not prove the current GPU shader candidate's material decode, robust endpoints, radiance, estimator correctness, runtime reachability, convergence, backend parity, performance, package, Shipping, or accepted-reference authority.
...
| 3 | canonical primary rays, stable samples, and minimal reviewable integrator in the Reference middle recipe | 90-150 h | Stage 2 | `CHK-RPT-03`, `04`, `05`, `07`, `17`, `18`, `19` |
| 4 | complete included surface estimator | 110-180 h | Stage 3 | `CHK-RPT-04`, `05`, `11`, `17`, `18`, `19` |
| 5 | material/geometry coverage and numeric robustness | 90-150 h | Stage 4 | `CHK-RPT-03`, `04`, `08`, `17`, `18`, `19` |
| 6 | canonical digest/invalidation, ranges, in-memory accumulation, live preview, and progress snapshots | 115-185 h | Stage 5 | `CHK-RPT-02`, `07`, `09`, focused `15`, `17`, `18`, `19` |
| 7 | first usable viewport mode, live navigation, progress/reset UX, and Lit comparison | 70-120 h | Stage 6 | `CHK-RPT-02`, `09`, `15`, `16`, `17`, `18`, `19` |
| 8 | Inline/RGS and D3D12/Vulkan parity | 90-150 h | Stage 7 | `CHK-RPT-07`, `08`, `12`, `17`, `18`, `19` |
| 9 | minimal evidence capture, EXR/checkpoint/offscreen publication, independent oracle, and failure evidence | 145-250 h | Stage 8 | `CHK-RPT-04` through `13`, plus `17`, `18`, `19` |
| 10 | package/adoption evidence, cleanup, completion report | 70-125 h | Stage 9, accepted `ReleaseMapSet`, support identities, `REL-03` | `CHK-RPT-01`, `14`, `15`, `16`, `17`, `18`, `19` |
| **Total** | full first-release closure | **915-1,535 h** | accepted scope | all applicable `AC-RPT-01` through `23` |
...
14. run immediately available static checks, `architecture_boundary_check` whenever Renderer/RHI boundaries change, focused build/shader/GPU/interactive checks when selected for the current execution, and `git diff --check`. Record every unrun owner-operated check as deferred with its command/workflow and oracle. Do not claim unrun checks passed and do not convert deferral alone into `BLOCKED`;
15. leave one iteration record containing gate revision, decisions, changed files, deletions, `ARCH-RPT-<stage>`, `FRAME-RPT-<stage>` when applicable, `CORE-RPT-<stage>` when applicable, checks actually run, exact outputs/artifact links, deferred-validation backlog, known limitations, confirmed blockers, and the next permitted implementation stage.
...
- Reference product/session/estimator policy remains inside `Engine/Renderer/Private/Passes/Lighting/ReferencePathTracer/` and its matching shader home. Outside edits are limited to the frozen integration-hook ledger or `CORE-RPT-*` extensions of existing focused shared path/RayTracing/Lighting/Common owners with affected consumers updated. `FramePipeline` may select, invoke, and publish only; generic Scene, View, frame resources/history, settings, RHI, Editor, and Application types never acquire feature mechanism or mutable feature truth.
...
Against source input `5e6badf03df44133eb75db54875417821a9909c8` plus the current uncommitted Stage-5 tree, Stage 5 is **IMPLEMENTED / VALIDATION DEFERRED**. The source closes the accepted GPU content route and replaces provisional path-ray tuning with the shared binary32 `MATH-11` endpoint implementation. At repository-owner direction, no build, shader cook, GPU workload, or Editor session was run; none is reported as passed.

| Evidence field | Retained result |
| --- | --- |
| Implemented content slice | glTF `KHR_texture_transform` on UV0, normal and occlusion strength, declared repeat/clamp/mirrored addressing, cooked transfer format, exact base-level bilinear reconstruction, vertex RGBA, factor/channel multiplication, deterministic alpha mask, one-/two-sided winding, tangent normal maps, emission, and current frozen skin/morph evaluation now flow through the canonical shared material/hit route. The cooked-material magic is clean-break advanced with the texture-mapping record ABI, so older material products are rejected and must be recooked. Nonzero texture-coordinate sets remain rejected at import because the accepted mesh ABI exposes UV0 only. AO remains available to raster consumers but never enters raw Reference transport. |
| `MATH-11` source binding | `RayEndpoints.hlsli` owns the frozen binary32 constants, triangle reconstruction/transform/traversal bound, geometric-normal offset, directed one-ULP rounding, camera interval wrapper, infinite continuation ray, and two-ended unnormalized connection interval. `MeshInstanceData` publishes the exact inverse transform needed by the bound. No Reference file contains offset arithmetic, fixed `MinT`, normal/grazing bias, or maximum-distance policy. |
...
- Focused `CHK-RPT-02`, `09`, `15`, `16`, `18`, and `19` close the primary viewport portions of `AC-RPT-02`, `04`, `13`, `18`, `20`, retained `AC-RPT-22`/`23`, and `FM-RPT-09`, `14`, `17`, `18`, `21`, `22`.

### Ready-to-use prompt

```text
Implement only Stage 7 of Docs/Architecture/Modules/Engine/Renderer/Features/Lighting/ReferencePathTracer/Plan.md after the Stage-6 live-accumulation source handoff is complete. Apply the Universal Execution Contract and carry every deferred GPU or interactive check forward without treating it as passed.
...
Use only thresholds, scenes, repetitions, seeds, regions, budgets, and stop rules frozen before execution. Record raw artifacts, hashes, manifests, source/compiler/driver identities, injected defect and detection, exact commands, outcomes, cleanup, and limitations. Retain `FRAME-RPT-9` through `CHK-RPT-18` and `CORE-RPT-9` through `CHK-RPT-19`, proving that offscreen intent uses the same original frame recipe, publication reads the same immutable prefix rather than launching another renderer, and artifact work neither duplicates capture infrastructure nor forks shared path semantics. Treat semantic mismatch or unexplained disagreement as Inconclusive/Blocked. Fix discovered defects at their owner and rerun only invalidated evidence; never tune output or thresholds to pass. End with a no-orphan acceptance/failure/check matrix and an exact Stage 10 readiness decision.
```
...
- `CHK-RPT-14`, `15`, `16`, `17`, `18`, and `19` pass in addition to retained Stage 9 evidence.
