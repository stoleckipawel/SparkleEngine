...
**Status:** **`PTD-00-R1 PASS`** remains the historical immutable discovery prerequisite; the 2026-09-13 repository-owner amendment replaces its Reference-shader binary64 policy with ordinary binary32 and must be included in the next independent numeric review; Stages 1-2 pass their selector/frame gates; **`PTD-03-R0 BLOCKED`** retains a minimal transport source slice whose latest shared-family refactor is not compiled/cooked and has no real raw GPU oracle evidence, so Stage 4 is not authorized; Stage 10/release closure remains separately gated by `REL-03`, release maps, support identities, and executable evidence
...
**Current readiness:** **35/100 (`30/5/0/0`)**. `PTD-03-R0 BLOCKED` retains a host-agnostic minimal camera/Lambertian/emissive/environment-miss source route in the original frame and reconciles its feature-independent ray-tracing mechanisms into shared owners. The latest shared-family source refactor is not compiled or cooked. No real raw GPU analytic/fault result, full estimator, session identity, accumulation, clickable product route, runtime transition evidence, parity, package, or reference-authority credit is claimed. See [Current Feature Readiness](../../../../../../../Acceptance/CurrentReadiness.md#renderer).

**Non-claims:** the Stage-3 source, builds, cooks, and host arithmetic probes do not prove compiled GPU radiance, estimator correctness, runtime reachability, convergence, backend parity, performance, package, Shipping, or accepted-reference authority.
...
> [!CAUTION]
> Do not advance a stage because this file or compiled source exists. Stage 4 requires the Stage-3 real raw GPU analytic/fault gate to pass. `REL-03`, release maps, and named support machines remain mandatory for Stage 10 and every release/package/oracle-adoption claim.
...
- one Reference estimator policy over one shared path-tracing semantic core, behind strict Inline and native Pipeline adapters on D3D12 and Vulkan;
...
5. preserve Scene-owned scene data, View-owned view identity/camera data, one private Reference Path Tracer capsule, one shared API-neutral path-tracing semantic core, thin RHI traversal adapters, and the single-truth/copy budget. Preserve the original Sparkle frame shell: one `BuildRenderFrameGraph` decision selects mutually exclusive Lit or Reference middle recipes, never a second renderer/frame loop/submission path and never both estimator middles together;
6. keep Reference-specific target, sampler identity, admissibility, estimator choices, termination, MIS, state, resource composition, evidence, and failure policy in the feature capsule. Put only genuinely identical path-state, surface/event, BSDF sample/evaluate/PDF, radiance, throughput, traversal, and numeric mechanisms in the narrow shared Renderer/RayTracing owner. Search current Reference, `Path*`, ReSTIR, and cache-facing code for semantic duplicates before adding code. A shared extraction requires a current consumer or removal of duplicate authority; named future ReSTIR PT, SHARC/cache, or denoiser use alone cannot create a utility/framework. No generic mechanism may retain a `ReferencePathTracer` name, and no biased/Reference-only policy may move into the shared owner;
...
13. from Stage 3 onward, execute `CHK-RPT-19` and retain `CORE-RPT-<stage>` classifying every changed path-tracing operation as shared invariant or estimator policy, proving current consumers, duplicate removal, dependency direction, Reference-policy enclosure, and affected Reference plus optimized-consumer shader compilation;
14. run `architecture_boundary_check` whenever Renderer/RHI boundaries change, focused build/shader checks required by the stage, and `git diff --check`. Do not claim unrun checks passed;
15. leave one iteration record containing gate revision, decisions, changed files, deletions, `ARCH-RPT-<stage>`, `FRAME-RPT-<stage>` when applicable, `CORE-RPT-<stage>` when applicable, checks actually run, exact outputs/artifact links, known limitations, blockers, and the next permitted stage.
...
- `RenderViewKind` identifies the ordinary View/camera producer (`Scene` for Editor-authored viewports, `Game` for runtime views); `RenderViewMode` independently selects the Lit or Reference frame recipe. Every supported Editor and Game view reaches the same feature entry, session semantics, estimator, and presentation path. Editor UI, Game UI, a development CVar, and offscreen requests are adapters only and may not fork Renderer behavior.
...
- Stages 3 through 10 pass `CHK-RPT-19` independently. A Reference-prefixed reusable primitive, duplicated optimized-path operation, or shared core containing Reference-only/unbiased or biased/cache/denoiser policy blocks the stage even when outputs compile.
...
## Stage 1 - Establish One Ordinary View Mode And A Feature-Local Owner

### Objective

Install the smallest production seam for an ordinary per-viewport view mode and remove the duplicate GBuffer-seeded public authority before transport expansion. Keep all path-tracer-specific execution state under the feature owner. No candidate artifact may yet be called an accepted reference.

### Work

1. Define `RenderViewMode::ReferencePathTracer` directly after `Lit` with contiguous C++/HLSL values. Carry it through the same per-viewport request and `RenderView` path as every other view mode.
...
- One contiguous C++/HLSL `RenderViewMode` target exists and flows through the ordinary request/View/frame path; no Lighting selector or settings copy remains.
- Editor session state, Renderer request data, the feature-local unavailable decision, and generic progress observation have distinct owners.
...
From the live tree, add `RenderViewMode::ReferencePathTracer` immediately after `Lit` with contiguous C++/HLSL values and carry it through the existing per-viewport request, `RenderViewBuilder`, `RenderView`, and `FramePipeline` like any other view mode. Editor selection belongs to `EditorViewportSession`, never `EngineRenderingSettingsState`. Put the only feature-specific production owner in Private below `Passes/Lighting/ReferencePathTracer`; in Stage 1 it recognizes only the selected semantic and reports generic unavailability without manufacturing Scene/View/session identity. Add no generic progress surface before a real Editor consumer proves that stable boundary. Keep the clickable item unavailable until Stage 7 connects real shaders and accumulation. Remove `LightingMode::ReferencePathTracer`, the GBuffer-seeded reference authority, every producer/consumer/setting that exists only for it, and all duplicate selector authority; update generated metadata, build membership, and docs as one clean break. Do not implement the new estimator, EXR writer, UI beyond the generic progress reader, generic job system, compatibility alias, or fallback selector.

NON-NEGOTIABLE: Reference Path Tracer implementation details must not spread across generic Renderer View, settings, Scene, frame resources/history, RHI, Editor, or Application contracts. The only unconditional shared addition is the ordinary enum value and its existing request/View transport. A generic progress payload is allowed only when the real Editor consumer proves that stable boundary; otherwise keep it feature-local until Stage 7. `FramePipeline` or Lighting owns at most one select/invoke/publish hook and no feature state transition, validation, allocation, digest, sample, shader, or error policy. No Stage 1 selector, progress object, retained Lit image, or result may imply authority evidence has not earned.

Exercise exact enum parity/order, Editor-session ownership, request/View propagation, feature-local ownership, `RenderViewMode` versus `LightingMode` selector removal, and clean-break claims with focused checks. Execute `CHK-RPT-17` and retain `ARCH-RPT-1`; run `architecture_boundary_check` and `git diff --check`. Stop on duplicate Scene/View/settings/selector authority, an unledgered outside-feature file, a public/generic contract without a proved consumer, repeated feature selection, or path-tracer mechanism escaping the capsule. Handoff exact changed/deleted files by hook role, checks actually run, evidence, limitations, and Stage 2 readiness. Do not change either Editor `RenderViewKind::Game` producer in this stage; that camera-semantic clean break belongs to Stage 2.
```
...
| --- | --- |
| Scope | Only the ordinary per-view selector seam, generic progress-bar payload, Private feature owner, old-route clean break, and documentation reconciliation are in scope. No estimator, accumulator, EXR/checkpoint/offscreen path, clickable Reference Path Tracer item, or accepted-reference result is added. |
| Architecture | `ViewportRenderRequest` carries the same `ViewMode` field for every mode; `EditorViewportSession` owns Editor selection; `RenderView` carries only the selected mode; `EngineRenderingSettingsState` contains neither view mode nor a lighting-route selector. `FramePipeline` calls one Private `Passes/Lighting/ReferencePathTracer` owner inside the existing renderer/frame architecture. No public Reference Path Tracer contract, feature-specific View state, second renderer, or copied Scene/View truth remains. |
| Selector | `RenderViewMode` is contiguous and source-ordered: `Lit=0`, `ReferencePathTracer=1`, then the shifted final/debug values through `GpuSceneInstances=16`, mirrored exactly in HLSL. The former `LightingMode`, `r.Lighting.Mode`, persisted setting, panel row, and all producers/consumers are absent. The Editor menu deliberately does not offer the new item before Stage 7. |
| State truth | The shared surface is renderer-agnostic: selected view mode, `None`/`Unavailable`/`Rendering`/`Complete`, completed work, and target work. `ViewportPanel` is the real consumer and suppresses a snapshot for a different selected mode. Stage 1 reports only generic unavailability with `0/0` work and never relabels retained Lit pixels as reference output. It deliberately contains no session generation, reset reason, backend, estimator, digest, or result claim before the real session owns those truths. |
| Architecture fitness | The Private owner alone recognizes `ReferencePathTracer`; `FramePipeline` contains one incomplete-type owner member, constructs it once, and performs one unconditional `Update`/publish call. It contains no feature switch, lifecycle transition, validation, reset, allocation, shader, sample, or error policy. The generic progress surface is justified by the existing Editor product-snapshot boundary and one feature-neutral overlay. |
...
| C++/HLSL `RenderViewMode` definitions | one ordinary selector value | Contiguous value `1` immediately after Lit. Removing the feature deletes only this mirrored row and shifts the later contiguous values together. |
| `ViewportRenderRequest` -> `RenderViewBuilder` -> `RenderView` | existing ordinary per-view transport | Carries the same `ViewMode` field used by every debug/final mode; no feature-named field or type exists. This transport remains useful if the feature is deleted. |
| `EditorViewportSession` -> `UI` -> `ViewportPanel`/`ViewportTopPanel` | existing viewport selection and minimal presentation | Session, not rendering settings, owns the mode. The dormant item is not listed in the combo; its exhaustive label/icon and generic unavailable overlay remove with the enum/progress hook. |
| `FramePipeline` | one composition/lifetime hook | One Private incomplete-type member, construction, and unconditional update/publication call; no repeated selector or feature policy. Removing those four sites leaves the frame architecture unchanged. |
...
4. Keep `RenderViewKind` and `RenderViewMode` orthogonal. `Scene` and `Game` requests carrying `ReferencePathTracer` enter the same graph key and feature composition entry; add no Editor check, Editor-owned transport path, runtime fork, or feature-specific render command.
5. Keep the dormant Reference recipe truthful: until Stage 3 connects real transport, it may publish only the existing generic unavailable state and a non-authoritative display product. It must not retain or relabel Lit output.
...
7. Validate recipe-key topology, mutually exclusive middle ownership, generic downstream shape, Editor/runtime View-kind routing, and bounded removal with source/build checks. Do not ship a feature-specific diagnostic pass, readback, event stream, debug UI, dashboard, or inspection API.
...
- For the same supported per-view intent, `Scene` and `Game` differ only in their normal View/camera producer semantics; both select the identical Reference recipe and no feature implementation branches on Editor versus Game.
...
Keep the original Renderer frame architecture. Add the ordinary selected `RenderViewMode` to the frame-graph recipe key and make `BuildRenderFrameGraph` the sole outside-feature choice between the existing Lit middle and one feature-local Reference Path Tracer composition entry; both routes retain the same frame admission, Scene/View preparation, RT-scene publication, graph execution, RHI submission, viewport product, host UI, and presentation owners. Keep this high-level function declarative: it selects the recipe once and passes the generic result to the shared tail; provider, upscaler, reconstruction, resource, and shader decisions stay inside the selected recipe, with no feature ternary in the shared tail. Clean-break the two Editor producers to `RenderViewKind::Scene`, while runtime remains `Game`. Treat `RenderViewKind` and `RenderViewMode` as orthogonal: a supported Scene or Game View carrying `ReferencePathTracer` must enter the identical graph key and feature composition entry, with no Editor check or runtime-specific transport fork. Until Stage 3 connects real transport, keep the Reference recipe truthfully unavailable and never retain or relabel Lit output.

NON-NEGOTIABLE: Stage 2 adds no Reference-specific camera, camera copy, matrix-to-ray implementation, sampler, CPU mirror, RNG utility, shader, resource, digest, session, range, or state without a current production consumer. The feature will consume the canonical View camera directly when Stage 3 installs the first transport pass. The Reference recipe has no GBuffer/ReSTIR/reconstruction dependency, no production state that lacks a current consumer, and no Editor-only implementation branch. Editor, Game/runtime, and later offscreen selectors remain thin adapters to the same ordinary `RenderViewMode` semantic.
...
**`PTD-02-R0 PASS`** is retained against source input `8b650c7450f8a59fb3bcc18edbb4d217a7b11ed5`; the production slice entered at `8e4ffba225411965dc51c0b783e5f47a075c7e84` and `git diff -- Engine` is empty at this review. This is an architecture/integration gate only. It does not claim a camera-ray implementation, sampler, transport result, accumulation, clickable product, or accepted reference.

| Evidence field | Retained result |
| --- | --- |
| Recipe key and decision | `RenderFrameGraphSettings::ViewMode` participates in defaulted full-key equality. `FramePipeline::RefreshGraphForTopology` compares that key and uses the existing history invalidation, graph retirement, and rebuild route. `BuildRenderFrameGraph` contains exactly one `ReferencePathTracer` mode test and one feature composition call. |
| `FRAME-RPT-2` shared shell | Both recipes share frame admission, prepared Scene/View, ray-tracing-scene publication, graph allocation/execution, RHI submission, viewport publication, and post-processing/presentation. The Lit middle schedules GBuffer, lighting, exposure, and reconstruction. The dormant Reference middle schedules an explicit unavailable clear, exposure, and provider-free linear resolve, then reaches the same downstream tail. It schedules no GBuffer, ReSTIR/Lighting, or ray reconstruction and cannot retain or relabel Lit output. |
| Host and View-kind equivalence | `ViewportRenderRequest` and `RenderView` carry `ViewKind` and `ViewMode` independently. The two Editor producers now submit `Scene`; runtime defaults remain `Game`. The recipe key and feature capsule inspect only `ViewMode`: no `RenderViewKind`, Editor type, host conditional, second renderer, or runtime-specific estimator route exists. DevelopmentEditor and DevelopmentGame compile the same `ReferencePathTracer.cpp` and `ReferencePathTracerComposition.cpp` in `SparkleRenderer`. |
| `ARCH-RPT-2` outside-feature ledger | `FramePipelineGraph.cpp` carries the existing ordinary mode into the graph key; `BuildRenderFrameGraph.h/.cpp` own the one generic key/recipe decision; `UI.cpp` and `ViewportPanel.cpp` correct existing Editor producer semantics; `PostProcessing.h/.cpp` delete the one-line pre-reconstruction wrapper so recipes call `AddExposurePass` directly. The only new feature files are Private `ReferencePathTracerComposition.h/.cpp`; recursive Renderer CMake membership includes them. No public feature type or RHI/Scene/View/settings/history field was added. |
...
| Focused checks | Host-agnostic source audit passed; `FRAME-RPT-2` static topology/dependency audit passed; `cmake --build build --config DevelopmentEditor --target SparkleRenderer SparkleEditor -j 8` passed and rebuilt the three touched Renderer units; `cmake --build build --config DevelopmentGame --target SparkleRenderer -j 8` passed and compiled the same feature composition; `cmake --build build --config DevelopmentEditor --target architecture_boundary_check` passed; `powershell -NoProfile -ExecutionPolicy Bypass -File CMake/CodeStyle.ps1 -Mode Check` passed for 2,015 tracked owned files with clang-format 22.1.3; strict UTF-8/trailing-whitespace/local-link checks passed for the reconciled feature/readiness documents; `git diff --check` passed after this record was written. |
...
4. Keep only the feature-specific conversion from shared trace/material-hit representation into the accepted Reference-domain surface, environment/emission eligibility, finite-diagnostic termination, and invalid-result policy in the Reference namespace/folder. Canonical camera rays, inline trace queries/results, generic material-hit loading, and reusable path algebra belong to their existing common Renderer/RayTracing owners and must not be copied or feature-prefixed.
...
- Generic traversal, trace-result, hit-data, camera-ray, path-state, radiance/throughput, BSDF-sample vocabulary, and numeric code has one shared semantic owner; the feature capsule contains no copied `OpaqueTraceResult`, `TraceOpaque`, `ReferencePathTracerSemantic`, generic math implementation, or feature-prefixed replacement. Shared changes have current Reference and optimized consumers where semantics truly match and retain no Reference-only policy.
...
Use the established `PathSurface` and `RayTracingPathSample` contracts and one API-neutral shared `PathTracer` shader core for path state, radiance contribution, exact Lambertian sampling/evaluation/PDF, and direction-sample throughput algebra used by both the Reference pass and current optimized path consumers. Do not create Reference-specific surface, radiance-event, environment-evaluation, hit-decoding, or BSDF-sample wrappers. Keep Reference-only target, sample-stream identity and dimension ledger, selected admitted-domain adapter, finite-diagnostic termination, and invalid-result policy in the `ReferencePathTracer` folder; do not create `ReferencePathTracerSemantic`. Consume the canonical View camera directly, adding only a shared shader camera helper actually called by this pass; never introduce a Reference-specific camera type, copy, CPU ray mirror, or second matrix-to-ray implementation. Reuse narrow `CommonRandom` Philox4x32/open-interval mechanics while keeping the accepted Reference counter/key packing and dimensions feature-local, with no frame-index/TAA-jitter dependence or unused host mirror. Reuse shared path-surface construction and `SampleSkyRadiance` directly. Connect the pass as the Reference middle recipe of the original `BuildRenderFrameGraph`, invoked through one feature-local composition entry; it replaces the GBuffer/ReSTIR/reconstruction middle for that mode while retaining the shared frame shell and RHI execution. Keep traversal behind the current accepted adapter and output in memory for analytic checks. Preserve equation-to-code correspondence and delete duplicated helper authority. Do not add future ReSTIR PT/SHARC/cache/denoiser implementations, a parallel diagnostic shader, event stream, readback path, or debug surface.
...
**`PTD-03-R0 BLOCKED`** is retained against source input `709e04385c3d98aa3492fb3f09dc9caabe34cb3a` plus the current uncommitted Stage-3 working tree. The production source, both development-profile builds, both shader-target cooks, shared-capability clean break, static frame/enclosure checks, and temporary arithmetic oracle pass. The gate does not pass because no real GPU execution/readback of the raw transient radiance was compared with the frozen black/environment/emissive/one-/two-segment cases, and neither backend executed the controlled shader faults. Stage 4 is not authorized from compilation or a host-side arithmetic mirror.
...
| Shared-capability reconciliation | The feature-local `OpaqueTraceResult`, `TraceOpaque`, `ReferencePathTracerSemantic`, surface, BSDF-sample, radiance-event, emission/environment wrappers, hit decoding, Philox permutation, and open-interval conversion duplicate owners were deleted. `RayTracingTraceResult`, opaque inline query, alpha-aware material query, material-hit loader/count uniform, canonical camera-ray transform, established path surface/direction sample, static opaque Lambertian hit-to-path adapter, shared sky evaluation, reusable `PathTracer` state/radiance/throughput/Lambertian operations, and narrow `CommonRandom` counter-RNG mechanics now have non-feature shared owners. Current optimized `PathSampling` and `PathLighting` consume the same path contracts and operations. `ReferencePathTracer` retains only its choice of admitted adapter, key/counter/dimension identity, finite termination, and invalid-result policy. The temporary custom double-math include was deleted. No generic framework, registry, facade, CPU RNG mirror, or public feature API was added. |
| `FRAME-RPT-3` | `BuildRenderFrameGraph` still contains the sole outside-feature mode decision. Lit retains its GBuffer/ReSTIR/reconstruction middle. Reference creates its own raw radiance texture and dispatches its camera-path pass before its feature-owned exposure/provider-free upscale choice. Both routes retain the original frame admission, prepared Scene/View, RT-scene publication, graph execution, RHI submission, viewport publication, and presentation shell. No second renderer, executable, frame loop, submission route, Editor/Game fork, or composition on Lit output exists. |
...
| Required blocker | The existing viewport capture publishes the post-presentation product, not this transient raw radiance, and Stage 3 forbids adding a permanent diagnostic/readback route. No approved existing workflow currently exposes the raw texture for the named analytic fixtures. A temporary GPU fixture/readback may be used only if removed before handoff, or the later generic raw-result route must be brought forward by an explicit plan decision. The owner-directed binary32 amendment also requires its independent maximum-SPP/numeric review. Until both evidence gaps close, the Stage-3 estimator and precision exits remain unproved. |
| Next authorization | **Stage 4 is blocked.** Resume Stage 3 at the current source slice, execute the real raw GPU oracle without retaining diagnostic infrastructure, reconcile any defects, rerun the focused build/cook/enclosure/frame checks, and replace this record with one immutable candidate verdict. |

## Stage 4 - Complete The Included Surface-Transport Estimator

### Objective

Extend the shared path-tracing core only where new material/light operations are estimator-neutral, then complete the Reference policy for every accepted reflective event with one reviewed NEE/MIS estimator and compensated Russian roulette.
...
Extend `PathTracer` with only reusable metallic-roughness diffuse/specular sample-evaluate-PDF, event, radiance, and throughput semantics that have a current consumer or remove a real duplicate. Complete the Reference capsule's accepted analytic/emissive/environment light PMFs and measure conversions, connection visibility, NEE, MIS heuristic, emissive/environment hit weighting, delta/zero cases, and compensated Russian roulette over that core. SurfaceTransportReference must have no silent deterministic cap, clamp, filter, cache, biased MIP, or invalid-to-black path; a safety-depth reach fails the affected result. Keep FinitePathDiagnostic separately named. Run `CHK-RPT-19` before accepting any shared move; do not force Reference-only policy into the common layer merely to maximize reuse.
...
Deliver the first usable product milestone: a first-class Reference Path Tracer view mode for ordinary Editor and Game/runtime views that remains navigable, presents progressive accumulation live, explains resets and completion truthfully, and supports reliable Lit comparison. Editor places it immediately after Lit; a Game host uses its ordinary development view-mode selector over the same semantic. Saving and offscreen automation remain outside this stage.

### Work

1. Expose the Stage-1 `RenderViewMode::ReferencePathTracer` semantic through ordinary host view-mode selection: immediately after Lit in the Editor viewport menu and through the existing approved development selector for Game/runtime views. Both adapters submit the same per-view mode and connect through the ordinary viewport product boundary to the one feature-local session. Confirm no reference choice remains in global Lighting settings.
...
6. Exercise Editor and Game camera producers through the same canonical View path and the same Reference recipe/session. No UI, host kind, or input-device state becomes reset authority or Renderer truth.
7. Keep `ShippingEditor`, `ShippingGame`, and consumer first-run paths free of every feature producer/session factory and user/CLI/export route unless accepted release scope explicitly admits them. The existing development CVar may remain a thin ordinary mode-selection adapter for a host that has no view-mode UI, but it is not Renderer authority and may not create different behavior.
...
Expose RenderViewMode::ReferencePathTracer immediately after Lit in the existing Editor viewport menu and through the approved ordinary development selector of a Game/runtime host. Both are thin adapters to the same per-view semantic and already-integrated alternate recipe of the original frame; do not introduce an Editor renderer, runtime fork, new frame loop, render command, or presentation route at the UI stage. Confirm no global Lighting reference selector remains. Selection must preserve Lit settings, resolve the accepted mode preset, validate and start automatically. Implement compact state/progress/reset/completion presentation as a read-only projection in each approved host, plus actions, canonical Editor and Game camera behavior, exact Lit suspension/revalidation, explicit eviction/second-view capacity, pause/restart, and unavailable/failure presentation. Keep Renderer truth in the capsule, Shipping exposure gated, and Evidence/Output unavailable or explicitly deferred rather than faking it.

NON-NEGOTIABLE: implement UserExperience.md P0/P1 as the primary product, not a debug panel or mandatory render wizard. One selection starts a supported Editor or Game view through the same `RenderViewMode` contract. Camera controls remain responsive; the displayed composition follows the newest accepted identity within the frozen budget; exact canonical camera/radiance changes reset before commit; and refinement resumes automatically when motion stops. Input flags/tolerances and host kind are not estimator authority; TAA jitter/frame count/display/scheduling changes do not reset. Lit restores untouched settings and resumes only after full identity match. Progress is exact target-prefix progress, never convergence.
...
- One shared `PathTracer` core and one enclosed Reference policy are visible in the source/dependency audit; traversal adapters contain mechanism only, and optimized consumers do not fork identical core operations.
...
NON-NEGOTIABLE: ship only the scope proved by the accepted TransportAndEstimator.md revision and only the workflow proved by UserExperience.md. Every view-mode label/order, preset, overlay, reset reason, Editor/Game/runtime/offscreen result, manifest, artifact action, map comparison, and support message must preserve the bounded claim, host-agnostic Renderer semantic, and raw/preview distinction. A host/View-kind estimator fork, map outside the domain, unexplained invalid-result state, unresolved MATH-* defect, inaccessible first-use step, stale/mixed prefix, silent capability substitution, plausible partial artifact, or surviving competing selector/estimator authority blocks FCR-REN-08.

Complete the clean break: remove or unambiguously relabel all old GBuffer-seeded reference producer/history/settings/shaders/selectors/docs and remove temporary probes; audit source/header/shader/generated/CMake/package/SBOM/license/docs membership and unrelated dirty work. Retain `FRAME-RPT-10` and `CORE-RPT-10`; run CHK-RPT-01, 14, 15, 16, 17, 18, 19 plus every invalidated prior check, architecture_boundary_check, focused builds/runs required by the claims, and git diff --check. File FCR-REN-08 with exact PASS/BLOCKED/EXCLUDED evidence and limitations. Do not call the feature accepted unless every applicable AC-RPT-01 through 23 passes conjunctively and the acceptance owner approves the report.
```
...
This plan is complete only when the acceptance owner records `FCR-REN-08 PASS` against one immutable evidence set and every applicable `AC-RPT-01` through `23` passes. If scope, math, sampler, material/light semantics, compiler/shader identity, backend/frontend, accumulation, artifact schema, architecture hooks, frame-recipe topology, shared path-tracing core, or release content changes later, the completion report names the invalidated evidence and reruns the smallest affected checks, including `CHK-RPT-17` for any source-shape change, `CHK-RPT-18` for any recipe/resource-lineage change, and `CHK-RPT-19` for any path-family semantic or ownership change.
