...
### Stage-9 source result - 2026-09-17

Stage 9 is **IMPLEMENTED / VALIDATION DEFERRED** against source input `e783f2cfe2e6be205eeedd24ca7caf2206bc9047` plus the scoped Stage-9 working tree. The Reference committed raw mean/M2 now enter the existing generic viewport/RHI readback route; partial/checkpoint intents first settle the existing session so copied bytes cannot outrun their recorded prefix; ApplicationEditor separately owns artifact encoding/schema, the filesystem transaction, and optional secondary output actions; and `ShowcaseEditor --reference-path-tracer-request` enters the same ordinary Game-kind frame/session route after one typed request parse. The transport digest excludes host/viewport-owner identity, so equivalent Editor/runtime/offscreen inputs are not forked by their producer. The codec is private to non-Shipping ApplicationEditor configuration, and neither Renderer nor RHI owns filesystem or EXR policy.
