...
**Status:** **`PTD-00-R1 PASS`** remains the historical immutable discovery prerequisite; the 2026-09-13 repository-owner amendment replaces its Reference-shader binary64 policy with ordinary binary32 and must be included in the next independent numeric review; Stages 1-2 retain their historical frame/clean-break results; Stages 3-8 are **IMPLEMENTED / VALIDATION DEFERRED**; the Stage-6A target/show-flag decision is **SUPERSEDED** by the 2026-09-15 one-mode amendment, Stage 6C source is committed at `3b654728db3a35b4e2c859c8bf11116f4737800a`, and Stage 9 is authorized for source continuation under the deferred-validation policy; final reference/release acceptance remains separately gated by the retained GPU validation backlog, `REL-03`, release maps, support identities, and executable evidence
...
**Current readiness:** **50/100 (`45/5/0/0`)**. Stage 7 makes the existing mode selectable immediately after Lit and adds the minimum generic request/progress fields needed for the live session. Stage 8 adds strict requested/active Inline and Pipeline source routes over one estimator kernel, generic material trace/payload/hit mechanics, and the existing D3D12/Vulkan RHI path without adding RHI feature policy or a second renderer route. Game/runtime retains the same ordinary `ViewportRenderRequest` entry rather than a host fork. The candidate remains uncompiled, uncooked, and GPU-unexercised, so no lifecycle, response-budget, camera-interaction, accessibility, four-route parity, numeric, or reference-authority credit is added. See [Current Feature Readiness](../../../../../../../Acceptance/CurrentReadiness.md#renderer).
...
5. preserve Scene-owned scene data, View-owned view identity/camera data, one private Reference Path Tracer product/policy capsule, the existing small shared path-tracing/RayTracing/Lighting/Common shader owners, thin RHI traversal adapters, and the single-truth/copy budget. Preserve the original Sparkle frame shell: `FramePipeline::BuildRenderFrameGraph` selects the accepted request's `RenderViewMode::ReferencePathTracer` and schedules either Lit or Reference directly; never add a recipe/base interface, graph factory, dependency bag, Editor mirror enum, target/show-flag translation, graph-settings copy, process-global selector, RHI mode, second renderer/frame loop/submission path, or both estimator middles together;
...
- `RenderViewKind` identifies the ordinary View/camera producer (`Scene` for Editor-authored viewports, `Game` for runtime views); the request's `RenderViewMode` independently selects the Lit or Reference middle. Every supported Editor and Game view reaches the same feature entry, session semantics, estimator, and presentation path. Editor UI, Game UI, and offscreen requests select the same concrete Renderer semantic and may not fork Renderer behavior or use global CVars for normal selection.
...
3. Add one Private, feature-local owner below `Passes/Lighting/ReferencePathTracer`. The existing `FramePipeline` owns its lifetime and invokes it at one composition point. Do not create a second renderer, frame loop, scene database, public feature facade, recipe hierarchy, graph factory, or path-tracer mechanism on generic View state.
...
| Architecture | The Stage-1 Editor mirror/CVar command transport is superseded. `FramePipeline` still owns one Private `Passes/Lighting/ReferencePathTracer` lifetime inside the existing renderer/frame architecture. Stages 6B/6C establish one `RenderViewMode` request/View value with no Application translation, selector CVar, graph-settings copy, or RHI field. No public Reference session contract, feature-specific View mechanism, second renderer, or copied Scene/View truth remains. |
...
| Architecture fitness | The Private owner alone implements Reference behavior; `FramePipeline` contains one incomplete-type owner member, constructs it once, reads the accepted request mode at the direct graph-composition branch, and performs narrow update/bind/progress/submission calls. It contains no feature validation, reset classification, allocation, shader, sample, or error policy. No recipe/base interface, Lit wrapper, graph factory, dependency bag, selector CVar, or selector parameter chain remains. |
...
| `FramePipeline` | one composition/lifetime hook | One Private incomplete-type member, construction, direct mode branch, and narrow update/bind/progress/submission calls; no feature mechanism or parallel selector. Removing those sites leaves the Lit frame architecture unchanged. |
...
1. Preserve the original frame shell and sole Lit-versus-Reference branch in `FramePipeline::BuildRenderFrameGraph`; Stage 6C supplies the current `RenderViewMode` selector. The Reference branch does not yet claim usable radiance.
2. Keep `BuildRenderFrameGraph` declarative: shared resource/RT-scene setup, one explicit mode branch, and one shared downstream tail. Lit directly schedules its established GBuffer/lighting/exposure/reconstruction/upscaling stages; Reference calls one feature-local owner. Do not introduce `RenderFrameGraphRecipe`, a Lit wrapper class, a graph factory, dependency bag, registry, callback, or parallel selector.
...
- Lit and Reference resolve mutually exclusive middle setups inside the same `FramePipeline::BuildRenderFrameGraph`; the current sole selector is `RenderViewMode::ReferencePathTracer`, and no recipe hierarchy, graph factory, dependency bag, second renderer, frame loop, submission route, or duplicated Scene/View owner exists.
...
| Evidence field | Retained result |
| --- | --- |
| `FRAME-RPT-6` session and composition result | The one `FramePipeline`-owned Private feature entry consumes the resolved canonical `RenderView` and `PreparedRenderScene` after View/GPU-scene preparation. Inside the capsule, `ReferencePathTracerSession` owns the exact committed/target prefix, work cursor, pending graphics submission, automatic identity invalidation, Lit suspension/retention, and generic progress projection; `ReferencePathTracerIdentity` owns digest construction; `ReferencePathTracerResources` owns persistent accumulation allocation/binding/retention; and `ReferencePathTracerPasses` owns graph declaration and shader bindings. Manual pause/restart/cancel, timeout, reset-reason, discarded-prefix, route, and ETA machinery was removed because no current product action or consumer owns it; Stage 7 must add only the controls its live UX consumes. `ReferencePathTracer` composes the remaining owners. Stage 6 originally used the transitional CVar; Stage 6C now makes `FramePipeline::BuildRenderFrameGraph` select the one per-view `RenderViewMode` and directly schedule either focused Lit composition or the feature entry. Only Lit declares ReSTIR reservoir histories, while Reference uses its feature-owned accumulation. No recipe hierarchy, Lit wrapper class, graph factory, dependency bag, or selector parameter chain remains. FramePipeline otherwise performs construction, per-frame update/resource binding, generic progress publication, and submission notification. No session mechanism moved into View, Scene, graph settings, RHI, capture, providers, or frame history. |
...
| Responsibility refinement | The initial all-in-one C++ owner and monolithic transport entry shader were rejected during the mandatory end-of-stage source-shape pass. C++ graph pass declaration, transport identity, session scheduling/lifecycle/progress, and persistent GPU resource lifetime now live in separate feature-local owners. The compute entry now owns only dispatch-to-camera-sample-to-accumulator orchestration; feature-local sampler, Reference selection PMF/order, direct-light/MIS, transport-loop, and committed-display files own the independently changing Reference policies. Reusable scene-light counting/index resolution, BSDF/light/visibility/endpoint/hit/RNG/accumulation mechanics stay in existing shared path owners. The identity stores only equality components, not unused reason/diagnostic state or a redundant aggregate hash. Frame submission advances row/prefix state only after the graph actually executes. Forwarding-only feature controls, composition, recipe, Lit-wrapper, graph-factory, and per-feature dependency-bag surfaces were deleted. The original `FramePipeline::BuildRenderFrameGraph` owns the single direct CVar branch; Editor owns ordered view-mode presentation, Application translates it once, Renderer sequences the concrete controls and owns visualization/execution, and RHI capture carries no UI mode metadata. Generic viewport progress renders from product state rather than another feature check. No manager, registry, context bag, numbered fragment, helper namespace, diagnostic surface, or feature session type entered public engine contracts. |
...
2. Use the accepted request value for native-resolution provider/topology selection and the direct Lit-versus-Reference branch in `FramePipeline::BuildRenderFrameGraph`.
...
- Removing the enum row, direct branch, and private feature owner leaves the ordinary Lit frame coherent.
...
```text
Implement only Stage 6C after Stage 6B. Add RenderViewMode::ReferencePathTracer as value 1 and keep all values contiguous. Use the accepted request mode only for the necessary native-resolution/topology decision and direct Lit-versus-Reference branch; let the private feature read immutable RenderView::viewMode for lifecycle state. Delete CVarReferencePathTracer, ReferencePathTracerCVar.h, its registration, built selector cache, bool forwarding parameter, and every orphan source/document consumer. Keep the original FramePipeline/Scene/View/frame-graph/RHI/presentation shell, feature-local mechanism, and generic progress output. Do not add an Editor mirror enum, visualization target, mode-shaped show flag, selector alias, recipe hierarchy, settings bag, diagnostics, Show menu, capture schema, or RHI field. Run focused source checks, enclosure/generalization review, architecture_boundary_check, documentation checks, and git diff --check; record deferred executable checks honestly.
```
...
| Frame and lifecycle | `ResolveFrameGraphSettings` selects the native image pipeline for Reference, `BuildRenderFrameGraph` schedules exactly one middle, and the private feature reads the frozen View mode for lifecycle state. |
...
| Enclosure | Reference estimator, session, identity, resources, and passes remain private. Outside-feature source contacts are the generic enum row, existing FramePipeline lifetime/invocation hooks, focused consumers, and shader registration. RHI contains no mode or Reference feature identity. |
...
7. Keep `ShippingEditor`, `ShippingGame`, and consumer first-run paths free of every Reference mode producer/control, session factory, and user/CLI/export route unless accepted release scope explicitly admits them. A host without a view-mode UI submits the same per-view Renderer mode; no development selector CVar remains.
...
Stage 7 is **IMPLEMENTED / VALIDATION DEFERRED** in the scoped working tree based on `7fa195192e81101990d7a3f31057680c07717b9d`. The Editor's ordinary view-mode menu exposes `RenderViewMode::ReferencePathTracer` immediately after Lit. The existing host-neutral viewport request now carries only generic render actions; the generic progress product carries the state, exact prefix, reset/discard, retention, route, backend, throughput, and ETA needed by a real UI consumer. The private feature session owns all accumulation, identity, capability, suspension, capacity, and action policy. The Editor-specific labels and controls are enclosed in `ReferencePathTracerOverlay`, leaving `ViewportPanel` with one integration call. No selector CVar, show flag, mirror enum, Renderer settings bag, RHI selector, debug pass, event stream, logging route, export path, or offscreen renderer was introduced.
...
Stage 8 is **IMPLEMENTED / VALIDATION DEFERRED** against source input `e6b27e679369bc153a32d9d434b4e0a624d7c8f9` plus the scoped working tree. One shared Reference kernel is invoked by thin Inline and native Pipeline entry adapters. Generic `RayTracing*` owners now provide the common material trace signature, full-hit payload translation, miss/closest-hit/alpha-any-hit shaders, and requested/active execution-plan resolution reused by the ray-traced GBuffer, direct shadows where their visibility payload differs, and Reference transport. The existing pipeline composition, scene SBT, frame-graph `TraceRays`, and D3D12/Vulkan RHI mechanisms are reused without a Reference-owned RHI contract or second submission path.

Strict unavailable Inline or Pipeline requests resolve to no active route and fail through the existing capability state without fallback. Requested and active frontend plus backend are exposed separately; frontend changes participate in Reference session identity and graph topology. Static source-shape, shader registration/include, focused formatting, architecture-boundary, and whitespace checks are recorded in [Stage 8 Source Evidence](Stage8SourceEvidence.md). Focused builds, shader cooking, all four backend/frontend GPU routes, native validation, raw parity/statistical comparisons, live Editor/Game behavior, response timing, and TDR budgets remain deferred and are not `PASS`. Stage 9 is authorized only as source continuation; no backend/frontend parity or Reference-authority claim is earned.
