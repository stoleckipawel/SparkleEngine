...
| Checkpoint/artifact | Checkpoint is little-endian schema `rpt-checkpoint-1`: fixed 8-byte magic `SPKRPT01`; uint32 schemaBytes, width, height, samplerVersion, dimensionVersion; uint64 committedBegin, committedEnd; 32-byte session digest; then tightly packed row-major binary32 `mean.R/G/B`, binary32 `M2.R/G/B`, and uint32 count planes. The 32-byte header hash field is zero while hashing the complete fixed header and stores that SHA-256 afterward; each plane has uint64 byte size plus SHA-256 in a trailing directory. No padding or native structs are serialized. Raw OpenEXR is scanline, lossless ZIP FLOAT, and `beauty.exr` has only `R,G,B`. Stage 9 deliberately adds no AOV or event-debug artifact. Non-finite values fail publication. Counters/counts are separate canonical JSON decimal unsigned values. `manifest.json` uses RFC 8785 JSON Canonicalization Scheme, UTF-8 without BOM/newline, and is written last; every non-manifest file carries size/SHA-256, while the manifest digest is published in the terminal record rather than self-embedded. |
...
| --- | --- | --- | --- |
| `MATH-01` | `ViewCameraRay.hlsli::BuildPerspectiveViewCameraRay` consumes the existing `ViewCameraUniformData`, maps raster/filter coordinates to NDC, applies the canonical inverse matrices with ordinary HLSL binary32 operations, normalizes once, and derives the near/far interval. | `ReferencePathTracerKernel.hlsli` supplies Philox film dimensions `0/1` and consumes the returned origin, direction, and interval directly; there is no Reference camera type, duplicate ray generator, or custom double-math layer. | Perspective camera, box filter, full viewport only. |
...
| finite `MATH-02` | `RayTracingSceneTrace.hlsli::TraceSceneRay`, `RayTracingTraceResult.hlsli`, `RayTracingHitData.hlsli::TryLoadRayTracingHitTriangle`, `RayTracingHitUniformData`, `RayTracingPathSurface`, and `RayTracingHitPathSurface.hlsli::BuildStaticOpaqueLambertianPathSurface` are shared ray/path-tracing capabilities. The trace API is material-alpha aware by default and thin Inline/Pipeline adapters hide traversal mechanics; hit-surface data remains separate from estimator policy. | The Reference pass consumes the shared trace and surface contracts directly. It owns no duplicate traversal entry point, hit decoding, payload, or surface representation. | Lambertian plus emissive surface only; unsupported material/geometry becomes invalid, never plausible black. |
| finite `MATH-03` | `PathSurface.hlsli` and `RayTracingPathSample.hlsli` own the established surface and direction/BSDF sample contracts. `PathTracer.hlsli` owns API-neutral binary32 path state, exact cosine/Lambertian sampling/evaluation/PDF, radiance contribution, and sample-to-throughput state update. Existing optimized `PathSampling` and `PathLighting` consume the same contracts and operations. None contains Reference sample-stream, target, finite-depth, MIS, cache, ReSTIR, or denoiser policy. | `ReferencePathTracerKernel.hlsli` invokes the feature transport that calls `PathTracer::SampleLambertian` and `PathTracer::ApplyDirectionSample`; the Reference sampler supplies the exact induced selection mass and dimensions. | One active diffuse lobe with exact induced selection mass; no specular/delta/GGX. |
...
This audit covers committed estimator slice `5e6badf03df44133eb75db54875417821a9909c8`, committed surface slice `78a2598a54b9ab70f0e9bbb91aaa15aeb3a11c99`, committed correction slice `3b41476f04968e5affc1f5300efaa39252d6ee4a`, and the current scoped alignment follow-up. NVIDIA implementation sources and Epic product documentation are precedent and challenge inputs, not a vendor-conformance label or local correctness oracle. The full capability-to-stage disposition is retained in the [NVIDIA and Epic mid-delivery alignment](Research.md#nvidia-and-epic-mid-delivery-alignment--2026-09-13).
...
| Triangle ray endpoints | Re-derived from `NV-RAY-OFFSET`: published-blog operation ordering, inverse-transpose object normal, scalar extent, candidate constants, projected source bound, source-dependent far-end bound, and one-ULP `TMax` shortening. | NVIDIA states that intersection constants are hardware-specific. The blog and pinned companion HLSL disagree on one extent subexpression; Sparkle follows the blog formula and retains that conflict for Stage-8 numeric evidence. Current source is a candidate until the declared compiler/API/hardware matrix passes. |
| Progressive accumulation | Falcor `AccumulatePass` and RTXPT reset/sample-index behavior remain accepted precedents. | **Source-present / unverified:** Stage 6 implements binary32 Welford mean/M2 through shared `PathAccumulation.hlsli`, exact uint32 committed count, immutable committed baselines, row-quantized work, graphics-token commit, digest invalidation, and a separate committed display derivative. Higher-precision/extreme-count and GPU lifecycle evidence remains deferred. |
| Epic view-mode product | Adopts `REF-UE-PT-UX`: ordinary view-mode selection, automatic accumulation, camera/scene reset, exact target progress, and runtime-capable semantics. | These are Stage-6/7 requirements, not claims about the current shader slices. Sparkle requires complete canonical invalidation rather than reproducing Unreal's documented dynamic-content streaking gap. |
| Epic raw/display split | Adopts `REF-UE-PT-RAW` as a separation challenge: raw radiance, display transform, optional denoised result, and evidence output are distinct products. | Sparkle does not adopt Unreal's `Max Path Intensity`, default denoising, post-process controls, or buffer set as reference semantics. |
