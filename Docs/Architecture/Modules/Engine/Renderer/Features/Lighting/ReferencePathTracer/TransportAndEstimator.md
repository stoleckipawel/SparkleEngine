...
**Status:** Stage-0-frozen mathematical contract with a 2026-09-13 repository-owner precision amendment and Stage-3 source correspondence; `PTD-03-R0 BLOCKED` retains a minimal compiled/cooked candidate but no real raw GPU oracle result or estimator-authority claim
...
**Precision amendment:** the production Reference Path Tracer shader uses ordinary IEEE-754 binary32 throughout camera, transport, BSDF, PDF, MIS, roulette, ray-endpoint, radiance, and accumulation work. It does not declare or emulate shader float64. A higher-precision host oracle remains valid for error measurement, and the generic RHI/backend float64 capability remains available to unrelated future shaders; neither is a Reference Path Tracer dependency. This replaces the earlier binary64 shader/accumulator choice and requires the normal Stage-3 GPU/numeric evidence before the amended implementation can pass.
...
This working-tree binding records where the deliberately narrow `FinitePathDiagnostic(2)` candidate implements the accepted terms. It is not runtime, GPU, statistical, backend-parity, or reference-authority evidence; the Stage-3 gate remains blocked until the real raw output is exercised against the frozen cases.

| Contract | Shared mechanism | Reference-specific binding | Current limit |
| --- | --- | --- | --- |
| `MATH-01` | `ViewCameraRay.hlsli::BuildPerspectiveViewCameraRay` consumes the existing `ViewCameraUniformData`, maps raster/filter coordinates to NDC, applies the canonical inverse matrices with ordinary HLSL binary32 operations, normalizes once, and derives the near/far interval. | `ReferencePathTracer.hlsl` supplies Philox film dimensions `0/1` and consumes the returned origin, direction, and interval directly; there is no Reference camera type, CPU ray mirror, or custom double-math layer. | Perspective camera, box filter, full viewport only. |
