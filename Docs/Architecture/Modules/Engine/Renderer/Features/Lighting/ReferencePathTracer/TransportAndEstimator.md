...
| Arithmetic | Camera, semantic path state, BSDF/light/PDF/MIS/roulette, endpoint bounds, radiance, and accumulation use ordinary IEEE-754 binary32. Traversal consumes those values directly; the Reference shader neither requests float64 nor carries a custom double-math emulation layer. Surface origins use the accepted binary32 error bound and directed `nextafter32` rounding. An infinite ray uses the binary32 unit direction, `TMin=0`, and `TMax=+Inf`. A finite connection computes binary32 `omega=normalize(y-x)`, independently rounds the two endpoints outward to `p0,p1`, computes binary32 `df=p1-p0`, and traverses `p0+t*df` over `TMin=0`, `TMax=nextafter32(1,-Inf)`; semantic BSDF/Jacobian factors use the original unoffset binary32 `(x,y,omega)`. Zero/non-finite `df` is invalid. Integer products use explicitly sized unsigned arithmetic and checked overflow. The exact compiler mode and shader hash remain evidence identity, while higher-precision host math is an oracle only. |
...
An infinite continuation/environment ray uses `TMin=0` from the binary32 source rounded along its outgoing direction. A finite connection first forms binary32 `omega=normalize(y-x)` from unoffset reconstructed receiver `x` to unoffset emitter point `y`; BSDF evaluation, cosines, distance-squared/Jacobian, and emitted radiance use this original unoffset `(x,y,omega)`. It rounds source `p0` from `x` along `omega` and light endpoint `p1` from `y` along `-omega`, computes unnormalized binary32 `df=p1-p0`, and traces `p0+t*df` on `[0,nextafter32(1,-Inf)]`. No separately normalized traversal direction or arbitrary offset distance may replace the semantic radiometric inputs. A zero/non-finite direction or collapsed interval is invalid. Primary rays use the binary32 camera interval in `MATH-01`. The constants and operation ordering are part of shader/compiler identity. Stage 5 must re-derive and numerically validate their conservatism for Sparkle's actual triangle formats, fused-operation settings, row-vector packing, Inline/RGS implementations, and D3D12/Vulkan compilers; a failing route is excluded rather than tuned per scene.
...
| Alpha, sides, normals, and emission | `RayTracingMaterialAlpha.hlsli` evaluates deterministic coverage from the canonical undeformed UV0, vertex color, material, and texture records without pulling deformation or previous-frame buffers into visibility-only consumers. `RayTracingEvaluatedTriangle.hlsli` is the one current morph/skin evaluator for hit and emissive-triangle geometry, with previous-snapshot evaluation invoked only by the GBuffer motion-vector consumer. `RayTracingMaterialHit.hlsli` uses traversal front-face plus two-sided material state, preserves separate geometric and shading normals, and rejects an opposite-hemisphere mapped normal rather than altering it. | Reference traces through the shared alpha-aware query, converts the canonical hit surface to `RayTracingPathSurface`, uses geometric normals for visibility/endpoints and shading normals only through the shared BSDF, and weights textured emission through the existing MIS route. | Alpha/sidedness/mirror/normal-map/deformation and emission sampling-versus-hit GPU cases remain unrun. Excluded blend, transmission, subsurface transport, media, and animation-time integration still require Stage-6 preflight rejection before sample zero. |
| `MATH-11` | `RayEndpoints.hlsli` owns `c0=2^-24`, `c1=3*2^-24`, `c2=2*2^-24`; prescribed `v0 + b.x*e1 + b.y*e2` triangle extent; object reconstruction, row-vector object/world and world/object traversal error; geometric-normal projected offset; directed binary32 rounding; canonical camera interval; infinite continuation; and source-plus-target shortened finite connection rays with unnormalized traversal direction and `TMax=nextafter32(1,-Inf)`. `MeshInstanceData` publishes world, inverse-world, and inverse-transpose matrices from the same prepared instance. | Reference passes the shared surface position/geometric normal/error bound to shared continuation and visibility operations. It owns no epsilon, bias, grazing multiplier, `MinT`, or distance cutoff. The optimized path consumer now calls the same continuation operation and its former bias/distance CVar authority is removed. | Re-derivation against actual BLAS vertex formats and D3D12/Vulkan compiler behavior plus the full scale/translation/rotation/nonuniform-scale/shear/mirror/grazing/coplanar/thin-gap matrix remains mandatory. |
