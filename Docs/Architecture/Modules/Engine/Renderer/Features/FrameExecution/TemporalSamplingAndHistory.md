...
The source alternatives must not be advertised as user-selectable anti-aliasing modes. The current product path is the Halton sequence only. [Resolution, Sampling, and Anti-Aliasing](../PostProcessing/ReconstructionAndGeneration/ResolutionSamplingAndAntiAliasing.md) owns how this sampling identity combines with render/output extents, provider resolution, active single-sample attachments, and absent AA modes.
