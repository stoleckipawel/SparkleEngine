...
| Tone mapping/output | `r.ToneMapper`, `r.OutputColorEncoding` | ACES approximation; Automatic encoding | resolved view, [Tone Mapping](../PostProcessing/DisplayPipeline/ToneMapping.md), and [Presentation/Output](../PostProcessing/DisplayPipeline/PresentationAndOutput.md) passes; graph parameters update per frame |
| Exposure | `r.Exposure.Mode`, `r.Exposure.MeteringMethod`, `r.Exposure.Manual`, `r.Exposure.Compensation`, `r.Exposure.TargetLuminance`, `r.Exposure.Min`, `r.Exposure.Max`, `r.Exposure.AdaptationSpeedUp`, `r.Exposure.AdaptationSpeedDown` | Automatic, ParallelReduction, 1.0, 0 EV, 0.18, 0.000001, 65536, 3 EV/s, 1 EV/s | view display resolution and [Exposure](../PostProcessing/DisplayPipeline/Exposure.md) pass/history |
...
| `r.PathTracing.Execution` | Automatic, Inline, Pipeline | shared execution-plan resolver and Reference frame-graph topology; Automatic prefers Inline then Pipeline | console surface for traversal implementation policy; not a view-mode selector and not mirrored by `EngineRenderingSettingsState` | [Reference Path Tracer](../Lighting/ReferencePathTracer/README.md) and [Ray Tracing](../RayTracing/README.md) |
...
| `r.Material.BindingMode` | RayTracingOnly; also registers Everything | no runtime consumer found; capability report supports RayTracingOnly only | ineffective registered selector; `Everything` is not a feature | [Geometry/Materials](../GeometryAndResources/GeometryMaterialsAndGBuffer.md#material-contract) |
...
| HDR display output | no HDR request, supported/active/fallback state, PQ/display-nit policy, or HDR swapchain selection exists; `DSP-7` must add it | [HDR Display Output](../PostProcessing/DisplayPipeline/HDRDisplayOutput/README.md); `REN-E34` preserves current absence truth until `FCR-REN-26` is implemented |
