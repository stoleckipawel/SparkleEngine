...
| `FRAME-11` Produce image | The active graph builds/updates the scene TLAS, writes a raster or ray GBuffer, computes Direct and Indirect surface lighting through ReSTIR, composites/sky-fills scene color, meters exposure, optionally reconstructs/upscales, optionally visualizes a debug quantity, tone maps, encodes, and copies to the back buffer or exports a viewport product. `RenderViewMode::ReferencePathTracer` currently reports unavailable and contributes no transport pass. There is no Volumetric Lighting, Color Grading, Chromatic Aberration, or Frame Generation stage. | Scene-linear surface/lighting data -> encoded final color | Every implemented feature branch rejoins one presentation boundary, while absent domains remain visible rather than implied. | [Lighting](Features/Lighting/README.md) and [Post Processing](Features/PostProcessing/README.md) |
...
`GBuffer`, `RestirLighting`, and `Exposure` demonstrate the intended local shape: the graph or stage aggregator calls one responsibility-bearing function while private collaborators remain beside that feature. This is precedent, not automatic proof that every existing file is ideal. Every new or materially changed feature retains a per-stage integration-hook ledger and fails architecture review when feature-specific state leaks into generic Scene, View, history, settings, RHI, UI, or frame-resource owners without a separately proved shared contract.

The Reference Path Tracer target is explicitly **one frame with an alternate middle recipe**. `BuildRenderFrameGraph` remains the single high-level composer: Lit builds GBuffer, ReSTIR lighting, and reconstruction; Reference Path Tracer instead invokes one feature-local graph entry that builds independent camera transport, raw accumulation, and a display resolve. Both recipes retain the existing `FramePipeline`, prepared Scene/View, ray-tracing-scene publication, frame-graph compiler/executor, RHI submission, viewport products, UI packets, and presentation tail. The feature is neither a second renderer nor a debug/post-process layer over Lit.
...
| Surface lighting | ReSTIR path-traced lighting is the only current producer; `RenderViewMode::ReferencePathTracer` is a per-view contract that rejects as unavailable until its specialized route is built in this same frame architecture | Ordinary lighting produces Direct and Indirect surface-lighting lobes and requires ray traversal. The reference contract does not allocate, dispatch, accumulate, or lend authority to Lit output. There is no non-ray deferred-lighting branch. |
...
| --- | --- | --- | --- |
| Scene color | `R16G16B16A16_Float`, render extent | ReSTIR composite/sky; the Reference Path Tracer route is not implemented in Stage 1 | upscaling, debug, tone mapping |
