...
| `FRAME-07` Build view | Build camera matrices, frustum, viewport/scissor, display settings, temporal uniform, and the current Renderer visualization scalar required by shaders. Parallel visibility classifies/culls primitives, then builds raster batches, workload metrics, and a per-view ray-tracing partition plan. Editor view-mode identity and Renderer feature/visualization selectors are not retained in the View. | `RenderViewInput` + prepared scene -> frame-slot `RenderView` | Camera, visibility, output, and history are view-owned; frontend UI state and Renderer feature policy stay with their actual owners. | [View Preparation](Features/SceneAndViewPreparation/ViewPreparation.md) and [Temporal Sampling](Features/FrameExecution/TemporalSamplingAndHistory.md) |
...
The Reference Path Tracer target is explicitly **one frame with an alternate middle recipe**. `FramePipeline::BuildRenderFrameGraph` reads `r.ReferencePathTracer` at the one composition branch: Lit builds GBuffer, ReSTIR lighting, and reconstruction; Reference Path Tracer builds independent camera transport, raw accumulation, and a display resolve. No recipe hierarchy or graph factory carries feature objects. Both branches retain the existing prepared Scene/View, ray-tracing-scene publication, frame-graph compiler/executor, RHI submission, viewport products, UI packets, and presentation tail. The feature is neither a second renderer nor a debug/post-process layer over Lit.
