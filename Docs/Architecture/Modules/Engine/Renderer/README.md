...
Current-state basis: source and build configuration inspected 2026-09-06 at committed `master` revision `8414b5dc`; no build, runtime, GPU-validation, visual, performance, or package result is claimed
...
| Frame graph and GPU scheduling | Declares typed raster/compute/ray/transfer/provider passes, compiles resource dependencies, barriers, transients, queues, recording chunks, and submission batches. | Source path present; native validation, overlap, aliasing, and rebuild-cost evidence remain open. | [Frame Graph and Scheduling](Features/FrameGraphAndScheduling.md) |
| Scene and view preparation | Maintains a persistent render-side scene, resolves per-frame geometry/deformation/lights, builds camera/temporal state, culls and batches raster work, and publishes GPU-scene bindings. | Source path present; capacity, failure, history, and concurrency behavior need executable evidence. | [Scene and View Preparation](Features/SceneAndViewPreparation.md) |
| Geometry, materials, and GBuffer | Renders static, instanced, skinned, and morphed triangle geometry through raster or ray GBuffer frontends into one deferred material contract. | Opaque and alpha-tested paths exist; transparent blending and broader material lobes are not supported. | [Geometry, Materials, and GBuffer](Features/GeometryMaterialsAndGBuffer.md) |
...
| Exposure | Resolves manual or automatic metering into one history-aware per-view multiplier before debug replacement and presentation. | Source path present; numerical response, reset, finite bounds, and async overlap remain unproved. | [Exposure](Features/PostProcessing/Exposure.md) |
| Image reconstruction and upscaling | Produces one output-extent resolved color through Linear, NVIDIA DLSS Super Resolution, or ReSTIR-specific DLSS Ray Reconstruction. | Linear baseline exists; NVIDIA routes are capability/backend/package gated and unproved. | [Image Reconstruction and Upscaling](Features/PostProcessing/ImageReconstructionAndUpscaling.md) |
| Tone mapping | Multiplies by exposure and maps HDR scene-referred color to display-linear color through Reinhard, ACES approximation, or ACES fitted filmic. | Source path present; no public bypass and numerical/colorimetric correctness remains unproved. | [Tone Mapping](Features/PostProcessing/ToneMapping.md) |
| Color grading | Would own authored grading transforms, parameters, and LUT workflows independently from tone mapping. | Not implemented; fixed tone-mapper choices are not color grading. | [Color Grading](Features/PostProcessing/ColorGrading.md) |
| Chromatic aberration | Would own intentional channel-dependent lens distortion and its placement/sampling policy. | Not implemented; reconstruction or filtering fringes are not feature support. | [Chromatic Aberration](Features/PostProcessing/ChromaticAberration.md) |
| Frame generation | Would own interpolated-frame synthesis, identity, pacing, UI, latency, and presentation contracts. | Not implemented; Streamline Reflex/PCL, DLSS Super Resolution, and Ray Reconstruction are not frame generation. | [Frame Generation](Features/PostProcessing/FrameGeneration.md) |
| Presentation and output | Applies output encoding and back-buffer or viewport-product publication after tone mapping. | HDR display output is absent; numerical color and exact debug presentation remain unproved. | [Presentation and Output](Features/PostProcessing/PresentationAndOutput.md) |
| UI and editor viewport composition | Replays immutable UI packets as a host overlay or presents an offscreen viewport product through an ImGui texture binding after scene rendering. | Source path present; blend/color/DPI behavior and long-session editor-texture lifetime are unproved. | [UI and Viewport Composition](Features/UiAndViewportComposition.md) |
...
| Diagnostics, viewport products, and capture | Collects frame/pass timing and memory data, publishes editor-facing products, and completes asynchronous texture readbacks. | Source path present; truthfulness, color/format semantics, observer cost, and support UX remain unproved. | [Diagnostics and Capture](Features/DiagnosticsAndCapture.md) |
| Shader programs and reload | Materializes typed cooked compute, graphics, and ray-tracing programs as a whole generation and retires replaced runtime state after queue completion. | Source path present; cook/runtime/backend parity and reload failure evidence remain open. | [Shader Program Catalog](Features/ShaderPrograms.md) and [Shader Compilation](../../Tools/ShaderCompiler/README.md) |
...
The [Feature Selector Catalog](Features/FeatureSelectors.md) maps the full current CVar/settings surface to active owners and calls out registered-but-ineffective or non-persisted controls.
...
| Which setting/CVar selects a feature, and is it actually consumed or persisted? | [Feature Selector Catalog](Features/FeatureSelectors.md) |
