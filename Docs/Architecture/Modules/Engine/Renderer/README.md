...
`RendererExecutionContext` owns the render-thread execution lifetime and coordinates only `RendererHost` plus `FramePipeline`. `RendererHost` owns the backend, pipeline-runtime cache, and memory monitor and is the sole factory for `FramePipeline`; it does not publish those services as a getter bag. `FramePipeline` owns the mesh/texture caches, persistent RenderScene, Scene/View preparation, View state, image providers, capture, UI-frame integration, and frame features that share its lifetime. Serial and threaded operation execute the same typed renderer controls, while the threaded mailbox keeps its coordinator-only frame-ready command outside `RendererExecutionContext`.

Once preparation has frozen a frame, downstream execution and feature owners consume the canonical `RenderFrame`. They may inspect its immutable identity, Scene, View, time, and ray-tracing bindings, but orchestration does not unpack those fields into parallel argument lists or carry the complete mutable viewport request beside the accepted View. The View is the per-frame authority for accepted viewport identity, mode, camera, extents, display state, and temporal state; a feature may additionally receive only the focused one-shot control edge it consumes.

The executable architecture check rejects external includes of `Renderer/Private`, Public-to-Private dependency leaks, reintroduction of known implementation-only header families under `Renderer/Public`, FramePipeline construction outside `RendererHost`, execution-context reach-through into subsystem implementations, coordinator mailbox commands entering the execution context, Scene/View/cache/provider ownership drifting back into the host, host service-locator getters, and decomposition of `RenderFrame` at the frame-execution boundary.
