...
| `REN-LGT-04` | Reference Path Tracer | Source-present, executable proof pending | `r.ReferencePathTracer` selects one feature-local GPU path-tracing middle directly in `FramePipeline::BuildRenderFrameGraph`. Session, identity, resources, passes, shaders, and accumulation remain in the Private feature capsule; Editor owns its optional ordered view-mode mapping. No build, runtime, GPU, convergence, numerical, or release evidence is inferred from source presence. | `B+S` | Pending remaining staged acceptance |
...
The public Renderer `Visualization` CVar domain contains exactly 16 final/debug values. Reference Path Tracer is a separate Renderer feature selector, while Editor alone owns its user-facing view-mode entry and ordering:
