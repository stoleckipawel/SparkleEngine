...
| `REN-LGT-04` | Reference Path Tracer | Source-present, executable proof pending | Per-view `RenderViewMode::ReferencePathTracer` selects one feature-local GPU path-tracing middle directly in `FramePipeline::BuildRenderFrameGraph`. Session, identity, resources, passes, shaders, and accumulation remain in the Private feature capsule; Editor owns only its menu presentation. No build, runtime, GPU, convergence, numerical, or release evidence is inferred from source presence. | `B+S` | Pending remaining staged acceptance |
...
The public Renderer `RenderViewMode` domain contains Lit, Reference Path Tracer, Wireframe, and 14 focused debug products/channels. It is the sole per-view mode authority. Editor owns labels, icons, grouping, and interaction but uses the same rendering semantic directly:

| Capability ID | Category | Modes | Current boundary |
| --- | --- | --- | --- |
| `REN-DBG-01` | Final/material | Lit, Wireframe | Lit selects ordinary rendering; Wireframe is consumed as raster-frontend state from the same immutable mode. |
...
| Capability ID | Capability | State | Exact current coverage and limit | Evidence | Release disposition |
| --- | --- | --- | --- | --- | --- |
| `REN-SET-01` | Aggregate public settings state | Implemented path | A 26-field value snapshot spans presentation/device, tone/output, exposure, reconstruction/upscaling, GBuffer/RT, batching, and TLAS/PTLAS; it records requested values, not resolved provider/capability state. Per-viewport view mode is deliberately outside this aggregate. | `S` | Pending |
| `REN-SET-02` | Application-owned rendering-settings persistence | Partial | Application loads and rewrites twenty-six CVar names in one owned section in workspace `Config/DefaultEngine.ini`; Renderer contains no filesystem policy and view mode is session-owned. Writes truncate in place and return no error, so durability/concurrent edit/package behavior is not established. | `S` | Pending |
...
The inspected public settings cover VSync, back-buffer format, adapter preference, tone mapper, exposure, output encoding, upscaler and quality, ray reconstruction, GBuffer algorithm, automatic mesh batching, classic TLAS refit, and partitioned TLAS activation/planning controls. Ray-tracing frontend selection is one automatic Renderer policy with no public or per-effect setting. View mode is non-persisted per-viewport state carried as one `RenderViewMode` on `ViewportRenderRequest`; it is not an engine rendering setting or a second lighting-route type. `r.BackBufferCount` and `r.MaximumFramesInFlight` are RHI console controls outside the Renderer settings state/persistence. `r.Material.BindingMode` is registered but has no inspected runtime consumer; its `Everything` value is vocabulary, not an active selectable result.

The settings section persists 26 named Renderer CVars in `/Script/SparkleRenderer.EngineRenderingSettings` inside workspace `Config/DefaultEngine.ini`, replacing that section while retaining other sections. View mode is session state and is not in that persisted set. Adapter preference and back-buffer format are written immediately but reported as pending restart; the other section values are applied to Renderer CVars on commit. The full lifecycle and known in-place-write/diagnostic gaps are owned by [Settings State and Persistence](Features/RuntimeConfiguration/SettingsStateAndPersistence.md).
...
- Current source contains the scene - referred HDR versus display
    - linear exact presentation split;
executable pixel, extent, backend, and capture evidence remains pending.
