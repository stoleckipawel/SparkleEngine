...
| `REN-SET-02` | Owned settings persistence | Partial | Twenty-six CVar names are loaded from and rewrite one owned section in workspace `Config/DefaultEngine.ini`; view mode is session-owned and not persisted here. Writes truncate in place and return no error, so durability/concurrent edit/package behavior is not established. | `S` | Pending |
