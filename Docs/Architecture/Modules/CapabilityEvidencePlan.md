...
**Inventory authority:** [Current Capability Inventory](README.md)
...
**Current tracked readiness:** **40/100** across 49 `FCR-*` families; every verification and delivery component is still zero. This evidence queue adds no credit until candidate-bound results are retained. See [Current Feature Readiness](../../Acceptance/CurrentReadiness.md).

**Release-wide sequencing authority:** [Release-First Principal Graphics Roadmap](../../Strategy/Roadmap.md)

**Release implementation phases:** [First Release Implementation Plan](../CrossModule/FirstRelease/README.md) selects when an evidence item is needed and which feature phase consumes it
...
| `INV-004` | Core and Tasks source capabilities are understood. | Reconcile [Core](Engine/Core/README.md) and [Tasks](Engine/Tasks/README.md) against live public headers/CMake after relevant changes. | Concurrency or lifetime claims trigger focused stress evidence. | Source audit complete; executable evidence open below. |
| `INV-005` | Platform and Application source capabilities are understood. | Reconcile [Platform](Engine/Platform/README.md) and [Application](Engine/Application/README.md) against live owners after relevant changes. | Package or privilege-sensitive paths move to clean-machine/package gates. | Source audit complete; executable evidence open below. |
| `INV-006` | GameFramework/world and Editor source capabilities are understood. | Reconcile [GameFramework](Engine/GameFramework/README.md) and [Editor](Engine/Editor/README.md) after world/editor contract changes. | Any data-owner ambiguity triggers architecture reconciliation before evidence. | Source audit complete; executable evidence open below. |
| `INV-007` | Assets, import, and cooking source capabilities are understood. | Reconcile [Engine Assets](Engine/Assets/README.md), [Importers](Tools/SourceImporters/README.md), and [Cooking](Tools/Cooking/README.md) after format/product changes. | Each advertised format/content family receives a clean import/cook/runtime fixture. | Source audit complete; executable evidence open below. |
| `INV-008` | Launcher, Showcase, build, and package source capabilities are understood. | Reconcile [Launcher](Tools/Launcher/README.md), [Showcase](Projects/Showcase/README.md), and [Build](BuildAndPackaging/README.md) after workflow/product changes. | Any route dependent on workspace/private state moves to package correction before release proof. | Source audit complete; formal package path absent and evidence open below. |
...
| `RHI-E09` | Inline and native-pipeline effects are semantically equivalent. | On one D3D12 and one Vulkan RT-capable adapter, compare ray GBuffer and direct shadows across automatically selected routes; use a temporary focused capability harness removed before handoff when the non-selected adapter needs direct exercise. | Mismatch creates effect-specific defects; source presence or one automatically selected route cannot establish adapter parity. | Open |
...
| `REN-E04` | Ray GBuffer matches raster semantics where both apply. | Compare raw GBuffer attachments for the same controlled matrix in Raster and every automatically selected frontend; use a temporary focused capability harness where needed to exercise the non-selected adapter. | Differences must be classified as intended or fixed before frontend inclusion. | Open |
...
| `REN-E25` | Deferred decals remain visibly absent until their admitted feature contract is implemented and proved. | Audit authoring/import/cook, scene/GPU data, selectors, frame-graph passes, shader registrations, GBuffer mutation, secondary-ray material evaluation, editor UX, and release claims; then execute `AC/FM/CHK-DECAL-*` through `GR-5`. | A partial, stale, raster-only, or silent fallback blocks `FCR-REN-23` and release advertisement. | Open; first-release target currently absent |
| `REN-E26` | Color grading remains explicit absence until the admitted transform/control/LUT contract is implemented and proved. | Audit current surfaces, then execute `AC/FM/CHK-CGR-*` through `DSP-5`, including controls, LUT assets/cook, color domain/order, view state, captures, package, and both backends. | A tone curve may not stand in for grading; partial or double-applied grading blocks `FCR-REN-24`. | Open; first-release target currently absent |
| `REN-E27` | Chromatic aberration remains explicit absence until the admitted bounded lens-effect contract is implemented and proved. | Audit current surfaces, then execute `AC/FM/CHK-CHR-*` through `DSP-6`, including zero identity, resolution scaling, edges/alpha, view/debug/UI order, package, and both backends. | Accidental fringing or an unbounded/incorrectly ordered effect is not support and blocks `FCR-REN-25`. | Open; first-release target currently absent |
...
| `REN-E34` | HDR10 output remains visibly absent until the admitted Renderer transform and RHI presentation tuple are implemented and proved. | Audit settings, output transforms, shaders, frame graph, UI, RHI format/color-space/metadata calls, swapchain transitions, captures, package surfaces, and claims; after implementation execute `CHK-HDR-*` on admitted HDR hardware and both backends. | A 10-bit/Linear surface, metadata-only path, false active state, missing SDR fallback, double transform, stale monitor transition, or unproved backend/hardware cell blocks `FCR-REN-26`. | Open; first-release target currently absent |
...
These rows track the explicit absent/target [Neural Graphics](../CrossModule/NeuralGraphics/README.md) family. They do not authorize feature work ahead of the Roadmap and cannot be executed until one bounded neural graphics problem is admitted.
...
The [inventory evidence marks](README.md#evidence-state) describe the source snapshot; the [acceptance contract](../../Acceptance/FirstRelease.md) owns release evidence meaning and approval. Executed results belong in the applicable `FCR-*` candidate report with its iteration traceability, risk/AC/FM/check ledgers, and artifacts. This plan only identifies the next claim-falsifying check.

Evidence may close one row without promoting neighboring rows. For example, a successful D3D12 inline ray-GBuffer run does not prove Vulkan, native pipeline, direct shadows, the Reference Path Tracer, ReSTIR indirect, partitioned TLAS, or performance.
