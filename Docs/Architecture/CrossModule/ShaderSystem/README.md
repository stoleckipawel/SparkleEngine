...
- Two directories can contain the same source basename. Sparkle's former basename-derived fallback could not distinguish them; the [migration baseline](MigrationBaseline.md) preserves that historical finding.
...
- The panel reads one immutable shader-operation/catalog model and submits typed requests to the shader-owned coordinator; it never scans artifact directories, invokes the compiler, mutates the active shader map, creates RHI objects, or enters another feature's operation owner. The coordinator may share the private Application task lifetime/slot mechanism, but shader request, execution, and result policy stay shader-owned.
...
Committed source uses one transactionally published `GlobalShaderMap.smap` plus one content-addressed `CookedShaderLibrary.slib`; the clean break deleted the former one-`.sparkshader`-per-package readers, writers, registry, and authored identity. Physical grouping remains generated cooker policy and is invisible to Renderer callers. Any later regrouping requires startup I/O, file-open, compression, or patching evidence and must regenerate the one current representation in place.
...
The source-backed Unreal, NVIDIA, and AMD comparisons behind these decisions live in [Shader System Design Precedent](DesignPrecedent.md). That research explains the alternatives; this document owns only the selected Sparkle design.
...
The fifth column in the next four design-space tables is non-authoritative source context retained to explain the selected target. It is not a uniformly current inventory. Use the [Shader Compilation Capability Inventory](../../Modules/Tools/ShaderCompiler/README.md) for the dated current-source shape and the [migration baseline](MigrationBaseline.md) for historical checkpoints.
...
PSO prewarming, preload controls, native cache integration, and speculative preparation remain unselected. [Shader System Design Precedent](DesignPrecedent.md#deferred-pso-prewarming-research) preserves the compared options and the evidence required before reopening that decision.
...
Current committed source carries RT exports, hit groups, payload/attribute/recursion metadata, and local-record contracts in the final global-map/library representation. It also contains typed Renderer RT shader classes and compositions, D3D12 state-object and Vulkan RT-pipeline creation, identifier/group-handle retrieval, neutral checked shader-table packing, typed frame-graph trace dispatch, generation lifetime, dual GBuffer/shadow frontends, alpha any-hit, and the shared Surface scene-table plan. That is source reachability, not paired executable conformance. Portable shader-visible nonzero local records, intersection/callable execution, native correctness, parity, reload/retirement, and performance remain unproved under the [feature acceptance contract](Acceptance.md). Precache telemetry is outside the accepted feature scope.
...
These sources supplement the detailed comparison in [Shader System Design Precedent](DesignPrecedent.md#external-precedent-and-what-sparkle-adopts):
...
There is no authored or generated `ShaderProgramDesc`, `ShaderProgramId`, `TShaderProgram`, or mandatory program manifest in the architecture. Graphics pipeline stage composition is the typed draw request. Ray tracing alone uses `RayTracingPipelineComposition` because a native pipeline must name exports and hit groups. Current source has deleted `.sparkshader` package readers, writers, cache, and path derivation; it publishes raster, compute, and RT stages through the same map/library representation with their native and graph consumers. No compatibility container or compiler-only replacement remains.
...
Report language/backend/target/stage/feature support as supported, unsupported, compiler-only, or runtime-validated. No backend silently ignores compile policy. Inline ray-query compute shaders use the normal compute class/map/dispatch route. Acceleration-structure storage and native descriptor representation are selected below Renderer shader/effect code: one semantic parameter and one shader code record cover classic and partitioned TLAS, and provider readiness is false unless the backend can bind that parameter through its exact native descriptor contract. Ray-generation, miss, hit, intersection, and callable declarations may ship only with typed composition, native pipelines, SBTs, graph trace dispatch, generation lifetime, and paired conformance; there is no accepted compiler-only RT checkpoint.
