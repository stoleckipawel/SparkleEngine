...
- Move the remaining shader sources from `Engine/Assets/Shaders/Passes/Deferred` to `Passes/GBuffer`, `Passes/Lighting/...`, `Passes/PostProcessing`, `Passes/Presentation`, `Passes/RayTracing`, and `Passes/Visualization` owners as applicable.
...
- [Ray-tracing target shader and composition contract](../../Modules/Engine/Renderer/Features/RayTracing/ExecutionArchitecture.md#shader-authoring-and-pipeline-composition)
...
> Implement Phase 7 as one product-effect vertical-slice CL directly in the unstaged `master` worktree. Apply the [common phase delivery contract](#common-phase-delivery-contract), including shared-semantics ownership proof, paired parity/alternate-path oracle coverage, and the mandatory source-only Code Review gate. Refactor the existing inline ray-traced GBuffer into shared semantic HLSL plus thin inline and pipeline frontends, use the one automatic engine-wide frontend resolver, and schedule exactly one typed frontend. Update the Phase 12 same-frame parity and explicit-raster validation route, but do not configure, build, compile shaders, cook, launch, run tests, capture, or collect performance evidence. Use the Phase 6 map, pipeline, table, graph, and lifetime owners directly; add no second scene/material/output or execution system.

#### Phase-specific references

- [Ray-tracing target dual-execution contract](../../Modules/Engine/Renderer/Features/RayTracing/ExecutionArchitecture.md#effect-level-dual-execution-contract)
- [Ray-tracing target shader/SBT contract](../../Modules/Engine/Renderer/Features/RayTracing/ExecutionArchitecture.md#pipeline-abi-and-shader-table-contract)
...
- Until Phase 8 adds any-hit, derive masked-geometry presence inside `RenderScene` from its authoritative primitives, instance groups, and material table. Refresh the bounded readiness bit only on the exact structural/material events that can change it; do not rescan the scene per frame or let another owner mirror it. Never run an opaque-only Pipeline route against masked geometry.
- Use one Renderer-owned automatic capability resolver for all dual-frontend effects: Pipeline when its shared requirements are complete, otherwise Inline, otherwise unavailable. Add no per-effect execution setting, CVar, plan wrapper, or fallback order. `FramePipeline` may retain only the resolved topology it actually built for rebuild detection. Keep `GBufferAlgorithm` selection independent and delete ambiguous execution-API wording rather than aliasing it.
- Schedule exactly one frontend pass and keep target creation, prepared scene, TLAS, material/geometry data, view, downstream consumers, histories, and the explicit rasterized-GBuffer algorithm unchanged.
...
- No consecutive-moving-frame comparison, screenshot-only acceptance, effect-local frontend selector, or product capability inferred from the conformance fixture alone.
...
- Shared capability state resolves deterministically to one active frontend; an unavailable result schedules no partial frontend and the explicit raster algorithm remains functional.
...
- The common phase evidence table and Code Review gate report `PASS` with no P0-P2 finding; duplicate outputs, scene/material tables, effect execution plans, or hidden per-effect selection block completion.
...
| One typed frontend is scheduled | Exact-search `Dispatch<RayTracingGBufferInlineCS>`, `TraceRays<RayTracingGBufferRGS>`, the shared frontend resolver, and GBuffer target creation | `PASS` as source shape: there is one typed call site for each frontend, one shared automatic resolver, one target factory, and one graph-setup switch. There is no execution-mode branch in a recording callback. |
| Automatic selection is honest | Trace the shared resolver through the scene-owned capability report, RHI AS/descriptor-indexing/Inline/Pipeline capabilities, GBuffer graph-construction failure, and built-topology rebuild observation | `PASS` as source shape: no GBuffer execution setting or derived effect plan is forwarded through frame-graph settings or feature-construction parameters; incomplete capability resolves unavailable before a frontend is scheduled; raster selection remains independent. Shader generation is not copied into policy or diagnostic labels. Native creation failure remains Phase 12 evidence and fails rather than becoming raster. |
...
| Generation lifetime and bounded capture join | Trace shader generation from `RenderPassRuntimeCache` through graph-topology identity, exact-composition runtime, native pipeline/table generations, reload replacement, and submission-token retirement | `PASS` as source shape: the resolved frontend plus shader generation identify built topology; the same generation creates the native pipeline and table; graph replacement follows identity change. Reload/several-frames-in-flight execution remains Phase 12 evidence. |
...
> Implement Phase 8 as one production hit-semantics and table-mapping CL directly in the unstaged `master` worktree. Apply the [common phase delivery contract](#common-phase-delivery-contract), including formula-corruption oracle coverage, scene/SBT authority closure, and the mandatory source-only Code Review gate. Add alpha-tested any-hit parity, a dual-frontend shadow consumer of the shared Surface hit contract, and one authoritative instance/geometry contribution plan shared by classic and partitioned TLAS construction. Update the Phase 12 validation route, but do not configure, build, compile shaders, cook, launch, run tests, capture, or collect performance evidence. Preserve the Phase 6 runtime and Phase 7 effect contracts; do not put material data or transient addresses in the SBT.

#### Phase-specific references

- [Ray-tracing target SBT index formula](../../Modules/Engine/Renderer/Features/RayTracing/ExecutionArchitecture.md#sbt-organization-and-index-formula)
- [Ray-tracing target scene/effect ownership](../../Modules/Engine/Renderer/Features/RayTracing/ExecutionArchitecture.md#target-ownership)
...
- Static checked-arithmetic inspection covers the full index formula, and Phase 12 owns multi-instance, multi-geometry, multi-material, Surface-record maximum-valid/first-invalid and independent corruption cases.
