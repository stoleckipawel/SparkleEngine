...
**Snapshot:** 2026-09-08 at committed `master` revision `ffe60e3a`; current module inventories, source/build ownership, product/tool routes, failure/exit boundaries, and CMake membership reconciled; evidence `S` only
...
## `PD-0` Persona Journey Freeze

The [release product classification, prerequisites, paths, and budgets](../../Acceptance/FirstRelease.md#frozen-product-and-output-classification) decide what each actor may expect. The trace below records the current source route through success, failure/recovery, and exit. A **Blocked** current route is still a decidable contract; it may not be described as a successful journey.

| Actor/journey | Entry and preparation | Execution and success oracle | Failure/recovery | Exit and current verdict |
| --- | --- | --- | --- | --- |
| Runtime consumer | Verify/extract the included archive -> `bin/ShowcaseRuntime.exe` -> `Projects/Showcase/Src/RuntimeMain.cpp` -> `RunRuntimeApplication()` -> `RuntimeApplication`; no environment, repository, admin, network, SDK, Launcher, or Editor is permitted. | Consumer first-run UI -> explicit included-map request -> `LevelSession` -> cooked load -> active catalog identity -> Renderer present; controls/help/settings/reset/quit and frozen path/budget observations complete success. | Unsupported machine, package/content/settings failure, or unavailable selection must stop/name/recover at its owner. Current source defaults to console-enabled runtime, uses `SPARKLE_STARTUP_LEVEL`, and can substitute built-in `Empty`, so success can be false. | UI/window quit -> Application stops new work -> console/Renderer/level/world/tasks/input/window/timer settle. **Blocked:** consumer UI, package, per-user state, Shipping console erasure, and failure/exit evidence are absent. |
| Source adopter | Root `README.md` -> immutable tag/dependency manifest -> supported source toolchain -> CMake configure. Root README is currently absent, and configure unconditionally enters Launcher/Qt before target selection. | Build `ShowcaseRuntime` plus required cook tools -> cook included products -> run one requested example -> active identity; exact exits, manifests, created paths, cold/warm durations, and no-SDK/unstable-source statement are the oracle. | Missing/old tool, network/cache/integrity failure, Unicode path, locked product, or cook failure must stop the dependent node and retain prior accepted output. Current optional-product membership does not make Qt optional. | Runtime exits; tool children/task scopes settle; generated roots remain removable through contained clean. **Blocked:** prerequisites are frozen but no clean cold/warm non-author result or separable runtime-only configure exists. |
| Optional contributor | `Docs/README.md` -> Engineering task map/change lifecycle -> owning Architecture/source/CMake -> local branch and supported source toolchain. This is Developer-only and does not create an SDK/contributor support promise. | Make one scoped change -> run the owner-selected claim-falsifying check -> inspect diff/status -> submit through the repository host; success is a reviewable change preserving unrelated work with exact evidence. | Missing guidance/tool/check, dirty overlap, failed validation, or unavailable hardware is recorded Failed/Blocked/Unavailable; no CI or private author repair can be implied. Root `CONTRIBUTING.md` and hosted CI are absent. | Stop processes, remove local test-only artifacts, preserve source and report exact status. **Blocked as a public promise; optional Developer-only workflow remains source-usable.** |
| Launcher developer | Developer artifact `artifacts/dev/launcher/<Profile>/SparkleLauncher.exe` plus deployed `RepositoryRoot.txt`, or its shell arguments -> repository-keyed `%LOCALAPPDATA%` state -> capability/preflight graph. | Typed sync/configure/build/cook/run/clean request -> revalidation -> child/result/artifact identity; run success follows the final product child through requested-level activation. | Missing tool/content, stale workspace, failed child, cancellation, timeout, locked output, or containment failure remains a distinct terminal result with recovery. Initial PID/output text/old artifact is not success. | Close/cancel joins Launcher task scope and descendants within budget, retaining final history/log. **Developer-only; source path present, acceptance Blocked.** |
| Editor developer | `ShowcaseEditor` -> `EditorMain.cpp` -> `RunEditorApplication()` -> `EditorApplication` over `RuntimeApplication` with runtime console disabled -> workspace/level UI. | Open registered level -> inspect -> typed edit -> undo/redo -> explicit Save All -> reload/render; world/source generations and bytes establish success. | Missing workspace/tool/cooked product, stale edit, save/recook/capture failure, or restart requirement stays visible and preserves the prior accepted world/source. Only explicit Save All may mutate the selected level source. | Close stops UI/operations then RuntimeApplication owners settle within budget. **Developer-only; source path present, non-author/build/package evidence Blocked.** |
...
| Persist/apply | Application settings persistence writes its fixed section; Application registers launch CVars, applies persisted values, then applies command-line assignments before host creation; Editor edits a private interaction model and Application submits the Renderer-owned value state | Typed CVar/settings state with command line taking precedence over persisted values | Renderer owns no filesystem/UI controller. View mode is session-only; adapter preference and back-buffer format require restart. Write failure/package path behavior is open. |
