...
| Debug buffer visualization | `r.Visualization`; 16 final/debug choices, independent of the Reference Path Tracer feature selector | Compute overwrites scene color before presentation | `VisualizeBuffersCS` reads lighting/GBuffer targets | Compute and requested source target | Yes | Yes | Not an exact raw/display-linear bypass; output still follows presentation path | Every visualization, unavailable combinations, presentation semantics |
