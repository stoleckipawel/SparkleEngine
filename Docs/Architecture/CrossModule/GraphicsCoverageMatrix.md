...
Module inventories: [RHI](../Modules/Engine/RHI/README.md), [Renderer](../Modules/Engine/Renderer/README.md), and [Shader Compilation](../Modules/Tools/ShaderCompiler/README.md)
...
| Reference path-traced lighting | `r.Lighting.Mode=ReferencePathTraced` | Direct sample + indirect sample + high-precision temporal accumulation | Two compute shaders with inline ray queries, then accumulation compute | AS + inline ray query + descriptor indexing | DXR tier 1.1 | Vulkan ray query | No native RT-pipeline adapter and no non-ray fallback | Convergence, reset correctness, numerical stability |
| Lighting composite and sky | Both lighting modes | Five lighting lobes composite into `SceneColor`; sky fills background from scene depth | `LightingCompositeCS`, then `SkyCS` | Compute, scene/lighting formats, sky texture | Yes | Yes | Invalid lighting enum throws during graph construction | Raw lobe accounting and backend image evidence |
...
| Tone mapping | ACES approximate default; Reinhard, Exponential, Uncharted 2, None | Full-resolution compute from resolved HDR scene color and exposure to display-linear intermediate | `ToneMappingCS` | Compute; float SRV/UAV | Yes | Yes | Invalid selection is a settings defect; `None` remains an explicit operator | Numeric ramps, finite HDR values, all mapper choices |
...
| Tone/output | Five tone mappers; Automatic/Linear/sRGB encoding | ACES approximate, Automatic | SDR-style output only in current contract |
...
Targeted source review did not find a complete current product path for runtime-sized bindless resources, raster shadow maps, a non-ray lighting fallback, transparent blending/transmission, 3D textures, mesh/task shaders, tessellation/geometry pipelines, multiple primitive topologies, HDR display transfer/metadata, variable-rate shading, sampler feedback, or a persistent cross-invocation shader compile cache. Low-level enums or compiler target vocabulary must not be promoted into product claims without a producer, consumer, selector, and executable evidence.

## Evidence Linkage

The executable closure items for these rows are `RHI-001` through `RHI-014`, `REN-001` through `REN-022`, and `SHD-001` through `SHD-012` in the [Capability Evidence Plan](../../Plans/CapabilityEvidence.md). Candidate-specific results belong in [Feature Completion Reports](../../Acceptance/FeatureCompletionReports.md), not in this snapshot.
