...
| Reference Path Tracer contract | `RenderViewMode::ReferencePathTracer`; not user-selectable before Stage 7 | Ordinary per-view selector transport, one Private feature owner, and a generic unavailable/progress observation; no reference-session identity exists yet | No estimator, reference shader, history, accumulator, digest, generation lease, or reset/result state exists in Stage 1 | No Reference Path Tracer preflight yet | Dormant semantic only; no execution | Dormant semantic only; no execution | Generic `Unavailable` observation only; ordinary Lit presentation remains visible but is not labelled reference output. Exact refusal reasons/actions arrive with the real session and user route. | Stage 1 selector/enclosure/build probe; later identity, estimator, accumulation, UX, parity, and oracle gates |
...
| Debug buffer visualization | per-view `RenderViewMode`; 16 final/debug modes plus the separate Reference Path Tracer semantic | Compute overwrites scene color before presentation | `VisualizeBuffersCS` reads lighting/GBuffer targets | Compute and requested source target | Yes | Yes | Not an exact raw/display-linear bypass; output still follows presentation path | Every view, unavailable combinations, presentation semantics |
