...
| Debug buffer visualization | Per-view `ViewportRenderRequest::ActiveVisualization`; 16 final/debug choices, independent of the transitional Reference Path Tracer feature selector | `RenderViewBuilder` freezes the request value and compute overwrites scene color before presentation | `VisualizeBuffersCS` reads the focused View scalar plus lighting/GBuffer targets | Compute and requested source target | Yes | Yes | Not an exact raw/display-linear bypass; output still follows presentation path | Every visualization, two-view isolation, unavailable combinations, presentation semantics |
