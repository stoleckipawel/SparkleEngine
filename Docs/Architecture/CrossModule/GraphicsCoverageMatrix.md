...
| Reference direct and indirect surface lighting | `r.Lighting.Mode=ReferencePathTracer` | GBuffer-seeded direct sample + indirect sample + high-precision temporal accumulation | Two compute shaders with inline ray queries, then accumulation compute | AS + inline ray query + descriptor indexing | DXR tier 1.1 | Vulkan ray query | No camera-ray-independent oracle, native RT-pipeline adapter, or non-ray fallback | `PTD-00`; dependency independence, estimator/domain, convergence, reset, numerical stability |
...
| Settings persistence | 27-name owned INI section; view mode session-only; adapter/format report pending restart | Current runtime/CVar defaults | Aggregate requested state does not expose every console/RHI control or resolved active/fallback state; in-place silent writes are a known gap |
