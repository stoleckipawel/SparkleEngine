...
| Render-pass naming, typed shader programs, and generated cooked artifacts | [Shader Authoring and Cooked Program Architecture](ShaderAuthoringAndCookedPrograms.md) | target proposal; implementation must still be proven |
...
For shader authoring or cooking changes, read [Shader Authoring and Cooked Program Architecture](ShaderAuthoringAndCookedPrograms.md) for the target identity model and migration gates, then verify the current shader registry, compiler, cooked schema, runtime cache, and pass definitions in code. The target document does not prove that manual pass or package strings have already been removed.
