...
- `Cook` remains a set of explicit diagnostic operations because shader, texture, asset, and aggregate cooks still own materially different request options and outputs. Run mode, build configuration, compiler, IDE, graphics API, and shader compiler remain in the persistent footer below activity output; the IDE choice selects generated workspace format and never opens the IDE. Run mode and graphics API feed `levels.run`, while DXC/Slang feeds shader cook requests; the Cook page does not duplicate those choices.
