...
- `Quick Start` is the level-catalog surface. It preserves the `Explore Sparkle` hero and projects the complete content-owned catalog, including the repository-resident `Empty` level, as thumbnail cards immediately below it. Each runnable card exposes one context-sensitive action: `Open` for the default Editor run mode or `Run` for Game. The persistent footer owns the single Editor/Game Run Mode selector, while the page footer retains Sync All/Clean All for catalog-wide maintenance.
- `levels.run` is a UI-independent Levels-domain operation that owns only the final selected-product process, selected startup-level environment, and graphics-API launch argument. A typed run mode selects the existing editor or runtime profile, artifact role, and product target. A prerequisite capability graph resolves the requested card into the existing typed operations for level acquisition, build-file generation, selected-product compilation, cook-tool preparation, and content cooking before that final operation runs. The main window submits the requested level ID plus footer context and renders progress; it does not implement those backend tasks.
...
- The former Products category, `launch.editor`/`launch.runtime` operations, `workspace.open-ide` operation, startup-level selector, and top header band were removed together. Opening a catalog map remains one `levels.run` goal parameterized by the footer Run Mode; there is no product-card, duplicated per-map action, or IDE-opening compatibility path. Graphics API remains a real selected-product choice and is projected only in the persistent footer.
...
- `Build` projects one `workspace.build` intent with selectable Editor, Game, Cooking tools, and Launcher scopes. Editor-profile targets are combined into one build invocation, the Game target uses its runtime profile, and CMake owns target dependency ordering and incremental work. Generated workspace files are an inferred prerequisite and refresh only when stale. The frontend shows a setup section only for missing or unhealthy prerequisites; the per-product build operations remain backend capability primitives for Quick Start and are not duplicate tabs.
- `Cook` projects one `cook.workspace` intent with selectable Shaders, Textures, and Scene assets scopes. The CookOperations planner turns that selection into one ordered request and limits required-tool checks to the chosen stages. Shader cooking discovers every registered package and leaves runtime-target selection to ShaderCompiler's canonical defaults; package filters and compatibility targets remain direct ShaderCompiler diagnostic options. Existing cook-tool artifacts can run without a current host-build workspace. Healthy prerequisites stay out of the page; when a selected cook tool is missing, only the host-tool or generated-build-file recovery prerequisites needed to rebuild it appear. The existing `cook.shaders`, `cook.textures`, `cook.assets`, and `cook.all` operations remain backend capability primitives for Quick Start rather than duplicate tabs. Run mode, build configuration, compiler, IDE, graphics API, and shader compiler remain in the persistent footer below activity output; the IDE choice selects generated workspace format and never opens the IDE. Run mode and graphics API feed `levels.run`, while DXC/Slang feeds shader cook requests; the Cook page does not duplicate those choices.
- Footer selectors are projections of existing authorities: LauncherSettings owns the normalized Run Mode selection, the build-profile catalog supplies configurations, toolchain detection supplies compiler/IDE/API/backend availability evidence, RHI owns graphics-API parsing, and CookOperations owns shader-backend execution. Each popup groups usable choices under `Available` and keeps recognized choices that still need setup visible but disabled under `Supported`; refresh rebuilds the projection after launcher operations or application activation. Visual Studio IDE availability requires an actual `devenv.exe` and remains separate from Visual Studio C++ Build Tools availability.
...
`-- LauncherLevelUiModel card catalog
    |-- Run(levelId, runMode) -> levels.run goal
    |   `-- capability graph
    |       |-- levels.sync(levelId) -> LevelOperations, when content is missing
    |       |-- selected-product workspace generation/build -> BuildWorkspaceOperations, when stale or missing
    |       |-- cook tool/content operations -> CookOperations, when stale or missing
    |       `-- levels.run -> LevelRunOperations final selected-product process
