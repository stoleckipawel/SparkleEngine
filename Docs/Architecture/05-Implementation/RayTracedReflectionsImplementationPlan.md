...
| `RayTracingPerformanceMetrics` timing fields | `Engine/Renderer/Private/RayTracing/Diagnostics/RayTracingPerformanceMetrics.h` and related metric structs | stores CPU/GPU milliseconds inside ray tracing domain metrics | delete timing fields; keep non-timing metrics | provider choices, counts, validation state, and capability reasons remain ray tracing diagnostics |
