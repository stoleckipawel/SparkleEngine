...
This table records the current unstaged `master` checkpoint only. It does not claim compilation, cooking, cancellation execution, backend output, runtime activation, capture, or performance evidence. Phase 4 retains the first executable obligations named below.
...
| Failure/cancellation cannot publish a partial generation | Trace the Application stop token and private cancellation signal through the child process; trace batch task failure to `ShaderPackageCooker` and `CookedShaderPackageEmitter`; inspect `TryPublishFileSet` membership | `PASS` as a source-consistency claim: cancellation observed before emission and every compile/ABI exception return before the emitter. The parent no longer hard-terminates the cooker. Packages, registry, dependency manifest, and recook signal are staged and published as one file set only after every logical result finalizes; cancellation after commit begins loses to that coherent publication rather than interrupting it. Phase 4 must execute cancellation and failed-job nonpublication oracles. |
...
| Existing validation consumers encode the deferred behavioral oracles | Inspect `ValidateShaderCompilerCli.cmake` and Phase 4 executable requirements | `PASS`: the focused validation route now performs a full dependency-producing cook followed by changed-path selection and two identical shader-target operations before artifact inspection. Phase 4 explicitly owns the additional instrumentation or temporary local harness needed to prove producer fan-out, actual repeated compiler invocation, exact closure, cancellation, failed publication, and portable hashes; none is inferred as executed here, and no test-only scaffold is submitted in Phase 3. |
