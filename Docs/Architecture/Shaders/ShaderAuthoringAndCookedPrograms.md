...
Current audit: 2026-08-23 against the unstaged `master` worktree based on `77964bd5da39f554d728f1f4d2b40397473df89e`
...
| D3D12/Vulkan lower the same complete neutral descriptor | Field-by-field source trace from `BuildGraphicsPipelineDesc` into both backend constructors plus exact searches for hard-coded opaque/sample-one/static-mesh pipeline substitution | `PASS` as a source claim: both backends consume the declared color writes/blend operations, fill/cull/front face/depth clip, depth/stencil, topology, vertex declaration, attachment formats/count, and sample count. Invalid counts, missing vertex bindings, unsupported enum mappings, non-explicit formats, incompatible depth/stencil formats, and invalid write masks fail at the backend boundary; paired execution remains Phase 12. |
