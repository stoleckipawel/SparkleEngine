...
Status: plan and implementation authority for unified shader, graphics-pipeline, and ray-tracing delivery; Phase 0 is frozen and the current unstaged `master` worktree contains the Phase 1-5 source-consistency checkpoints, while the complete target and executable proof still require Phases 6-12
...
| vertex declaration, front-face convention, depth clipping, topology, streams, index format, and draw counts | `GpuMesh` | the stable declaration/topology enter the key/descriptor, while streams and draw arguments remain recording data |
...
| viewport and scissor | current `RenderView` through `GBufferMeshPass::PrepareRasterPass` | dynamic command state; absent from key and descriptor identity |
...
| native state lowering | complete `GraphicsPipelineDesc` at public RHI boundary; D3D12/Vulkan private consumers | both backends consume explicit blend/write, raster, depth/stencil, topology, vertex-input, attachment, and sample facts or reject unsupported counts before recording |
...
| Prepared mesh/material work owns geometry and raster variants | Trace `GpuMesh` declaration/topology/stream binding and `GBufferMeshBatchDrawer` material/view filtering in both request collection and drawing | `PASS` as a source claim: one mesh topology-setting call remains, actual vertex attributes replace the one-value selector, and only drawable material batches request variants. No mesh-command cache was added. |
| Complete key and exact lazy materialization have one generation owner | Inspect every `GraphicsPipelineKey` field and explicit hash contribution, request collection before graph setup/parallel recording, cache lookup during drawing, reload replacement, and retired generation lifetime | `PASS` as a source claim: generation, shader code, binding signature, blend, raster, depth/stencil, topology, vertex input, attachment formats/count/depth/sample all affect equality/hash. Labels, viewport/scissor, streams, and draw arguments do not. Missing lookups assert; materialization is absent from recording callbacks. Phase 12 executes perturbation, collision, stale-generation, exact-count, reuse, and retirement oracles. |
| D3D12/Vulkan lower the same complete neutral descriptor | Field-by-field source trace from `BuildGraphicsPipelineDesc` into both backend constructors plus exact searches for hard-coded opaque/sample-one/static-mesh pipeline substitution | `PASS` as a source claim: both backends consume the declared color writes/blend, fill/cull/front face/depth clip, depth/stencil, topology, vertex declaration, attachment formats/count, and sample count. Invalid counts or vertex bindings fail at the backend boundary; paired execution remains Phase 12. |
| Source/build/docs hygiene and review | Recursive CMake membership inspection, exact stale-name and semantic-equivalent searches, pinned no-write formatting, local Markdown-link validation, `architecture_boundary_check`, `git diff --check`, branch/staging inspection, and Code Review procedure | `PASS` as a source-only review when the recorded handoff commands below remain clean; no executable result is inferred. |
