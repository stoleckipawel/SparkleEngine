...
Status: plan and implementation authority for unified shader, graphics-pipeline, and ray-tracing delivery; Phase 0 is frozen and the current unstaged `master` worktree contains the Phase 1-4 source-consistency checkpoints, while the complete target and executable proof still require Phases 5-12
...
| One generated map and code-library production route exists | Trace `GlobalShaderCooker` -> compile results -> `ShaderArtifactPublication` -> staged map/library validation -> one `TryPublishFileSet`; inspect logical-key ordering, exact code-hash deduplication, empty-code rejection, and temporary-file cleanup | `PASS` as a source claim: `(ShaderTypeId, Target)` entries are deterministically ordered, identical bytecode shares one exact hash record, collisions fail, map/library share one publication hash, and partial publication is unreachable. Phase 12 cooks and corrupts artifacts. |
