...
| demodulated indirect diffuse | Noisy diffuse radiance divided by diffuse albedo/material factor | `FrameAssemblyDenoiserProviderResources.IndirectReconstruction.DemodulatedIndirectDiffuse`, `LightingRenderTargets.IndirectDiffuseDemodulatedRadiance` | Required by NRD-style denoisers that apply material factors after reconstruction |
| demodulated indirect specular | Noisy specular radiance divided by specular/F0 material factor | `FrameAssemblyDenoiserProviderResources.IndirectReconstruction.DemodulatedIndirectSpecular`, `LightingRenderTargets.IndirectSpecularDemodulatedRadiance` | Required by NRD-style denoisers that apply material factors after reconstruction |
| diffuse albedo | Provider-ready diffuse reflectance guide, linear | `FrameAssemblyDenoiserProviderResources.IndirectReconstruction.DiffuseAlbedo`, `LightingRenderTargets.IndirectDiffuseAlbedo` | Required by DLRR-style reconstruction when selected; derived from base color and metallic, not copied blindly from `GBufferBaseColor` |
| specular albedo | Provider-ready specular reflectance/F0 guide, linear | `FrameAssemblyDenoiserProviderResources.IndirectReconstruction.SpecularAlbedo`, `LightingRenderTargets.IndirectSpecularAlbedo` | Required by DLRR-style reconstruction when selected; derived from the same F0/metallic policy used by lighting |
| material guide | `float4(roughness, metallic, dielectricF0, valid)` | `FrameAssemblyDenoiserProviderResources.IndirectReconstruction.MaterialGuide`, `LightingRenderTargets.IndirectMaterialGuide` | Supplies Streamline-style roughness plus local material metadata without adding separate placeholder resources |
| diffuse sample guide | `float4(hitDistanceWorld, sampleConfidence, lobeId, hitValid)` for the first diffuse-selected path sample | `FrameAssemblyDenoiserProviderResources.IndirectReconstruction.DiffuseSampleGuide`, `LightingRenderTargets.IndirectDiffuseSampleGuide` | Optional for providers that reconstruct diffuse/GI rays |
| specular sample guide | `float4(hitDistanceWorld, sampleConfidence, lobeId, hitValid)` for the first specular-selected path sample | `FrameAssemblyDenoiserProviderResources.IndirectReconstruction.SpecularSampleGuide`, `LightingRenderTargets.IndirectSpecularSampleGuide` | Supplies Streamline/NRD-style specular hit-distance data without a private DLRR shader path |
