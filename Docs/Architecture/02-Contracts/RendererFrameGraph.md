...
Primary-visibility conclusion:

- NVIDIA hybrid ray tracing commonly uses rasterized primary visibility data such as a GBuffer or visibility buffer, then traces secondary effects and denoises lighting around that stable primary surface data.
- Falcor's path tracer can sit in a graph that supplies a packed visibility buffer, motion vectors, and optional guide outputs, but `PathTracer` remains the owner of path generation, path tracing, and resolve.
- RTXPT's pure path tracing configuration does not rely on rasterization for light transport; primary rays and secondary rays are part of the path-tracing renderer, with guide buffers generated by that path rather than borrowed from a deferred pass.
- Sparkle's current reference path follows the pure primary-ray variant. It traces camera rays through TLAS and therefore must not consume or publish realtime GBuffer products unless a future explicit visibility-buffer/reference-depth stage writes them.
...
8. Top-level renderer-path selection is expressed by `FrameRenderPath`. Individual pass settings may choose the path, but frame orchestration should depend on the renderer-path enum rather than on one pass's CVar or settings struct.
9. `FrameBuildResult` and `FrameGraphBuildResult` carry the render path used to build the graph. Runtime pipeline state should track that returned value instead of re-resolving settings after graph construction.
...
- `Engine/Renderer/Private/Frame/Core/FrameRenderPath.h`
- `Engine/Renderer/Private/Frame/Core/FrameRenderPath.cpp`
