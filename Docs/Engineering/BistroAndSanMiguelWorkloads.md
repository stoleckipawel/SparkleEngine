...
All external maps are absent from the repository by default. The `Levels` group presents direct Sync/Clean actions for each level plus Sync All/Clean All actions for the catalog. A direct Sync acquires the packages referenced by its chosen level; runtime-supported levels enter the active selection, while source-only packs do not. `Sync All` on that page acquires every downloadable package and excludes download-disabled future packs; it does not sync code dependencies. While a direct level sync is running, its action becomes `Cancel`: that explicit action cancels the owned acquisition process tree, waits for it to settle, then removes the level's extracted content, transaction staging and backup paths, cached archive, and partial archive before deselecting the level. It does not ask for a second confirmation; parent packs still required by another selected level are retained. Both level sync paths cache the publisher archive in the per-repository user-local launcher state, validate its published byte count and catalog-pinned SHA-256 before extraction, and transactionally publish the extracted content together with its provenance manifest into gitignored project content roots. Every Modern Sponza sync acquires the publisher base and Colorful Curtains first; later add-ons layer on that user-facing baseline.

The launcher must distinguish `ready`, `selected`, `source ready`, and `future` states. A source archive being present does not make a workload runtime-supported. Runtime-unsupported packages remain visible with their official source page and exact blocker. A direct Sync action may acquire a pack whose download metadata is supported, but it does not select that level or admit it to runtime cook and launch paths. Packages with unsupported acquisition remain disabled and cannot enter a sync plan.
