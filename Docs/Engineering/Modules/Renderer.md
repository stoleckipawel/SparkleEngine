...
- High-level composition bodies read as rendering intent in execution order: add the selected renderer, add exposure, add optional scene visualization, add scene denoising, add presentation upscaling, and add presentation. Their conditionals may select a real implementation or output policy, but they do not inspect handle validity, choose providers, allocate targets/history, assemble parameter records, map products, or repeat a child feature's enablement policy. Those mechanics belong to the narrow `Add...Pass`/`Add...Passes`, resource, or publication owner named by the orchestration call.
- Scene denoising and presentation upscaling are separate composable stages. A lighting denoiser owns a render-resolution product such as `DenoisedSceneColor`; it never chooses presentation resolution or writes `ResolvedSceneColor`. Linear or DLSS Super Resolution then consumes the selected raw/denoised scene color and exclusively owns `ResolvedSceneColor` at output resolution. Enabling Ray Reconstruction must not disable, replace, or silently select the presentation upscaler. A provider that requires depth, motion, exposure, or other guides must reject before resource allocation when the selected renderer does not publish them; never bind another renderer's products, fabricate guides, or silently substitute a provider.
...
- File and function names state the rendered operation and composition level: `AddToneMappingPass`, `AddRealTimePathTracerPasses`, `SceneRenderingPasses.cpp`, and `GBufferRenderTargets.cpp` are representative. Avoid vague `Process`, `Execute`, `Utility`, or feature-only filenames when the unit actually owns a narrower pass, target, or orchestration responsibility.
- Pass folders follow semantic change boundaries, not pass count. Keep a thin family root for composition and shared products; add a child such as `Raster`, `RayTracing`, `Indirect`, `Reconstruction`, `Exposure`, `Upscaling`, or `Display` only when it owns a cohesive implementation family. Do not retain a god folder, create one folder per pass, or use `Debug`, `Utility`, or `Common` as a substitute for the actual owner.
...
When a feature has a small closed implementation set, its focused `Add...Passes` function uses one exhaustive switch over the established selector and calls one concretely named operation per case. `AddGBufferPasses` visibly selects Rasterized or Ray Tracing, and `AddSceneUpscalingPasses` visibly selects Linear or NVIDIA DLSS. Shared resource preparation occurs once before the switch; provider interfaces, capability handling, parameter binding, and dispatch remain inside the selected operation. Do not replace pass selection with a factory, registry, callback table, or generic provider call at the selector; factories may still own implementation lifetime below it.

An optional pass or pass family owns its activation test at the start of its own `Add...Pass`/`Add...Passes` entry point and returns before declaring resources when inactive. Higher-level composition invokes the semantic operation unconditionally; it does not collect `Is...Active`/`Should...Run` predicates or grow one `if` per child. A composition-level branch remains appropriate only when it selects mutually exclusive implementations or changes graph topology or products that must be known before entering either implementation.

Visualization passes are partitioned by the semantic product family they read, such as GBuffer, lighting, or GPU-scene data. Each family entry point privately resolves its activation from `RenderViewMode`, returns before declaring resources when inactive, and owns one focused pass/shader parameter surface. The visualization composition owner names the family entry points in frame order; it does not repeat their activation branches, infer activation from enum ordering, or bind unrelated families to one catch-all shader.
...
- A mode has one rendering meaning and one declared primary consumer. Reference Path Tracer selects the alternate middle, Wireframe selects raster fill, and buffer/lobe/instance modes select a focused visualization resolve. Do not re-encode these as CVars, show flags, target enums, Editor enums, graph settings, feature settings, or RHI fields. Passes and shaders receive the mode only where they select among the products that pass owns.
...
## Shader Authoring And Ownership

Renderer-owned HLSL/HLSLI follows the same readability, enclosure, naming, and single-responsibility rules as Renderer C++; [Code Style](../Foundations/CodeStyle.md#hlslhlsli) owns the language-wide rule. A shader entry point reads as a short GPU operation: establish the invocation coordinate, load the owned inputs, invoke focused semantic operations, and publish the result. It does not become a second location for material reconstruction, traversal policy, BSDF math, feature admission, or diagnostic plumbing.

- Shared ray/path-tracing owners contain reusable scene tracing, hit reconstruction, material evaluation, endpoint construction, BSDF evaluation/sampling, light sampling, and visibility semantics. A Reference Path Tracer shader owns only its estimator, deterministic sample dimensions, reference-specific termination policy, and accumulation contract; it does not prefix or copy operations an optimized path tracer can consume.
- Prefer one cohesive returned shader record when an operation produces several values with one meaning. Do not expose a long list of `out` parameters, parallel validity values, or duplicated scalar carriers merely because HLSL permits them.
- Split a shader file when its parts have different semantic owners or change for different reasons. Do not split a short cohesive kernel into forwarding includes, and do not collect unrelated shader helpers under `Common`, `Utility`, or a feature folder.
- Shader whitespace marks the same semantic phases as C++ whitespace. Keep initialization of one record together; isolate a wrapped statement as its own paragraph; separate acquisition, derivation, stochastic choice, transport update, and publication when those phases are materially distinct.
