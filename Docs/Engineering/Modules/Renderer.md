...
- UI view modes are frontend preset state. UI enums, labels, ordering, grouping, and selection identity do not enter Renderer. The ordinary `ViewportRenderRequest` may carry concrete Renderer-semantic `Visualization` and `RenderShowFlagSet` values because they are intrinsically scoped to one view; Renderer freezes them into `RenderView`, and RHI remains unaware.
- Renderer owns renderable facts and mechanisms: canonical scene/view data, show-flag semantics, global implementation CVars, graph composition, passes, resources, shader parameters, feature lifecycle, and generic products/progress. It does not own a frontend aggregate whose purpose is to remember which menu item produced those facts.
- A visualization selects the concrete debug/final product or channel. A show flag controls an implemented contribution for one view and may select a mutually exclusive frame composition when that is its declared semantic. Neither is a Renderer view-mode mirror. Passes and shaders receive only the focused values they consume.
...
- Frontend code never uses process-global CVars as the normal transport for per-view visualization or show flags. A genuinely global Renderer CVar action still crosses the sequenced control boundary and is applied on the Renderer owner thread. This is not permission to introduce a Renderer view-mode enum, generic settings bag, or duplicate per-view/global selector.
...
- Does the frame composition contain one readable branch on an immutable per-view Renderer semantic, with feature mechanism enclosed and no recipe hierarchy, dependency bag, repeated selector, UI vocabulary, global CVar mirror, or RHI leakage?
