...
- [Naming and Vocabulary](Naming.md) for identifiers and domain terms;
- [Repository Structure and Ownership](ModuleOwnership.md) for responsibility, APIs, and lifetimes;
- [Validation, Performance, and Evidence](../Verification/ValidationAndEvidence.md) for comments, assertions, logging, and tests.
...
The clang-tidy naming configuration follows the binding scope in [Naming and Vocabulary](Naming.md): it checks lower-camel locals and parameters and the `m_` convention for non-public implementation state, but it does not impose one casing rule on public aggregate or ABI fields. Established lowercase literal suffixes remain accepted. Repository-wide `[[nodiscard]]` adoption remains a candidate rather than an active gate, as recorded in [Adding A Detailed Rule](#adding-a-detailed-rule).
...
Ownership/lifetime and inheritance rules belong to [Repository Structure and Ownership](ModuleOwnership.md); units, domains, and ABI vocabulary belong to [Naming and Vocabulary](Naming.md).
...
- Portable byte serialization, hashing, text/UTF-8, JSON token decoding, identifier formatting, and filesystem/path conversion extend their established `Core` owner instead of appearing as feature-, Editor-, Renderer-, or RHI-local utility functions. Product schemas, subsystem resource selection, native API conversion, and feature orchestration remain with their semantic owner; syntactic similarity is not permission to move policy into `Core`.
...
- Do not introduce a namespace, wrapper function, phase function, or one-method collaborator merely to rename or forward one operation. Keep the direct call until an extraction owns real policy/state, hides nontrivial mechanism, establishes a required boundary, or has multiple meaningful consumers; the proposed abstraction must make its caller easier to understand without concealing the actual workflow.
...
A frontend or simulation thread MUST NOT mutate a Renderer-owned CVar directly when rendering may execute on another thread. An intrinsically per-view rendering choice travels as the one concrete `RenderViewMode` on the ordinary viewport request and is frozen into the corresponding View; it is not a CVar. Do not mirror that choice as an Editor enum, visualization target, show flag, graph setting, feature setting, or RHI field. A genuinely process-global CVar action crosses the existing sequenced Renderer control boundary and is applied on the Renderer owner thread. Neither route carries labels, icons, widget state, or a generic settings/context mirror into FrameGraph, feature state, or RHI.
...
The C++ readability and ownership rules in this document apply to owned shaders by default: cohesive functions, descriptive names, semantic whitespace, narrow files, one authoritative invariant, and a clear separation between orchestration, reusable math, and feature-specific policy. Diverge only where HLSL syntax, shader-stage ABI, wave/group execution, resource binding, or measured GPU behavior requires it; document the reason at the owning shader contract instead of allowing shaders to become a lower-quality parallel codebase.
...
| Naming cases and domain vocabulary | Binding | [Naming and Vocabulary](Naming.md) and `.clang-tidy` | migration impact and ABI/serialization review for changes |
| Files, headers, includes, namespaces | Binding | this document | compile boundaries and dependency graph |
| Ownership, RAII, API and lifetime expression | Binding | [Repository Structure and Ownership](ModuleOwnership.md) | concrete owner/consumer analysis |
...
| comments and API documentation | Binding core; detailed conventions may expand | [Validation, Performance, and Evidence](../Verification/ValidationAndEvidence.md) | generated-doc needs and review signal |
...
Formatter precedents, pinned revisions, and the accepted comparison belong to the [Clang-Format Decision Record](../Decisions/ClangFormat.md). Any new precedent must record the exact artifact/revision, observed behavior, local adoption boundary, migration cost, and falsifying check.
