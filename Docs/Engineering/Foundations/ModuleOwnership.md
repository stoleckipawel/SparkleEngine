...
Legacy and compatibility paths are not complexity-budget candidates under the [current clean-break policy](../Workflow/ChangeIntegration.md#current-clean-break-policy); they are prohibited for owned Sparkle representations.
...
Apply the [single-truth and copy budget](DataAndMemory.md#single-truth-and-copy-budget) to representation placement. A value holder belongs at the real lifetime or publication boundary it serves; convenience mirrors and copied settings do not become owners merely because they have a type name.
...
High-level pipeline, graph, scheduler, and factory dependency surfaces MUST NOT grow one concrete feature parameter, member, or dependency-record field at every call layer whenever a new variant is introduced. Put the single policy branch in the existing lifecycle/composition owner, read registered policy there, and call the selected concrete capability directly. The implementor owns its passes, resources, state, and local mechanism. Do not replace one readable branch with a base interface, recipe hierarchy, registry, service locator, universal context bag, callback list, forwarding facade, or dependency aggregate that exists only to hide which capabilities are composed.

Do not plumb globally registered runtime policy through orchestration layers merely to save a repeated lookup. The narrow capability owner reads its CVar or equivalent registered setting where the decision is made, following [Console-variable access](CodeStyle.md#console-variable-access). Snapshot a resolved value only when frame consistency, thread handoff, topology identity, persistence, or another named lifetime boundary requires one immutable observation.
