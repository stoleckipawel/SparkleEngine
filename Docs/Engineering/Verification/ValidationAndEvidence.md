...
Comments do not narrate obvious code, preserve obsolete history, promise unspecified refactors, repeat a prompt, call unsupported behavior “thread safe” or “lock free,” or justify internal versioning and compatibility machinery prohibited by the [current clean-break policy](../Workflow/ChangeIntegration.md#current-clean-break-policy).

Public API documentation conventions beyond this baseline remain an explicit [coding-style decision](../Foundations/CodeStyle.md#decision-status).
...
## Check And Test Design Contract

`CHK-*` is the identity of a validation action, not permission to add a permanent test executable, fixture, class, function, source file, or CTest entry. A check may be a static query, schema/format validation, focused build, runtime action, image/reference comparison, native API validation run, capture, benchmark, controlled fault, or authorized automated test.

Every material check record MUST name:

1. the `AC-*` acceptance claim or `FM-*` failure contract it can falsify and any affected `RISK-*`;
2. the smallest owner/producer/consumer boundary exercised and why a narrower check is insufficient;
3. exact initial state, revision/dirty state, configuration, executable, inputs, backend, hardware/driver/tool versions, and cache/package state;
4. the action or injected fault, including timing and repetition/sample policy where relevant;
5. an oracle fixed before execution: expected value/state/image/error, threshold/tolerance, permitted warning set, and maximum duration/resource bound;
6. a defect-detection control showing that the check fails when its target invariant is deliberately violated or otherwise demonstrating that a false pass is unlikely;
7. captured raw output, artifact path/hash, exit status, created/modified files, and observer effect;
8. cleanup and isolation proving that fault injection did not alter trusted source, candidate bytes, retained evidence, or unrelated user state;
9. `PASS`, `FAIL`, `BLOCKED`, or `INCONCLUSIVE`, the exact reason, and the condition that triggers a broader check.

Coverage is explicit and many-to-many: every applicable `AC-*` maps to one or more `CHK-*`; every applicable `FM-*` maps to a controlled negative `CHK-*`; every `CHK-*` names the criteria and risks it informs. A successful happy-path run cannot close a failure contract, and a check that never observes its oracle cannot pass. When a criterion changes after seeing a result, record the reviewed contract change, invalidate affected evidence, and rerun it.
