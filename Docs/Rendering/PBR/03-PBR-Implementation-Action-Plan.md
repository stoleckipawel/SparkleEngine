...
- Validation commands: rebuilt `ShowcaseEditor` and `ShaderCompiler`; cooked `GBuffer`, `DirectLightingNoRayQuery`, `DirectLighting`, `DirectLightingDeviceAddress`, `IndirectDiffuse`, `IndirectSpecular`, and `VisualizeBuffers` for `DxilSm66` and `SpirV16`. `LightingComposite` HLSL compiled for both targets, but full package cook remains blocked by the pre-existing reflected binding mismatch between shader cbuffer `PerFrameConstantBufferData` and registration layout alias `PerFrame`.
...
- Validation commands: rebuilt `ShaderCompiler` and `ShowcaseEditor`; cooked `GBuffer`, `DirectLightingNoRayQuery`, `DirectLighting`, `DirectLightingDeviceAddress`, `IndirectDiffuse`, and `IndirectSpecular` for `DxilSm66` and `SpirV16`. Code validation confirms the mirror-sky path uses exact reflection through `BRDF::SpecularSampling::SampleReflectionLobe` and linear HDR sky miss radiance through `SampleSkyEnvironmentRadiance`; image capture validation remains Stage 13 validation asset work.
...
- Validation commands: cooked `GBuffer`, `DirectLightingNoRayQuery`, `DirectLighting`, `DirectLightingDeviceAddress`, `IndirectDiffuse`, and `IndirectSpecular` for `DxilSm66` and `SpirV16`. Deterministic image tests for primary-vs-ray-hit normal parity, motion-vector movement, and alpha-tested direct-shadow parity remain Stage 13/Stage 5 validation asset work.
...
- Local deviation: Stage 4 locks punctual-light units/falloff only. Finite-source production shading is handled by Stage 4A with physically sampled area-light radiance and PDFs.
...
- Implementation: `PunctualLights::ComputeDistanceAttenuation` now uses `1 / max(distance^2, 1.0e-4)` with the `smooth^2` `1 - x^4` range cutoff, and `ComputeSpotConeAttenuation` now squares the cone ramp. glTF/imported and authored light intensity values pass through without arbitrary compensation. The editor labels directional intensity as lux and point/spot intensity as candela; serialized levels write and parse only explicit unit fields (`IntensityLux`, `IntensityCandela`, or `LuminanceCdPerM2`). Cooked/runtime/render-view/shader light structs carry the same unit contract. Spot outer cone defaults now match the glTF default `pi/4` where local light records are initialized.
- Build checks: cooked `DirectLightingNoRayQuery`, `DirectLighting`, `DirectLightingDeviceAddress`, `IndirectDiffuse`, and `IndirectSpecular` for `DxilSm66` and `SpirV16`; built `ShowcaseEditor`, `SourceImporters`, `SceneCooker`, and `ShaderCompiler` for `DevelopmentEditor`.
...
- Finite point-like sources become sphere lights when `sourceRadius > 0`.
- Finite spot sources become disk or cone-projected disk emitters oriented by the spot direction when `sourceRadius > 0`.
...
- Shadow-only jitter is not supported as a fallback finite-light mode.
...
- Reference lineage: finite source sampling now follows the PBRT `SampleLi`/`PDF_Li` shape: a sampled emitter position or direction produces `wi`, distance, emitted radiance, and a solid-angle PDF before BRDF evaluation. Sparkle follows the Falcor/RTXPT direct-light structure by keeping a single analytic-light sample payload that can later be combined with BSDF sampling for MIS. Unreal physical-unit terminology is used for authored/editor units: directional lights in lux, point/spot punctual lights in candela, and rect area lights in `cd/m^2`. Filament remains the reference only for true punctual lights with no finite emitter shape.
- Implementation: `Lighting/AreaLights.hlsli` is the canonical finite-light shape sampler, while `Lighting/LightSampling.hlsli` owns the direct-light sample payload, incident radiance/PDF separation, area-to-solid-angle PDF conversion, and stable 2D light-sample generation. Directional angular diameter samples a distant cone/solid angle; point `sourceRadius > 0` samples a sphere emitter; spot `sourceRadius > 0` samples a disk emitter oriented by the spot direction; rect lights sample a rectangle with luminance. Area-measure samples convert with `pdfW = pdfA * distance^2 / abs(dot(lightNormal, -wi))`, and finite paths do not reuse point inverse-square attenuation as area-light integration. Primary direct lighting and ray-hit direct lighting both call `SurfaceLighting::AccumulateDirectLightSample`.
- Runtime/editor workflow: rect lights are first-class scene lights through level serialization, cooked records, scene-asset translation, scene snapshots, render scene data, frame-light upload, render-view buffers, shader parameters, renderer settings, and the editor light inspector. The editor shows rect brightness as `Luminance (cd/m^2)`.
- Local deviations: real-time direct lighting currently uses light sampling only; the sample payload carries the PDFs needed for the later MIS/reference path, but BSDF-sampled direct-light MIS is deferred. Emissive mesh/triangle lights remain routed to Stage 10 and must reuse the same sample payload instead of adding a second estimator. Finite spot lights use the existing smooth spot cone as an emission mask over the sampled disk; this is the local approximation to a fully modeled projected emitter.
- Reuse/DRY audit: scanned `RayTracedShadowSampling`, `RayTracedShadowTrace`, `PunctualLights`, `SurfaceLighting`, `RayTracingHitLighting`, `DirectLighting`, shader registrations, pass bindings, render-view lighting data, frame-light upload, glTF/cooked import flow, level serialization, scene snapshots, render scene data, renderer settings, and editor light controls before editing. The previous pass-local directional/point/spot accumulation wrappers were removed; kept helpers own emitter sampling, PDF conversion, visibility distance policy, estimator payload construction, or repeated pass accumulation.
- Cleanup: the old shadow-only finite-light quality CVar, enum, uniform field, and per-light visibility-jitter trace helpers were removed. Production finite lights sample emitter radiance and visibility together.
- Build checks: built `ShowcaseEditor`, `SourceImporters`, `SceneCooker`, and `ShaderCompiler` for `DevelopmentEditor`; cooked `DirectLightingNoRayQuery`, `DirectLighting`, `DirectLightingDeviceAddress`, `IndirectDiffuse`, and `IndirectSpecular` for `DxilSm66` and `SpirV16`.
...
- Hard shadows and one-sample soft shadows still work on descriptor TLAS and device-address TLAS paths.
...
- Reference lineage: Streamline DLRR resource vocabulary is recorded for future reconstruction providers, but Sparkle keeps the image upscaler boundary separate from denoiser/reconstruction resources. `UpscalerInputContract` owns only `ScalingInputColor`, `ScalingOutputColor`, `Depth`, `MotionVectors`, `Exposure`, temporal state, camera state, and extents. NRD, RTXPT, and Streamline DLRR guide the denoiser-side treatment of noisy radiance, demodulated radiance, diffuse/specular albedo, normal, roughness/material metadata, hit distance, and temporal reset/history. Sparkle keeps this as a provider contract stage only: no shader-side DLRR path or provider SDK call was added.
- Implementation: `FrameAssemblyDenoiserProviderResources.IndirectReconstruction` carries explicit indirect reconstruction slots for noisy indirect diffuse/specular radiance, demodulated diffuse/specular radiance, diffuse/specular albedo, material guide, and diffuse/specular sample guides. `FrameAssemblyUpscalerProviderResources` registers only the scaling resources consumed by the upscaler pass.
- Local deviation: provider-ready diffuse/specular albedo and roughness are not exposed by handing a provider raw `GBufferBaseColor` or packed `GBufferMaterial`. Stage 11A writes resolved guide resources with explicit semantics before any DLRR/NRD-capable provider can select indirect reconstruction.
...
- Reference lineage: Streamline DLRR requires provider-visible noisy color/radiance, diffuse albedo, specular albedo, normal, roughness, depth, motion, exposure, and specular hit-distance style inputs; NRD REBLUR/RELAX motivate demodulated radiance plus material-factor application after denoising; RTXPT/Falcor motivate writing denoiser-facing path outputs at the path pass boundary; AMD FidelityFX motivates keeping provider resources as explicit frame-graph products with temporal metadata outside the shader math.
- Implementation: `LightingRenderTargets` now allocates provider guide buffers owned by indirect lighting: demodulated diffuse/specular radiance, diffuse/specular albedo, `IndirectMaterialGuide = float4(roughness, metallic, dielectricF0, valid)`, and diffuse/specular sample guides `float4(hitDistanceWorld, sampleConfidence, lobeId, hitValid)`. `IndirectDiffuse.hlsl` writes shared material guides and diffuse path guides; `IndirectSpecular.hlsl` writes specular path guides. The realtime `IndirectDiffuse` and `IndirectSpecular` radiance math remains unchanged.
- Reuse/DRY audit: scanned `IndirectDiffuse.hlsl`, `IndirectSpecular.hlsl`, `PathLighting.hlsli`, `RayTracingPathSample.hlsli`, lighting target allocation/clear, pass bindings, `FrameAssembly`, `FrameProviderInputs`, `UpscalerInputContract`, and provider docs before editing. The change reuses lighting targets and frame assembly instead of adding a global signal registry, upscaler-side DLRR glue, or pass-local duplicated guide layouts.
- Simplification note: no new guide-wrapper include is kept. Specular/F0 guide derivation reuses `SurfaceLighting::BuildF0`; diffuse albedo, demodulation, and guide payload writes stay direct in the two indirect pass writers. Provider resources stay separate from `UpscalerInputContract`.
- Validation commands: rebuilt `ShaderCompiler`; cooked `IndirectDiffuse` and `IndirectSpecular` for `DxilSm66` and `SpirV16`; rebuilt `ShowcaseEditor` DevelopmentEditor.
...
- Implementation: added an opt-in `ReferencePathTracing` compute pass and shader package controlled by `r.RayTracing.ReferencePathTracing.Enabled`, with `SamplesPerPixel`, `Bounces`, `NormalBias`, and `MaxDistance` CVars. The shader lives under `Passes/Reference`, the C++ pass under `Passes/Reference`, frame scheduling under `Frame/Reference`, and reference target creation/clear under `Frame/Reference/ReferenceRenderTargets`. `FrameRenderPath` now owns top-level renderer-path selection so orchestration can distinguish realtime deferred rendering from path-traced reference rendering without depending directly on a reference-pass settings struct; `FrameBuildResult` and `FrameGraphBuildResult` carry the render path that was actually built. `Frame.cpp` keeps the replacement point visible: shared scene/TLAS setup runs first, then the frame builds either `AddReferenceRenderingPasses` or the existing `AddGBufferPasses` plus `AddLightingPasses` hierarchy, followed by common post-processing, debug, and presentation. The reference path traces primary camera rays through the TLAS, reconstructs the first surface from ray-hit material data, writes `ReferenceDirect`, `ReferenceIndirectDiffuse`, `ReferenceIndirectSpecular`, and `ReferenceSceneColor` as linear HDR reference targets, then copies `ReferenceSceneColor` into the common scene-color products for exposure and presentation.
...
- Simplification note: no reference-only BRDF, material decode, sky transform, alpha-test resolver, GBuffer reader, lighting-target dependency, or path-throughput wrapper was added. The only shared shader change was adding `sampleIndex` to the existing path random-sample generator so high-Spp reference samples can reuse the same path estimator with distinct random streams.
- Validation commands: rebuilt `ShaderCompiler`; validated the typed shader catalog; cooked `ReferencePathTracing`, `Sky`, `IndirectDiffuse`, and `IndirectSpecular` for `DxilSm66` and `SpirV16`; rebuilt `ShowcaseEditor` DevelopmentEditor after the reference target ownership split.
