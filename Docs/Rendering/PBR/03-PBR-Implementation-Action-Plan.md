...
- Local deviation: the current raw shadow target stores a single per-pixel aggregate of the most occluding direct-light sample. Full per-light or temporal SIGMA consumption remains Stage 6, using the same packed signal format rather than a second shadow contract.
