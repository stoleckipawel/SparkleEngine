...
set(smoke_editor_validation_path "${RHI_BACKEND_PARITY_SOURCE_DIR}/Engine/Application/Private/Validation/RhiSmokeEditorValidation.cpp")
...
read_required_rhi_backend_parity_file("${smoke_editor_validation_path}" smoke_editor_validation_text)
...
if(smoke_validation_text)
    ...
    require_rhi_backend_parity_text("${smoke_validation_path}" "${smoke_validation_text}" "SPARKLE_SMOKE_LEVEL_SWITCH_INTERVAL_FRAMES" "runtime smoke validation must be able to exercise level switching across registered levels")
    require_rhi_backend_parity_text("${smoke_validation_path}" "${smoke_validation_text}" "RequestLevelChange" "runtime smoke validation must drive level switching through LevelManager")
    require_rhi_backend_parity_text("${smoke_validation_path}" "${smoke_validation_text}" "completed all level switch targets" "runtime smoke validation must log all-level switch completion evidence")
endif()

if(smoke_editor_validation_text)
    require_rhi_backend_parity_text("${smoke_editor_validation_path}" "${smoke_editor_validation_text}" "SPARKLE_SMOKE_VALIDATE_RHI" "editor smoke validation must be opt-in through environment")
    require_rhi_backend_parity_text("${smoke_editor_validation_path}" "${smoke_editor_validation_text}" "RHI smoke diagnostics capabilities" "editor smoke logs must show diagnostics capability support")
    require_rhi_backend_parity_text("${smoke_editor_validation_path}" "${smoke_editor_validation_text}" "SPARKLE_SMOKE_LEVEL_SWITCH_INTERVAL_FRAMES" "editor smoke validation must be able to exercise level switching across registered levels")
    require_rhi_backend_parity_text("${smoke_editor_validation_path}" "${smoke_editor_validation_text}" "RequestLevelChange" "editor smoke validation must drive level switching through LevelManager")
    require_rhi_backend_parity_text("${smoke_editor_validation_path}" "${smoke_editor_validation_text}" "completed all level switch targets" "editor smoke validation must log all-level switch completion evidence")
endif()
...
if(smoke_doc_text)
    ...
    require_rhi_backend_parity_text("${smoke_doc_path}" "${smoke_doc_text}" "completed all level switch targets" "smoke test docs must name all-level switching completion evidence")
    require_rhi_backend_parity_text("${smoke_doc_path}" "${smoke_doc_text}" "SPARKLE_SMOKE_LEVEL_SWITCH_INTERVAL_FRAMES" "smoke test docs must describe automated level-switch cadence control")
endif()
