...
set_property(GLOBAL PROPERTY SPARKLE_RHI_BACKEND_BOUNDARY_VIOLATIONS "")

function(append_rhi_backend_violation message_text)
    set_property(GLOBAL APPEND PROPERTY SPARKLE_RHI_BACKEND_BOUNDARY_VIOLATIONS "${message_text}")
endfunction()
...
if(vulkan_type_conversions_text)
    ...
    require_text("${vulkan_type_conversions_path}" "${vulkan_type_conversions_text}" "StageMask" "Vulkan ResourceState mapping must use synchronization2 stage masks")
    require_text("${vulkan_type_conversions_path}" "${vulkan_type_conversions_text}" "AccessMask" "Vulkan ResourceState mapping must use synchronization2 access masks")
    require_text("${vulkan_type_conversions_path}" "${vulkan_type_conversions_text}" "ImageLayout" "Vulkan ResourceState mapping must define image layouts explicitly")
...
set(vulkan_descriptor_allocator_path "${RHI_BACKEND_BOUNDARY_SOURCE_DIR}/Engine/RHI/Private/Vulkan/Descriptors/VulkanDescriptorAllocator.cpp")
read_required_file("${vulkan_descriptor_allocator_path}" vulkan_descriptor_allocator_text)
if(vulkan_descriptor_allocator_text)
    require_text("${vulkan_descriptor_allocator_path}" "${vulkan_descriptor_allocator_text}" "vkCreateDescriptorPool" "Vulkan descriptors must allocate descriptor sets from backend-private descriptor pool pages")
    require_text("${vulkan_descriptor_allocator_path}" "${vulkan_descriptor_allocator_text}" "vkDestroyDescriptorPool" "Vulkan descriptor pool pages must own native pool lifetime")
    require_text("${vulkan_descriptor_allocator_path}" "${vulkan_descriptor_allocator_text}" "vkResetDescriptorPool" "Vulkan transient descriptor pages must reset only after the owning frame fence completes")
    require_text("${vulkan_descriptor_allocator_path}" "${vulkan_descriptor_allocator_text}" "vkAllocateDescriptorSets" "Vulkan binding sets must allocate native descriptor sets privately")
    require_text("${vulkan_descriptor_allocator_path}" "${vulkan_descriptor_allocator_text}" "vkUpdateDescriptorSets" "Vulkan binding sets must write descriptor payloads privately")
    require_text("${vulkan_descriptor_allocator_path}" "${vulkan_descriptor_allocator_text}" "VkWriteDescriptorSet" "Vulkan descriptor writes must be typed backend-private writes")
    require_text("${vulkan_descriptor_allocator_path}" "${vulkan_descriptor_allocator_text}" "VkDescriptorImageInfo" "Vulkan descriptor writes must support image and sampler payloads")
    require_text("${vulkan_descriptor_allocator_path}" "${vulkan_descriptor_allocator_text}" "VkDescriptorBufferInfo" "Vulkan descriptor writes must support buffer payloads")
    require_text("${vulkan_descriptor_allocator_path}" "${vulkan_descriptor_allocator_text}" "std::scoped_lock" "Vulkan descriptor allocation must remain thread-safe while per-thread pages are deferred")
endif()
...
set(vulkan_pipeline_layout_builder_path "${RHI_BACKEND_BOUNDARY_SOURCE_DIR}/Engine/RHI/Private/Vulkan/Pipeline/VulkanPipelineLayoutBuilder.cpp")
read_required_file("${vulkan_pipeline_layout_builder_path}" vulkan_pipeline_layout_builder_text)
if(vulkan_pipeline_layout_builder_text)
    require_text("${vulkan_pipeline_layout_builder_path}" "${vulkan_pipeline_layout_builder_text}" "vkCreatePipelineLayout" "Vulkan PSOs must create native pipeline layouts privately")
    require_text("${vulkan_pipeline_layout_builder_path}" "${vulkan_pipeline_layout_builder_text}" "vkDestroyPipelineLayout" "Vulkan PSOs must release pipeline layouts privately")
endif()
...
set(d3d12_memory_allocator_path "${RHI_BACKEND_BOUNDARY_SOURCE_DIR}/Engine/RHI/Private/D3D12/Memory/D3D12GpuMemoryAllocator.cpp")
read_required_file("${d3d12_memory_allocator_path}" d3d12_memory_allocator_text)
if(d3d12_memory_allocator_text)
    require_text("${d3d12_memory_allocator_path}" "${d3d12_memory_allocator_text}" "D3D12MA::ALLOCATION_FLAG_CAN_ALIAS" "D3D12 transient FrameGraph blocks must be backed by alias-capable D3D12MA allocations")
    require_text("${d3d12_memory_allocator_path}" "${d3d12_memory_allocator_text}" "CreateTransientHeap" "D3D12 backend must own transient memory block mechanics")
    require_text("${d3d12_memory_allocator_path}" "${d3d12_memory_allocator_text}" "CreateAliasingResource" "D3D12 backend must own aliasing resource mechanics")
    require_text("${d3d12_memory_allocator_path}" "${d3d12_memory_allocator_text}" "RhiMemoryCategory::FrameGraphTransient" "D3D12 transient diagnostics must use the shared FrameGraphTransient category")
endif()
...
if(vulkan_command_list_text)
    ...
    require_text("${vulkan_command_list_path}" "${vulkan_command_list_text}" "BeginLabel" "Vulkan command list must support debug markers when debug utils are available")
    ...
    require_text("${vulkan_command_list_path}" "${vulkan_command_list_text}" "vkCmdBindDescriptorSets" "Vulkan command list must bind descriptor sets through active binding layouts")
    require_text("${vulkan_command_list_path}" "${vulkan_command_list_text}" "vkCmdPushConstants" "Vulkan command list must translate shared push-constant bindings")
...
if(vulkan_memory_allocator_text)
    ...
    require_text("${vulkan_memory_allocator_path}" "${vulkan_memory_allocator_text}" "VMA_ALLOCATION_CREATE_CAN_ALIAS_BIT" "Vulkan transient FrameGraph blocks must be allocated for aliasing")
    require_text("${vulkan_memory_allocator_path}" "${vulkan_memory_allocator_text}" "vmaCreateAliasingBuffer2" "Vulkan transient buffers must materialize from VMA aliasing allocations")
    require_text("${vulkan_memory_allocator_path}" "${vulkan_memory_allocator_text}" "vmaCreateAliasingImage2" "Vulkan transient images must materialize from VMA aliasing allocations")
    require_text("${vulkan_memory_allocator_path}" "${vulkan_memory_allocator_text}" "RhiMemoryCategory::FrameGraphTransient" "Vulkan transient diagnostics must use the shared FrameGraphTransient category")
    require_text("${vulkan_memory_allocator_path}" "${vulkan_memory_allocator_text}" "QueueDestroyMemoryBlock" "Vulkan transient memory blocks must release only after the owning frame fence retires")
...
if(vulkan_memory_record_text)
    ...
    require_text("${vulkan_memory_record_path}" "${vulkan_memory_record_text}" "VulkanGpuMemoryBlockRecord" "Vulkan transient memory blocks must have backend-private records")
    require_text("${vulkan_memory_record_path}" "${vulkan_memory_record_text}" "ParentMemoryBlock" "Vulkan aliasing resources must not take ownership of the parent transient allocation")
    require_text("${vulkan_memory_record_path}" "${vulkan_memory_record_text}" "AliasingResourceCount" "Vulkan transient memory diagnostics must count aliased resources through the owning block")
...
get_property(RHI_BACKEND_BOUNDARY_VIOLATIONS GLOBAL PROPERTY SPARKLE_RHI_BACKEND_BOUNDARY_VIOLATIONS)
if(RHI_BACKEND_BOUNDARY_VIOLATIONS)
    list(JOIN RHI_BACKEND_BOUNDARY_VIOLATIONS "\n" RHI_BACKEND_BOUNDARY_VIOLATIONS)
