...
| `Engine/Assets/Meshes`, `Engine/Assets/Shaders`, `Engine/Assets/Textures` | This is a non-code asset root. Stage 32 documents its current role as built-in source assets, default/sky textures, and transitional shader source. | Improve/extract. Do not add project content, generated packages, validation captures, or temporary files here; future shader moves should split RHI fixtures and renderer pass shaders into owner roots. |
...
| `Tools/Conversion/AssetConverter` | A direct conversion CLI can become a second production cook path. | Removed as a production path in Stage 30. Useful read-only behavior lives in explicit AssetCooker inspect/debug subcommands. |
...
| `Engine/Assets` current root | Built-in engine source assets, fallback/default textures, sky/environment candidates, and transitional engine shader source tracked by [project-asset-ownership-contract.md](../project-asset-ownership-contract.md). | Project content, generated shader packages, validation captures, local logs, or temporary source dumps. |
| `Projects/*/Assets`, future `Projects/*/Data`, and `Projects/*/Shaders` | Sample/project content used as validation evidence. Runnable level descriptors must reference cookable scene assets or be intentionally empty. | Engine built-ins, tool implementation code, generated artifacts, local logs, or broken runtime level declarations. |
...
| 27-30 | Keep `Tools/Import/SourceImporters` as the role-centered import target; keep generic support in `Tools/Support/ToolConsoleSupport`; keep retired AssetConverter behavior only as explicit AssetCooker inspect/debug commands. |
...
| 32 | Keep `Engine/Assets` documented and narrow until owner-specific shader/data moves happen; keep Showcase levels limited to loadable evidence; keep generated project logs and validation artifacts out of durable source roots. |
