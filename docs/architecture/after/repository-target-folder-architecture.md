...
| 27-30 | Rename/extract `Tools/Import/SourceImportAdapters` to `Tools/Import/SourceImporters`; split `Tools/Cooking/CookCommon` into `Tools/Support/ToolConsoleSupport` and/or `Tools/Cooking/CookDiagnostics`; retire `Tools/Conversion/AssetConverter` as a production path. |
