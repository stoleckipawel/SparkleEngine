...
| PSO key | Explicit printable identity for a pass runtime/pipeline state. | [PipelineRuntimeKey.h](../../Engine/Renderer/Private/PipelineRuntime/PipelineRuntimeKey.h), [pipeline-runtime-contract.md](pipeline-runtime-contract.md) | Renderer pipeline runtime owns the key. Existing pass entry still flows through legacy `std::type_index` storage until Stage 17 removes traits. |
| RenderPassPipelineTraits | Current central traits table that maps pass C++ type to runtime creation logic. | [RenderPassPipelineTraits.h](../../Engine/Renderer/Private/Pipeline/RenderPassPipelineTraits.h) | Compatibility mechanism after Stage 16. Stage 17 replaces it for ordinary pass additions. |
| PipelineRuntimeLibrary | Owner for package loading, capability validation, binding layout creation, explicit PSO key logging, and normalized RHI PSO creation. | [PipelineRuntimeLibrary.h](../../Engine/Renderer/Private/PipelineRuntime/PipelineRuntimeLibrary.h), [pipeline-runtime-contract.md](pipeline-runtime-contract.md) | Implemented in Stage 16 as the compatibility bridge used by existing pass runtimes. |
