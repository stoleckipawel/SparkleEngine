...
| `Debug` | Needs refactor | Renderer diagnostics/debug policy | Debug CVars are tiny but need to stay tied to smoke evidence and validation contracts. | Stage 20, Stage 36 | Debug-view smoke evidence and config contract check. | Debug CVars and view modes are documented and stable in smoke validation. |
...
| `Passes` | Needs refactor | Renderer pass system | Ordinary pass authoring is still too ceremonial even after central traits were removed; current additions touch pass files, shader registration, package identity, frame insertion, cook/list validation, and smoke evidence. | Stage 17, Stage 17A, Stage 17B, Stage 20 | Proof pass before/after touch-count audit plus build/package validation. | Simple compute/raster pass can be added within the pass authoring friction budget without RHI/backend/central runtime edits or duplicated shader/package/layout constants. |
...
| `RayTracing` | Needs refactor | Renderer ray tracing feature system | Stage 18 clarified scene AS ownership and added TLAS/BLAS diagnostic counts, but D3D12/Vulkan camera-motion shadow smoke remains Stage 20 evidence. | Stage 18, Stage 20 | RT capability/TLAS/shadow smoke report. | Ray tracing contract explains BLAS/TLAS lifetime, frame graph AS import, pass services, shadow uniform ownership, rejected mesh/BLAS diagnostics, and unsupported RT fallback reasons. |
...
| `Debug` | Needs refactor | Renderer public debug contract | Debug view modes are key evidence but not yet fully validated across APIs. | Stage 10, Stage 20 | D3D12/Vulkan debug/normal captures. | View modes work in both backends and are documented in validation contracts. |
...
| `CVars` | Needs refactor | RHI runtime config | RHI CVars should not be hidden architecture switches. | Stage 36 | Config contract check. | RHI CVars are documented or removed if not needed. |
...
| Area | Status | Owner layer | Primary risk | Stage / decision question | First validation artifact | Final acceptance evidence |
| --- | --- | --- | --- | --- | --- | --- |
| `Tools/Shaders/ShaderCompiler/CMakeLists.txt` and `Source/main.cpp` | Needs refactor | Shader compiler CLI/build entry | CLI/build options may not expose the evidence needed by pass authoring and runtime package validation. | Stage 4, Stage 17, Stage 29 | `ShaderCompiler` build and command listing. | CLI can compile/list/inspect packages needed by renderer pass authoring without RHI-specific pass edits. |
...
| `Private/Cli` | Needs refactor | Shader tooling UX | Shader/package evidence needs reproducible commands. | Stage 4, Stage 29 | `ShaderCompiler` help/list/inspect command output. | CLI exposes compile, list, inspect, and failure evidence for final validation. |
| `Private/Analysis` and `Private/Inspection` | Needs refactor | Shader package diagnostics | Package stats and inspection are useful but not yet required acceptance artifacts. | Stage 4, Stage 16, Stage 29 | Package stats/inspection output. | Shader package diagnostics are part of final evidence. |
