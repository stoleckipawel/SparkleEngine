...
| `Upscaling` | Needs refactor | Renderer upscaling feature system | Native interop is sensitive and provider/backend ownership must be explicit. | Stage 9, Stage 10, Stage 20 | DLSS capability/provider smoke log. | Provider owns SDK details; RHI owns native metadata; fallback reasons are deterministic. |
...
| Area | Status | Owner layer | Primary risk | Stage / decision question | First validation artifact | Final acceptance evidence |
| --- | --- | --- | --- | --- | --- | --- |
| `Device` | Needs refactor | RHI facade / service access | Root facade is broad and backend implementations are large. | Stage 6, Stage 7, Stage 19 | RHI method ownership table. | Every method has an owner service; facade is slimmed behind service responsibilities. |
...
| `Interop` | Needs refactor | RHI external interop service | Native interop can become a catch-all. | Stage 7, Stage 9, Stage 10 | DLSS/native interop smoke logs. | Interop structs are consumer-scoped and filled deterministically by backends. |
...
| `Diagnostics` | Needs refactor | RHI diagnostics service | Diagnostics must become final evidence, not optional logs. | Stage 7, Stage 10, Stage 20 | Backend diagnostics smoke report. | Debug layers, markers, names, errors, capability logs flow through RHI diagnostics. |
...
| `Config` / `Core` / `CVars` / `UI` | Needs refactor | RHI config/backend selection/UI bridge | Config and UI bridge can grow into hidden coupling if not owned. | Stage 6, Stage 7, Stage 12, Stage 19 | Backend selection and UI bridge docs. | Config/capability contracts are stable; ImGui uses RHI UI bridge without renderer/application internals. |
...
| `Diagnostics` | Needs refactor | Vulkan diagnostics service | Validation warnings must be zero or classified. | Stage 10, Stage 20 | Vulkan validation layer smoke output. | Validation warnings are zero in smoke or documented as known exceptions. |
...
| `Engine/Application/Private/Validation/RhiSmokeValidation.*` | Needs refactor | Runtime smoke orchestration | Runtime smoke has diagnostics but needs backend parity artifacts. | Stage 8, Stage 10, Stage 20 | Runtime smoke logs. | Runtime smoke produces backend, frame, feature, and failure evidence. |
