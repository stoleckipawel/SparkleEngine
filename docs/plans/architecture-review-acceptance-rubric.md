...
| Content pipeline architecture | Candidate can separate source import, cooking, cooked schemas, runtime loading, and renderer resource creation. | SourceImporters, TextureCooker, MeshCooker, MaterialCooker, SceneCooker, AssetCooker, GameFramework loaders, renderer resource managers. | Source import/cook changes update runtime schemas and validation without leaking tool internals into runtime modules. |
...
| Criterion | What Reviewers Usually Look For | Sparkle Acceptance Evidence |
| --- | --- | --- |
...
| Runtime behavior clarity | The system can be explained as an execution path, not just a folder tree. | Diagrams or ordered flows show frame setup, pass declaration, graph compile, resource resolution, command recording, submission, presentation, source import, cooking, package writing, runtime loading, and launcher operation flow. |
...
Critical categories for whole-repository work:
...
2. Which layer owns the decision: Core, Platform, RHI common, D3D12 backend, Vulkan backend, Renderer frame graph, Renderer pass, GameFramework runtime, SourceImporters, TextureCooker, AssetCooker, ShaderCompiler, Launcher, Application/Editor, or vendor provider?
