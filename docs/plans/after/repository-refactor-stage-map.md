...
| 25 | Fully completed | GameFramework lighting snapshots and level lighting descriptions now store dynamic scene light arrays; renderer/RHI fixed light limits remain in GPU packing code; `SparkleGameFramework` no longer links `SparkleRHI`; dependency scans and affected runtime/tool builds passed. | Stage 26 still pairs every cooked loader schema with producer/consumer inspection evidence; reopen Stage 25 only if GameFramework regains Renderer-private, RHI-private, full RHI, tool-private, or source-import policy dependencies. |
