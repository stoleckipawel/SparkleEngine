...
| 17B | Started | Frame insertion boilerplate was reduced with `AddComputeShaderPass`/`AddRasterShaderPass`; `VisualizeBuffers`, `GBuffer`, and sibling ordinary frame passes use the typed helpers; `ShaderCompiler list-shaders --validate` now validates authoring records; `ShowcaseEditor`, `ShaderCompiler`, and boundary checks passed. | Finish the catalog/scaffolder decision, central registration-list cleanup, and final three/four-touch authoring budget for new ordinary passes. |
| 18 | Started | Ray tracing contract now names scene snapshot -> BLAS cache -> TLAS build -> frame graph AS -> DirectLighting ownership; RT diagnostics report candidate/accepted TLAS instances, missing GPU mesh data, rejected BLAS handles, and inline ray-query fallback reason. | Stage 20 runs D3D12/Vulkan camera-motion shadow smoke; Stage 19 extracts backend RT service shape if needed. |
| 19 | Started | D3D12/Vulkan ray tracing prebuild and AS resource creation moved from root facades into composed `D3D12RayTracingServices` and `VulkanRayTracingServices`; `ShowcaseEditor` and `architecture_boundary_check` passed in the VS2026 tree. | Continue service-by-service slimming for command, descriptors, memory, resources, pipeline, presentation/UI, and diagnostics; Stage 20 owns full runtime backend parity smoke. |
