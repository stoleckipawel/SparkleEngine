...
| 27 | Started | `Tools/Import/SourceImporters` is now the production source import folder and CMake target; focused cookers and AssetConverter link `SourceImporters`; import results carry source path/importer id report identity; runtime source/CMake scan has no retired adapter-target references. | Build affected import/cook targets, run targeted Showcase import/cook evidence, run runtime-to-tools dependency scan, then mark complete if green. |
