...
| 16 | Fully completed | `PipelineRuntimeLibrary` and `PipelineRuntimeKey` exist; existing pass runtimes now load packages, validate capabilities, create binding layouts, log explicit backend-normalized PSO keys, and create RHI PSOs through the runtime library. `ShowcaseEditor` and `architecture_boundary_check` passed in the VS2026 build tree. | Stage 17 removes remaining `RenderPassPipelineTraits` and legacy type-index entry identity. |
