...
Codex-facing implementation packets live in [../implementation/stage-prompt-packets.md](../implementation/stage-prompt-packets.md). Human-facing learning notes should be linked here again only if the tutor guide is restored as an active document.
...
| 17 | Fully completed | `RenderPassDefinition` and `RenderPassDefinitionRuntime` exist; all ordinary renderer passes expose definitions; `RenderPassPipelineTraits.h` was deleted; package/layout identity is shared through `RendererShaderPackages`; `ShowcaseEditor`, `ShaderCompiler`, `architecture_boundary_check`, and `ShaderCompiler.exe list-shaders --validate` passed in the VS2026 tree. | Stage 20 captures D3D12/Vulkan smoke with PSO key logs; Stage 29 decides whether `RendererShaderPackages` remains lightweight or becomes a fuller ShaderContracts manifest. |
...
| 17B | Fully completed | Frame insertion boilerplate was reduced with `AddComputeShaderPass`/`AddRasterShaderPass`; `VisualizeBuffers`, `GBuffer`, and sibling ordinary frame passes use the typed helpers; `ShaderCompiler list-shaders --validate` validates authoring records; `RendererGlobalShaders.cpp` and `RendererShaderRegistration.h` were deleted; CMake links renderer shader registration objects into runtime consumers without a hand-written central C++ list; `ShowcaseEditor`, `ShowcaseRuntime`, `ShaderCompiler`, boundary checks, and Stage 20 smoke passed. | Stage 29 owns any larger ShaderContracts catalog/scaffolder decision; do not reopen Stage 17B for presentation-only cleanup. |
| 18 | Fully completed | Ray tracing contract now names scene snapshot -> BLAS cache -> TLAS build -> frame graph AS -> DirectLighting ownership; RT diagnostics report candidate/accepted TLAS instances, missing GPU mesh data, rejected BLAS handles, and inline ray-query fallback reason. Stage 20 D3D12/Vulkan editor/runtime smoke reported RT and inline ray query available, `builtBlas=103`, `tlasInstances=103`, zero missing GPU mesh data, and zero rejected BLAS handles on both backends. Deterministic camera-motion smoke passed for D3D12/Vulkan editor/runtime with `tlasValid=true` and `frameGraphUnresolvedBarrierWarnings=0`. | Reopen only if renderer RT code leaks backend headers, shadow/pass data moves into RHI, or camera-motion RT smoke regresses. |
| 19 | Started | D3D12/Vulkan ray tracing prebuild and AS resource creation moved from root facades into composed `D3D12RayTracingServices` and `VulkanRayTracingServices`; `ShowcaseEditor` and `architecture_boundary_check` passed in the VS2026 tree. Stage 20 full backend smoke passed for both backends, but only ray tracing service extraction is complete so far. | Continue service-by-service slimming for command, descriptors, memory, resources, pipeline, presentation/UI, and diagnostics. |
| 20 | Fully completed | `ShaderCompiler`, `SparkleLauncher`, `ShowcaseEditor`, `ShowcaseRuntime`, and `architecture_boundary_check` passed in `build/windows-vs2026-stage5`; launcher-shaped D3D12/Vulkan editor Lit and GBufferNormal smoke produced BMP captures; D3D12/Vulkan runtime smoke exited `0`; all six static smoke runs reported zero diagnostic matches and zero frame graph unresolved warnings; shader package validation reported 17 valid typed registrations; lit/normal BMP byte comparisons are recorded under `artifacts/validation/stage20`. Deterministic camera-motion smoke passed for D3D12/Vulkan editor/runtime and is recorded under `artifacts/validation/stage20-camera-motion`. | Stage 19 is still open for deeper backend service slimming, but the Stage 20 validation milestone itself is complete. |
| 23 | Fully completed | `repository-coverage-status.md` has a Stage 23 freeze matrix for Engine, Tools, Projects, CMake, CI, docs, and External vendor roots with owner, producer/consumer role, allowed dependencies, forbidden dependencies, validation target, active refactor stage, and final acceptance evidence. `repository-system-map.md` has the broad Stage 23 dependency-intent table. Docs/link scans passed; no build required. | Reopen only if a durable source root is added without a coverage row or if a later stage changes dependency intent without updating the map. |
| 24 | Fully completed | Core named-tool log routing and package-root launcher probing were replaced with generic process/package markers; Platform no longer links or includes ImGui and receives host-owned input capture through a query; `InputLayer::Editor` became the neutral `InputLayer::UI`; Core/Platform policy scans and targeted builds passed. | Reopen only if Core/Platform regain renderer, RHI, tool, launcher, gameplay, editor, or scheduler policy. |
| 25 | Fully completed | `LevelDesc::lights` stores typed `SceneLightDesc` arrays with directional, point, and spot parity; the old directional-only level-lighting builder and one-field `LevelLightingDesc` wrapper were deleted; lighting snapshots remain dynamic scene light arrays; renderer/RHI fixed light limits remain in GPU packing code; `SparkleGameFramework` no longer links `SparkleRHI`; dependency scans and affected runtime/tool builds passed. | Stage 26 still pairs every cooked loader schema with producer/consumer inspection evidence; reopen Stage 25 only if GameFramework regains Renderer-private, RHI-private, full RHI, tool-private, source-import policy dependencies, or directional-only lighting schema paths. |
| 25C | Fully completed | Shallow dead-wrapper/pass-through scan removed the obvious Stage 25 lighting wrapper debt: `LevelLightingDesc`, `LevelLightingSceneBuilder`, and `BuildLevelDesc`; `LightingSectionParser` now has a tiny header and private `.cpp` implementation. | Repeat this scan in later broad stages; keep small wrappers only when they own invariants, ABI, serialization, diagnostics, type-safety, or cross-owner handoff value. |
| 26 | Fully completed | Runtime cooked asset loader/schema pairing tables connect cooked mesh/material/texture/scene/animation/skeleton artifacts to producer cookers, public schema headers, runtime loaders, renderer/runtime consumers, and validation paths; runtime cooked loaders share `CookedAssetLoaderDiagnostics` asset-oriented failure messages; `SparkleGameFramework`, `AssetCooker`, `TextureCooker`, `ShowcaseRuntime`, Showcase `cook-assets`, Showcase `cook-textures`, D3D12 runtime smoke, boundary checks, include scans, and `git diff --check` passed. | Reopen only if a cooked schema changes without paired producer/loader/runtime/renderer validation, GameFramework parses source assets, tools include private loaders, or loader diagnostics lose asset/path/schema/record/reason context. |
| 27 | Fully completed | `Tools/Import/SourceImporters` is the production source import folder and CMake target; focused cookers and AssetConverter link `SourceImporters`; import results carry source path/importer id report identity; Showcase `cook-assets` reports `importer=GltfImporter`; `SourceImporters`, `MeshCooker`, `MaterialCooker`, `SceneCooker`, `AssetCooker`, and `AssetConverter` built; runtime-to-tools dependency scan, architecture boundary check, and `git diff --check` passed. | Reopen only if adapter/importer folders return as parallel production paths, runtime modules include importer/tool headers, or cookers bypass imported DTO/report contracts. |
| 28 | Fully completed | `Tools/Support/ToolConsoleSupport` replaced the retired common support target; `AssetCooker`, `TextureCooker`, and `ShaderCompiler` link the precise support target; focused cookers remain focused transformation targets; `ToolConsoleSupport`, `TextureCooker`, `MeshCooker`, `MaterialCooker`, `SceneCooker`, `AssetCooker`, and `ShaderCompiler` built; Showcase `cook-assets` and `cook-textures` passed; controlled invalid texture request reported asset id, source path, output path, and reason; dependency/boundary/stale-name scans and `git diff --check` passed. | Reopen only if broad common support names return as production targets, runtime modules include tool implementation headers, focused cookers absorb project/UI orchestration, or failure reports lose source/artifact/reason context. |
| 29 | Fully completed | `Tools/Shaders/ShaderContracts` defines shader catalog, stage, verification failure, and deterministic job identity DTOs; `ShaderCompiler` list/inspect/validation/cook planning consume `ShaderContractCatalog`; cook progress reports job keys; `ShaderCompiler` links `SparkleRendererShaderRegistrations`, not full `SparkleRenderer`; build, list, validate, inspect-shader, targeted Sky cook, inspect-package, deterministic list comparison, and renderer-private dependency scans passed. | Reopen only if ShaderCompiler links full renderer runtime, compiler internals become a pass metadata source of truth, backend compiler details leak into runtime contracts, or shader job/report identity loses package/backend/options/hash fields. |
...
| Docs / architecture and implementation evidence | 1, 2, 23, 29, 30, 34, 35, 36 |
...
| 27 | `Tools/Import/SourceImporters` is the role-centered import folder and CMake target. | Adapter/importer folders kept as parallel production paths. |
| 28 | Focused cookers and `ToolConsoleSupport` have separate role folders; shared cook diagnostics become a target only when real shared cook-domain logic exists. | `CookCommon`, `Tools/Common`, `Tools/Utils`, or generic conversion folders as production architecture. |
...
| 28 | Focused cookers and `ToolConsoleSupport`. | `CookCommon` as broad policy sink or focused cookers owning orchestration. |
...
- Open the Codex packet in [../implementation/stage-prompt-packets.md](../implementation/stage-prompt-packets.md) and any active tutor note if that guide is restored.
